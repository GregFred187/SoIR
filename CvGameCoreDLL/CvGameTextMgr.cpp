	...
	if (pUnit->isSpy())
	{
		...
		if (NO_PLAYER != eOwner && GET_PLAYER(eOwner).getTeam() != pUnit->getTeam())
		{
			...
			else if (pUnit->getFortifyTurns() > 0)
			{
				...
				// edead: start SuperSpies
				iModifier -= pUnit->getUpgradeDiscount();
				iModifier = std::max(iModifier, -(GC.getDefineINT("MAX_FORTIFY_TURNS") * GC.getDefineINT("ESPIONAGE_EACH_TURN_UNIT_COST_DECREASE")));
				// edead: end
				if (0 != iModifier)
				{
					szBuffer.append(NEWLINE);
...
void CvGameTextMgr::setUnitHelp(CvWStringBuffer &szString, const CvUnit* pUnit, bool bOneLine, bool bShort)
{
	...
	szTempBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_UNIT_TEXT"), wcrtrim(pUnit->getName().GetCString())); // edead: trim whitespace
	//szTempBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_UNIT_TEXT"), pUnit->getName().GetCString()); // edead
	...
	if (!bOneLine)
	{
		...
		if (!bShort)
		{
			...
			if (pUnit->getExtraEnemyHeal() != 0 && !pUnit->isSpy()) // edead: SuperSpies
			{
				szString.append(NEWLINE);
			...
			if (pUnit->getExtraNeutralHeal() != 0 && !pUnit->isSpy()) // edead: SuperSpies
			{
				szString.append(NEWLINE);
			...
			if (pUnit->getExtraFriendlyHeal() != 0 && !pUnit->isSpy()) // edead: SuperSpies
			{
				szString.append(NEWLINE);
			...
			if (pUnit->getSameTileHeal() != 0 && !pUnit->isSpy()) // edead: SuperSpies
			{
				szString.append(NEWLINE);
			...
			if (pUnit->getAdjacentTileHeal() != 0 && !pUnit->isSpy()) // edead: SuperSpies
			{
				szString.append(NEWLINE);
		...
		if (pUnit->getCollateralDamageProtection() > 0)
		{
			...
			{
				szString.append(NEWLINE);
				szString.append(gDLL->getText("TXT_KEY_PROMOTION_ASSASSIN_TEXT_SPY", pUnit->getCollateralDamageProtection()));
			}
			else
			{
				szString.append(NEWLINE);
		...
		if (pUnit->cityAttackModifier() == pUnit->cityDefenseModifier())
		{
			...
			if (pUnit->cityAttackModifier() != 0 && !pUnit->isSpy()) // edead: SuperSpies
			{
				szString.append(NEWLINE);
		...
		else
		{
			...
			if (pUnit->cityDefenseModifier() != 0 && !pUnit->isSpy()) // edead: SuperSpies
			{
				szString.append(NEWLINE);
		...
		if (pPlot->isVisible(GC.getGameINLINE().getActiveTeam(), GC.getGameINLINE().isDebugMode()))
		{
			...
			while(pUnitNode != NULL)
			{
				...
				if (pHeadUnit && pHeadUnit->isGroupHead() && !pHeadUnit->isCargo() && !pHeadUnit->isInvisible(GC.getGameINLINE().getActiveTeam(), GC.getGameINLINE().isDebugMode()))
				{
					...
					//szString.append(CvWString::format(SETCOLR L"%s" ENDCOLR, 255,190,0,255, pHeadUnit->getName().GetCString())); // edead
					szString.append(CvWString::format(SETCOLR L"%s" ENDCOLR, 255,190,0,255, wcrtrim(pHeadUnit->getName().GetCString()))); // edead: trim whitespace
					...
					for (uint i = 0; i < aCargoUnits.size(); ++i)
					{
						...
						{
							// name and unitai
							//szString.append(CvWString::format(SETCOLR L"\n %s" ENDCOLR, TEXT_COLOR("COLOR_ALT_HIGHLIGHT_TEXT"), pCargoUnit->getName().GetCString())); // edead
							szString.append(CvWString::format(SETCOLR L"\n %s" ENDCOLR, TEXT_COLOR("COLOR_ALT_HIGHLIGHT_TEXT"), wcrtrim(pCargoUnit->getName().GetCString()))); // edead: trim whitespace
					...
					while(pUnitNode3 != NULL)
					{
						...
						if (pUnit && (pUnit != pHeadUnit) && (pUnit->getGroupID() == pHeadUnit->getGroupID()) && !pUnit->isInvisible(GC.getGameINLINE().getActiveTeam(), GC.getGameINLINE().isDebugMode()))
						{
							...
							// name and unitai
							//szString.append(CvWString::format(SETCOLR L"\n-%s" ENDCOLR, TEXT_COLOR("COLOR_UNIT_TEXT"), pUnit->getName().GetCString())); // edead
							szString.append(CvWString::format(SETCOLR L"\n-%s" ENDCOLR, TEXT_COLOR("COLOR_UNIT_TEXT"), wcrtrim(pUnit->getName().GetCString()))); // edead: trim whitespace
...
//			}
	...
	else if (bShift && !bAlt && (gDLL->getChtLvl() > 0))
	{
		...
		szTempBuffer.Format(L"\nFound Value: %d, (Calc:%d, Start:%d, Map:%d)", iActualFoundValue, iCalcFoundValue, iStartingFoundValue, pPlot->getFoundModifier(eActivePlayer)); // edead: debug
		szString.append(szTempBuffer);
			...
			if (kPlayer.isAlive())
            {
					...
					|| ((pPlot->getOwner() == iI) && (iBestAreaFoundValue > 0)))
				{
					...
					if (iCalcFoundValue > 0 || iStartingFoundValue > 0)
					{
						...
						szTempBuffer.Format(L" (Calc:%d,Start:%d,Map:%d)", iCalcFoundValue, iStartingFoundValue, pPlot->getFoundModifier(ePlayer)); // edead: debug
						szString.append(szTempBuffer);
					}
	...
	else
	{
		...
		if (pPlot->getRegionID() != -1)
		{
			szString.append(pPlot->getRegionName(true));
			long result = -1;
			CyArgsList argsList;
			argsList.add(GC.getGameINLINE().getActivePlayer());
			argsList.add(pPlot->getRegionID());
			gDLL->getPythonIFace()->callFunction(PYScreensModule, "getRegionStabilityLevel", argsList.makeFunctionArgs(), &result);
			int iResult = (int)result;
			if (iResult == 0) {
				szString.append(gDLL->getText(" "));
				szString.append(gDLL->getText("TXT_KEY_TOOLTIP_FOREIGN"));
			}
			else if (iResult == 2) {
				szString.append(gDLL->getText(" "));
				szString.append(gDLL->getText("TXT_KEY_TOOLTIP_CONTESTED"));
			}
			else if (iResult == 3) {
				szString.append(gDLL->getText(" "));
				szString.append(gDLL->getText("TXT_KEY_TOOLTIP_BORDER"));
			}
			else if (iResult == 4) {
				szString.append(gDLL->getText(" "));
				szString.append(gDLL->getText("TXT_KEY_TOOLTIP_CORE"));
			}
			szString.append(NEWLINE);
		}
		// edead: end
		...
		//if (pPlot->isLake()) // edead (salt lakes)
...
{
	return; // edead: invisible traits/UPs
	PROFILE_FUNC();
	...
	if (!bDawnOfMan)
	{
		...
		// edead - start Diplomatic trait bonus
		if (GC.getTraitInfo(eTrait).getDiplomacyModifier() != 0)
		{
			szHelpString.append(gDLL->getText("TXT_KEY_TRAIT_DIPLOMACY", GC.getTraitInfo(eTrait).getDiplomacyModifier()));
		}
		// edead - end
...
void CvGameTextMgr::parseCivInfos(CvWStringBuffer &szInfoText, CivilizationTypes eCivilization, bool bDawnOfMan, bool bLinks)
{
	...
	{
		// edead: start
		// note that while I added like 10 python callbacks here, this doesn't affect performance
		// because it's only called during civ-selection and in the civilopedia
		CvString szResult;
		long lResult;
		CyArgsList argsList;
		argsList.add(eCivilization); // XML index must match player ID
		// edead: end
...
*/		}
		...
		// Rhye / edead: start starting yYear & loading time
		if (bDawnOfMan)
		{
		...
		lResult = 0;
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "getStartingYear", argsList.makeFunctionArgs(), &lResult);
		if (lResult < 0)
		{
			szBuffer.Format(L"%d %s", (int)lResult, gDLL->getText("TXT_KEY_BC").GetCString());
		}
		else
		{
			szBuffer.Format(L"%d %s", (int)lResult, gDLL->getText("TXT_KEY_AD").GetCString());
		}
		...
		if (bDawnOfMan)
		{
			...
			{
				lResult = 0;
				gDLL->getPythonIFace()->callFunction(PYScreensModule, "getLoadingTime", argsList.makeFunctionArgs(), &lResult);
				if (lResult == 1)
				{
					szBuffer.Format(L"%s %d %s%s", gDLL->getText("TXT_KEY_LOADING_TIME").GetCString(), (int)lResult, gDLL->getText("TXT_KEY_MINUTE").GetCString(), NEWLINE);
					szInfoText.append(szBuffer.GetCString());
				}
				else if (lResult > 1)
				{
					szBuffer.Format(L"%s %d %s%s", gDLL->getText("TXT_KEY_LOADING_TIME").GetCString(), (int)lResult, gDLL->getText("TXT_KEY_MINUTES").GetCString(), NEWLINE);
					szInfoText.append(szBuffer.GetCString());
				}
				else
				{
		...
		if (!bFound)
		{
			...
			{
				szTempString.Format(L"%s  %c%s", NEWLINE, gDLL->getSymbolID(BULLET_CHAR), szText.GetCString());
			}
			szInfoText.append(szTempString);
		}
		...
		else
		{
			...
			szTempString.Format(NEWLINE SETCOLR L"%s" ENDCOLR NEWLINE, TEXT_COLOR("COLOR_ALT_HIGHLIGHT_TEXT"), szText.GetCString());
		}
		szInfoText.append(szTempString);
		...
		{
			szResult="";
			gDLL->getPythonIFace()->callFunction(PYScreensModule, "getUniquePowerName", argsList.makeFunctionArgs(), &szResult);
			szText = gDLL->getText(szResult);
		}
		else
		{
			szText = gDLL->getText("TXT_KEY_UNIQUE_POWER_NONE");
		}
		...
		{
			szResult="";
			gDLL->getPythonIFace()->callFunction(PYScreensModule, "getUniquePowerDescription", argsList.makeFunctionArgs(), &szResult);
			szText = gDLL->getText(szResult);
			szTempString.Format(L"%s" NEWLINE, szText.GetCString());
			szInfoText.append(szTempString);
		}
		...
		if (GC.getCivilizationInfo(eCivilization).isPlayable())
		{
			...
			szResult="";
			gDLL->getPythonIFace()->callFunction(PYScreensModule, "getGoalText1", argsList.makeFunctionArgs(), &szResult);
			szText += gDLL->getText("TXT_KEY_ICON_BULLET");
			szText += gDLL->getText(szResult);
			
			szResult="";
			gDLL->getPythonIFace()->callFunction(PYScreensModule, "getGoalText2", argsList.makeFunctionArgs(), &szResult);
			szText += NEWLINE L"  " + gDLL->getText("TXT_KEY_ICON_BULLET");
			szText += gDLL->getText(szResult);
			
			szResult="";
			gDLL->getPythonIFace()->callFunction(PYScreensModule, "getGoalText3", argsList.makeFunctionArgs(), &szResult);
			szText += NEWLINE L"  " + gDLL->getText("TXT_KEY_ICON_BULLET");
			szText += gDLL->getText(szResult);
			szText += NEWLINE L"  ";
		...
		lResult = 0;
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "getRating1", argsList.makeFunctionArgs(), &lResult);
		szText += gDLL->getText("TXT_KEY_MM_TRADE");
		for (int iI = 0; iI < lResult; iI++) szText += gDLL->getText("TXT_KEY_ICON_STAR");
		
		lResult = 0;
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "getRating2", argsList.makeFunctionArgs(), &lResult);
		szText += NEWLINE L"  " + gDLL->getText("TXT_KEY_MM_PRODUCTION");
		for (int iI = 0; iI < lResult; iI++) szText += gDLL->getText("TXT_KEY_ICON_STAR");

		lResult = 0;
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "getRating3", argsList.makeFunctionArgs(), &lResult);
		szText += NEWLINE L"  " + gDLL->getText("TXT_KEY_MM_CULTURE");
		for (int iI = 0; iI < lResult; iI++) szText += gDLL->getText("TXT_KEY_ICON_STAR");

		lResult = 0;
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "getRating4", argsList.makeFunctionArgs(), &lResult);
		szText += NEWLINE L"  " + gDLL->getText("TXT_KEY_MM_GROWTH");
		for (int iI = 0; iI < lResult; iI++) szText += gDLL->getText("TXT_KEY_ICON_STAR");

		lResult = 0;
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "getRating5", argsList.makeFunctionArgs(), &lResult);
...
void CvGameTextMgr::parsePromotionHelp(CvWStringBuffer &szBuffer, PromotionTypes ePromotion, const wchar* pcNewline)
{
	...
	{
		//SuperSpies: TSHEEP / EDEAD Display Spy Promotions Differently
		if(GC.getPromotionInfo(ePromotion).getSound()[5] == 'P')
		{
			szBuffer.append(pcNewline);
			szBuffer.append(gDLL->getText("TXT_KEY_PROMOTION_CITY_ATTACK_TEXT_SPY", GC.getPromotionInfo(ePromotion).getCityAttackPercent()));
		}
		else
		{
			szBuffer.append(pcNewline);
	...
	if (GC.getPromotionInfo(ePromotion).getCityDefensePercent() != 0)
	{
		...
		{
			szBuffer.append(pcNewline);
			szBuffer.append(gDLL->getText("TXT_KEY_PROMOTION_INTERCEPT_TEXT_SPY_COUNTER", GC.getPromotionInfo(ePromotion).getCityDefensePercent()));
		}
		else
		{
			szBuffer.append(pcNewline);
	...
	if (GC.getPromotionInfo(ePromotion).getCollateralDamageProtection() != 0)
	{
		...
		{
			szBuffer.append(pcNewline);
			szBuffer.append(gDLL->getText("TXT_KEY_PROMOTION_ASSASSIN_TEXT_SPY", GC.getPromotionInfo(ePromotion).getCollateralDamageProtection()));
		}
		else
		{
			szBuffer.append(pcNewline);
	...
	// edead: start
	if (GC.getPromotionInfo(ePromotion).getBarbarianPercent() != 0)
	{
		szBuffer.append(pcNewline);
		szBuffer.append(gDLL->getText("TXT_KEY_PROMOTION_BARBARIAN_TEXT", GC.getPromotionInfo(ePromotion).getBarbarianPercent()));
	}
	// edead: end
...
void CvGameTextMgr::parseCivicInfo(CvWStringBuffer &szHelpText, CivicTypes eCivic, bool bCivilopediaText, bool bPlayerContext, bool bSkipName)
{
	...
	for (iI = 0; iI < GC.getNumSpecialBuildingInfos(); ++iI)
	{
		if (GC.getCivicInfo(eCivic).isSpecialBuildingNotRequired(iI))
		{
			...
			// edead - start ugly text description
			if (iI == 0) // first building in CIV4SpecialBuildingInfo.xml = Apostolic Palace
			{
				szHelpText.append(NEWLINE);
				szHelpText.append(gDLL->getText("TXT_KEY_CIVIC_BUILD_INQUISITORS"));
			}
			else
			{
			szHelpText.append(NEWLINE);
...
/************************************************************************************************/
	...
	//	edead - start General Civic Happiness
	if (GC.getCivicInfo(eCivic).getCivicHappiness() != 0)
	{
		szHelpText.append(NEWLINE);
		szHelpText.append(gDLL->getText("TXT_KEY_CIVIC_HAPPINESS", abs(GC.getCivicInfo(eCivic).getCivicHappiness()), ((GC.getCivicInfo(eCivic).getCivicHappiness() > 0) ? gDLL->getSymbolID(HAPPY_CHAR) : gDLL->getSymbolID(UNHAPPY_CHAR))));
	}
	//	edead - end
...
/************************************************************************************************/
    ...
    //	edead start - Coastal Trade Routes from FfH
...
void CvGameTextMgr::setBasicUnitHelp(CvWStringBuffer &szBuffer, UnitTypes eUnit, bool bCivilopediaText)
{
	...
	// edead: start SuperSpies
	if (GC.getUnitInfo(eUnit).isSpy() && GC.getUnitInfo(eUnit).getCityDefenseModifier() != 0)
	{
		szBuffer.append(NEWLINE);
		szBuffer.append(gDLL->getText("TXT_KEY_UNIT_INTERCEPT_AIRCRAFT_SPY_COUNTER", GC.getUnitInfo(eUnit).getCityDefenseModifier()));
	}
	else
	{
	// edead: end
	...
	if (NO_UNIT != eDefaultUnit && eDefaultUnit != eUnit)
	{
		for (iI  = 0; iI < GC.getNumCivilizationInfos(); ++iI)
		{
			...
			if (eUniqueUnit == eUnit)
			{
				...
				// edead: start - moved here from below so that the text only shows up if unit is actually a UU
				szBuffer.append(NEWLINE);
...
void CvGameTextMgr::setProjectHelp(CvWStringBuffer &szBuffer, ProjectTypes eProject, bool bCivilopediaText, CvCity* pCity)
{
	...
	// edead: start help
	if (!CvWString(GC.getProjectInfo(eProject).getHelp()).empty())
	{
		//szBuffer.append(NEWLINE);
		szBuffer.append(GC.getProjectInfo(eProject).getHelp());
	}
	// edead: end
...
void CvGameTextMgr::setProcessHelp(CvWStringBuffer &szBuffer, ProcessTypes eProcess)
{
	...
	for (iI = 0; iI < NUM_COMMERCE_TYPES; ++iI)
	{
		...
		{
			szBuffer.append(NEWLINE);
			// edead: Sindhi UP: Craftsmanship
			if (GC.getGame().getActivePlayer() == SINDH)
				if ((eProcess == COMMERCE_GOLD) || (eProcess == COMMERCE_CULTURE))
...
void CvGameTextMgr::setAngerHelp(CvWStringBuffer &szBuffer, CvCity& city)
{
	...
	else if (city.unhappyLevel() > 0)
	{
		...
		// edead - start
		iNewAnger -= std::min(0, GET_PLAYER(city.getOwnerINLINE()).getCivicHappiness());
		iAnger = ((iNewAnger - iOldAnger) + std::min(0, iOldAnger));
		if (iAnger > 0)
		{
			szBuffer.append(gDLL->getText("TXT_KEY_ANGER_CIVIC", iAnger));
			szBuffer.append(NEWLINE);
		}
		iOldAnger = iNewAnger;
		// edead - end
...
void CvGameTextMgr::setCorporationHelp(CvWStringBuffer &szBuffer, CorporationTypes eCorporation, bool bCivilopedia)
{
	...
	// edead: start comment - no competition
...
void CvGameTextMgr::getDealString(CvWStringBuffer& szBuffer, PlayerTypes ePlayer1, PlayerTypes ePlayer2, const CLinkList<TradeData>* pListPlayer1, const CLinkList<TradeData>* pListPlayer2, PlayerTypes ePlayerPerspective)
{
	...
	if (!szDealOne.isEmpty())
	{
		if (!szDealTwo.isEmpty())
		{
			if (ePlayerPerspective == ePlayer1)
			{
				...
				szBuffer.append(gDLL->getText("TXT_KEY_MISC_OUR_DEAL", szDealOne.getCString(), GET_PLAYER(ePlayer2).getCivilizationShortDescriptionKey(), szDealTwo.getCString())); // edead
			}
			else if (ePlayerPerspective == ePlayer2)
			{
				...
				szBuffer.append(gDLL->getText("TXT_KEY_MISC_OUR_DEAL", szDealTwo.getCString(), GET_PLAYER(ePlayer1).getCivilizationShortDescriptionKey(), szDealOne.getCString())); // edead
			}
			else
			{
				...
				szBuffer.append(gDLL->getText("TXT_KEY_MISC_DEAL", GET_PLAYER(ePlayer1).getCivilizationShortDescriptionKey(), szDealOne.getCString(), GET_PLAYER(ePlayer2).getCivilizationShortDescriptionKey(), szDealTwo.getCString())); // edead
			}
		}
		else
		{
			if (ePlayerPerspective == ePlayer1)
			{
				...
				szBuffer.append(gDLL->getText("TXT_KEY_MISC_DEAL_ONESIDED_OURS", szDealOne.getCString(), GET_PLAYER(ePlayer2).getCivilizationShortDescriptionKey())); // edead
			}
			else if (ePlayerPerspective == ePlayer2)
			{
				//szBuffer.append(gDLL->getText("TXT_KEY_MISC_DEAL_ONESIDED_THEIRS", szDealOne.getCString(), GET_PLAYER(ePlayer1).getNameKey()));
				szBuffer.append(gDLL->getText("TXT_KEY_MISC_DEAL_ONESIDED_THEIRS", szDealOne.getCString(), GET_PLAYER(ePlayer1).getCivilizationShortDescriptionKey())); // edead
			}
			else
			{
				...
				szBuffer.append(gDLL->getText("TXT_KEY_MISC_DEAL_ONESIDED", GET_PLAYER(ePlayer1).getCivilizationShortDescriptionKey(), szDealOne.getCString(), GET_PLAYER(ePlayer2).getCivilizationShortDescriptionKey())); // edead
			}
		}
	}
	else if (!szDealTwo.isEmpty())
	{
		if (ePlayerPerspective == ePlayer1)
		{
			...
			szBuffer.append(gDLL->getText("TXT_KEY_MISC_DEAL_ONESIDED_THEIRS", szDealTwo.getCString(), GET_PLAYER(ePlayer2).getCivilizationShortDescriptionKey())); // edead
		}
		else if (ePlayerPerspective == ePlayer2)
		{
			//szBuffer.append(gDLL->getText("TXT_KEY_MISC_DEAL_ONESIDED_OURS", szDealTwo.getCString(), GET_PLAYER(ePlayer1).getNameKey()));
			szBuffer.append(gDLL->getText("TXT_KEY_MISC_DEAL_ONESIDED_OURS", szDealTwo.getCString(), GET_PLAYER(ePlayer1).getCivilizationShortDescriptionKey())); // edead
		}
		else
		{
			...
			szBuffer.append(gDLL->getText("TXT_KEY_MISC_DEAL_ONESIDED", GET_PLAYER(ePlayer2).getCivilizationShortDescriptionKey(), szDealTwo.getCString(), GET_PLAYER(ePlayer1).getCivilizationShortDescriptionKey())); // edead
		}
	}
}
...
void CvGameTextMgr::getAttitudeString(CvWStringBuffer& szBuffer, PlayerTypes ePlayer, PlayerTypes eTargetPlayer)
{
		...
		if (kLoopTeam.isAlive())
		{
			if (NO_PLAYER != eTargetPlayer)
			{
				...
				if (kTargetTeam.isHasMet((TeamTypes)iTeam))
				{
					...
					{
						szBuffer.append(NEWLINE);
						//szBuffer.append(gDLL->getText("TXT_KEY_ATTITUDE_VASSAL_OF", kLoopTeam.getName().GetCString())); // edead
						szBuffer.append(gDLL->getText("TXT_KEY_ATTITUDE_VASSAL_OF", GET_PLAYER((PlayerTypes)iTeam).getCivilizationDescription())); // edead
						setVassalRevoltHelp(szBuffer, (TeamTypes)iTeam, kTeam.getID());
					}
					else if (kLoopTeam.isVassal(kTeam.getID()))
					{
						szBuffer.append(NEWLINE);
						//szBuffer.append(gDLL->getText("TXT_KEY_ATTITUDE_MASTER_OF", kLoopTeam.getName().GetCString())); // edead
						szBuffer.append(gDLL->getText("TXT_KEY_ATTITUDE_MASTER_OF", GET_PLAYER((PlayerTypes)iTeam).getCivilizationDescription())); // edead
					}
				}
			}
		}
	...
	for (iPass = 0; iPass < 2; iPass++)
	{
		...
		iAttitudeChange = kPlayer.AI_getColonyAttitude(eTargetPlayer); // edead
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
		...
		iAttitudeChange = kPlayer.AI_getAttitudeExtra(eTargetPlayer); // edead
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
		...
		// edead - start bonus to Diplomacy from Diplomatic trait
		iAttitudeChange = GET_PLAYER(eTargetPlayer).getDiplomacyModifier();
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
		...
		// edead - start bonus to Diplomacy from the Caliph title
		iAttitudeChange = kPlayer.AI_getAttitudeCaliph(eTargetPlayer);
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText(((iAttitudeChange > 0) ? "TXT_KEY_ATTITUDE_CALIPH" : "TXT_KEY_ATTITUDE_CALIPH"), iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		// edead - end
	
		// edead - start bonus to Diplomacy from the Sharif of Mecca title
		iAttitudeChange = kPlayer.AI_getAttitudeSharif(eTargetPlayer);
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText(((iAttitudeChange > 0) ? "TXT_KEY_ATTITUDE_SHARIF" : "TXT_KEY_ATTITUDE_SHARIF"), iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		// edead - end

		// edead - start bonus to Diplomacy from the Raja Vikramaditya title
		iAttitudeChange = kPlayer.AI_getAttitudeRaja(eTargetPlayer);
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText(((iAttitudeChange > 0) ? "TXT_KEY_ATTITUDE_RAJA" : "TXT_KEY_ATTITUDE_RAJA"), iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		// edead - end
...
void CvGameTextMgr::getEspionageString(CvWStringBuffer& szBuffer, PlayerTypes ePlayer, PlayerTypes eTargetPlayer)
{
	if (!GC.getGameINLINE().isOption(GAMEOPTION_NO_ESPIONAGE))
	{
		...
		szBuffer.append(gDLL->getText("TXT_KEY_ESPIONAGE_AGAINST_PLAYER", kTargetPlayer.getCivilizationDescriptionKey(), kTeam.getEspionagePointsAgainstTeam(kTargetPlayer.getTeam()), GET_TEAM(kTargetPlayer.getTeam()).getEspionagePointsAgainstTeam(kPlayer.getTeam()))); //Rhye
	}
}
	...
	for (int iTeam = 0; iTeam < MAX_CIV_TEAMS; ++iTeam)
	{
		...
		if (kTeam.isAlive() && !kTeam.isMinorCiv() && iTeam != kThisPlayer.getTeam() && iTeam != kOtherPlayer.getTeam())
		{
			if (kTeam.isHasMet(kOtherPlayer.getTeam()))
			{
				...
				{
					szString.append(NEWLINE);
					//szString.append(gDLL->getText(L"TXT_KEY_AT_WAR_WITH", kTeam.getName().GetCString())); // edead
					szString.append(gDLL->getText(L"TXT_KEY_AT_WAR_WITH", kPlayer.getCivilizationDescription())); // edead
				}
...
void CvGameTextMgr::setEventHelp(CvWStringBuffer& szBuffer, EventTypes eEvent, int iEventTriggeredId, PlayerTypes ePlayer)
{
	...
	{
		szBuffer.append(NEWLINE);
		//szBuffer.append(gDLL->getText("TXT_KEY_EVENT_GIFT_BONUS_TO_PLAYER", GC.getBonusInfo((BonusTypes)kEvent.getBonusGift()).getTextKeyWide(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getNameKey())); // edead
		szBuffer.append(gDLL->getText("TXT_KEY_EVENT_GIFT_BONUS_TO_PLAYER", GC.getBonusInfo((BonusTypes)kEvent.getBonusGift()).getTextKeyWide(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getCivilizationDescriptionKey())); // edead
	}
	...
	if (kEvent.getHappy() != 0)
	{
		if (NO_PLAYER != pTriggeredData->m_eOtherPlayer)
		{
			...
			{
				szBuffer.append(NEWLINE);
				//szBuffer.append(gDLL->getText("TXT_KEY_EVENT_HAPPY_FROM_PLAYER", kEvent.getHappy(), kEvent.getHappy(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getNameKey())); // edead
				szBuffer.append(gDLL->getText("TXT_KEY_EVENT_HAPPY_FROM_PLAYER", kEvent.getHappy(), kEvent.getHappy(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getCivilizationDescriptionKey())); // edead
			}
			else
			{
				szBuffer.append(NEWLINE);
				//szBuffer.append(gDLL->getText("TXT_KEY_EVENT_HAPPY_TO_PLAYER", -kEvent.getHappy(), -kEvent.getHappy(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getNameKey())); // edead
				szBuffer.append(gDLL->getText("TXT_KEY_EVENT_HAPPY_TO_PLAYER", -kEvent.getHappy(), -kEvent.getHappy(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getCivilizationDescriptionKey())); // edead
			}
		}
		else
		{
	...
	if (kEvent.getHealth() != 0)
	{
		if (NO_PLAYER != pTriggeredData->m_eOtherPlayer)
		{
			...
			{
				szBuffer.append(NEWLINE);
				//szBuffer.append(gDLL->getText("TXT_KEY_EVENT_HEALTH_FROM_PLAYER", kEvent.getHealth(), kEvent.getHealth(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getNameKey())); // edead
				szBuffer.append(gDLL->getText("TXT_KEY_EVENT_HEALTH_FROM_PLAYER", kEvent.getHealth(), kEvent.getHealth(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getCivilizationDescriptionKey())); // edead
			}
			else
			{
				szBuffer.append(NEWLINE);
				//szBuffer.append(gDLL->getText("TXT_KEY_EVENT_HEALTH_TO_PLAYER", -kEvent.getHealth(), -kEvent.getHealth(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getNameKey())); // edead
				szBuffer.append(gDLL->getText("TXT_KEY_EVENT_HEALTH_TO_PLAYER", -kEvent.getHealth(), -kEvent.getHealth(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getCivilizationDescriptionKey())); // edead
			}
		}
		else
		{
	...
	if (NO_PLAYER != pTriggeredData->m_eOtherPlayer)
	{
		...
		{
			szBuffer.append(NEWLINE);
			//szBuffer.append(gDLL->getText("TXT_KEY_EVENT_ATTITUDE_GOOD", kEvent.getAttitudeModifier(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getNameKey())); // edead
			szBuffer.append(gDLL->getText("TXT_KEY_EVENT_ATTITUDE_GOOD", kEvent.getAttitudeModifier(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getCivilizationDescriptionKey())); // edead
		}
		else if (kEvent.getAttitudeModifier() < 0)
		{
			szBuffer.append(NEWLINE);
			//szBuffer.append(gDLL->getText("TXT_KEY_EVENT_ATTITUDE_BAD", kEvent.getAttitudeModifier(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getNameKey())); // edead
			szBuffer.append(gDLL->getText("TXT_KEY_EVENT_ATTITUDE_BAD", kEvent.getAttitudeModifier(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getCivilizationDescriptionKey())); // edead
		}
	}
	...
	if (NO_PLAYER != pTriggeredData->m_eOtherPlayer)
	{
		...
		{
			szBuffer.append(NEWLINE);
			//szBuffer.append(gDLL->getText("TXT_KEY_EVENT_ESPIONAGE_POINTS", kEvent.getEspionagePoints(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getNameKey())); // edead
			szBuffer.append(gDLL->getText("TXT_KEY_EVENT_ESPIONAGE_POINTS", kEvent.getEspionagePoints(), GET_PLAYER(pTriggeredData->m_eOtherPlayer).getCivilizationDescriptionKey())); // edead
		}
...
void CvGameTextMgr::eventTechHelp(CvWStringBuffer& szBuffer, EventTypes eEvent, TechTypes eTech, PlayerTypes eActivePlayer, PlayerTypes eOtherPlayer)
{
	...
	if (eTech != NO_TECH)
	{
		if (100 == kEvent.getTechPercent())
		{
			...
			{
				szBuffer.append(NEWLINE);
				//szBuffer.append(gDLL->getText("TXT_KEY_EVENT_TECH_GAINED_FROM_PLAYER", GC.getTechInfo(eTech).getTextKeyWide(), GET_PLAYER(eOtherPlayer).getNameKey())); // edead
				szBuffer.append(gDLL->getText("TXT_KEY_EVENT_TECH_GAINED_FROM_PLAYER", GC.getTechInfo(eTech).getTextKeyWide(), GET_PLAYER(eOtherPlayer).getCivilizationDescriptionKey())); // edead
			}
			else
			{
				szBuffer.append(NEWLINE);
		...
		else if (0 != kEvent.getTechPercent())
		{
			...
			{
				szBuffer.append(NEWLINE);
				//szBuffer.append(gDLL->getText("TXT_KEY_EVENT_TECH_GAINED_FROM_PLAYER_PERCENT", iBeakers, GC.getTechInfo(eTech).getTextKeyWide(), GET_PLAYER(eOtherPlayer).getNameKey())); // edead
				szBuffer.append(gDLL->getText("TXT_KEY_EVENT_TECH_GAINED_FROM_PLAYER_PERCENT", iBeakers, GC.getTechInfo(eTech).getTextKeyWide(), GET_PLAYER(eOtherPlayer).getCivilizationDescriptionKey())); // edead
			}
			else
			{
				szBuffer.append(NEWLINE);
	...
	if (0 != iGold1 || 0 != iGold2)
	{
		if (iGold1 == iGold2)
		{
			if (NO_PLAYER != eOtherPlayer && kEvent.isGoldToPlayer())
			{
				...
				{
					szBuffer.append(NEWLINE);
					//szBuffer.append(gDLL->getText("TXT_KEY_EVENT_GOLD_FROM_PLAYER", iGold1, GET_PLAYER(eOtherPlayer).getNameKey())); // edead
					szBuffer.append(gDLL->getText("TXT_KEY_EVENT_GOLD_FROM_PLAYER", iGold1, GET_PLAYER(eOtherPlayer).getCivilizationDescriptionKey())); // edead
				}
				else
				{
					szBuffer.append(NEWLINE);
					//szBuffer.append(gDLL->getText("TXT_KEY_EVENT_GOLD_TO_PLAYER", -iGold1, GET_PLAYER(eOtherPlayer).getNameKey())); // edead
					szBuffer.append(gDLL->getText("TXT_KEY_EVENT_GOLD_TO_PLAYER", -iGold1, GET_PLAYER(eOtherPlayer).getCivilizationDescriptionKey())); // edead
				}
			}
			else
			{
		...
		else
		{
			if (NO_PLAYER != eOtherPlayer && kEvent.isGoldToPlayer())
			{
				...
				{
					szBuffer.append(NEWLINE);
					//szBuffer.append(gDLL->getText("TXT_KEY_EVENT_GOLD_RANGE_FROM_PLAYER", iGold1, iGold2, GET_PLAYER(eOtherPlayer).getNameKey())); // edead
					szBuffer.append(gDLL->getText("TXT_KEY_EVENT_GOLD_RANGE_FROM_PLAYER", iGold1, iGold2, GET_PLAYER(eOtherPlayer).getCivilizationDescriptionKey())); // edead
				}
				else
				{
					szBuffer.append(NEWLINE);
					//szBuffer.append(gDLL->getText("TXT_KEY_EVENT_GOLD_RANGE_TO_PLAYER", -iGold1, -iGold2, GET_PLAYER(eOtherPlayer).getNameKey())); // edead
					szBuffer.append(gDLL->getText("TXT_KEY_EVENT_GOLD_RANGE_TO_PLAYER", -iGold1, -iGold2, GET_PLAYER(eOtherPlayer).getCivilizationDescriptionKey())); // edead
				}
			}
			else
			{
...
void CvGameTextMgr::setEspionageCostHelp(CvWStringBuffer &szBuffer, EspionageMissionTypes eMission, PlayerTypes eTargetPlayer, const CvPlot* pPlot, int iExtraData, const CvUnit* pSpyUnit)
{
	...
	if (kMission.getPlayerAnarchyCounter() > 0)
	{
		if (NO_PLAYER != eTargetPlayer)
		{
			...
			szBuffer.append(gDLL->getText("TXT_KEY_ESPIONAGE_HELP_ANARCHY", GET_PLAYER(eTargetPlayer).getCivilizationDescription(0), iTurns)); // edead
			szBuffer.append(NEWLINE);
		}
	}
	...
	if (kPlayer.getEspionageMissionCost(eMission, eTargetPlayer, pPlot, iExtraData, pSpyUnit) > 0)
	{
		...
		if (NULL != pSpyUnit)
		{
			// SuperSpies: edead start
			int iSkill = 0;
			if (kMission.getDestroyUnitCostFactor() > 0 || kMission.getPlayerAnarchyCounter() > 0)
			{
				iSkill = pSpyUnit->getCollateralDamageProtection();
			}
			int iInterceptChance = (pSpyUnit->getSpyInterceptPercent(GET_PLAYER(eTargetPlayer).getTeam()) * (100 + kMission.getDifficultyMod() - iSkill) + std::max(0, kMission.getDifficultyMod() - iSkill) * 10 ) / 100;
			//int iInterceptChance = (pSpyUnit->getSpyInterceptPercent(GET_PLAYER(eTargetPlayer).getTeam()) * (100 + kMission.getDifficultyMod())) / 100;
			// SuperSpies: edead end
...
void CvGameTextMgr::getTerrainDataForWB(std::vector<CvWBData>& mapTerrainData, std::vector<CvWBData>& mapFeatureData, std::vector<CvWBData>& mapPlotData, std::vector<CvWBData>& mapRouteData)
{
	...
	{
		// edead: start comment - needed to display bGraphicalOnly Terrain in WB
