...
/************************************************************************************************/
	...
	if (!bDanger && (!hasActiveWorldWonder()) && (kPlayer.getNumCities() <= (getOwnerINLINE() == ABBASIDS ? 6 : 4)))
	{
...
/************************************************************************************************/
    ...
    if (iUnitCostPercentage < (iMaxUnitSpending + 10))
	{
		...
		if (NULL != pWaterArea && (bAssault))
		{
			...
			// edead: start comment - speed / unused features
	...
	int iNeededSpies = iNumCitiesInArea / 4; //Rhye
...
UnitTypes CvCityAI::AI_bestUnit(bool bAsync, AdvisorTypes eIgnoreAdvisor, UnitAITypes* peBestUnitAI)
{
	...
	aiUnitAIVal[UNITAI_ATTACK_SEA] *= 4; // edead (5)
	aiUnitAIVal[UNITAI_RESERVE_SEA] *= 3; // edead (4)
	aiUnitAIVal[UNITAI_ESCORT_SEA] *= 16; // edead (20)
	aiUnitAIVal[UNITAI_EXPLORE_SEA] *= 14; // edead (18)
	aiUnitAIVal[UNITAI_ASSAULT_SEA] *= 12; // edead (14)
	aiUnitAIVal[UNITAI_SETTLER_SEA] *= 16; // edead (16)
	...
	//edead: start Rhye's code modified for MEM
	//Rhye - start switch (unit AI types)

	if (GET_PLAYER((PlayerTypes)getOwnerINLINE()).verifySettlersHalt(200)) {} // edead (150)
	else if (GET_PLAYER((PlayerTypes)getOwnerINLINE()).verifySettlersHalt(100)) // edead (40)
	...
	if (GET_PLAYER(getOwnerINLINE()).getNumCities() >= 25)
		...
		aiUnitAIVal[UNITAI_SETTLE] /= 2;
	else if (GET_PLAYER(getOwnerINLINE()).getCurrentEra() >= 3 && GET_PLAYER(getOwnerINLINE()).getNumCities() >= 10)
		{
			aiUnitAIVal[UNITAI_SETTLE] *= 2;
			aiUnitAIVal[UNITAI_SETTLE] /= 3;
		}
	...
	case BYZANTIUM:
		...
		aiUnitAIVal[UNITAI_SPY] *= 5;
		aiUnitAIVal[UNITAI_SPY] /= 4;
		break;
	case MAKURIA:
		aiUnitAIVal[UNITAI_EXPLORE] /= 2;
	...
	case YEMEN:
		...
		aiUnitAIVal[UNITAI_RESERVE_SEA] /= 3;
		aiUnitAIVal[UNITAI_ASSAULT_SEA] *= 5;
		aiUnitAIVal[UNITAI_ASSAULT_SEA] /= 4;
		break;
	case FATIMIDS:
		aiUnitAIVal[UNITAI_MISSIONARY] *= 3;
		aiUnitAIVal[UNITAI_MISSIONARY] /= 2;
	...
	case SINDH:
		break;
	case TURKS:
		aiUnitAIVal[UNITAI_PILLAGE] *= 5;
		aiUnitAIVal[UNITAI_PILLAGE] /= 4;
		break;
	case KHWAREZM:
		aiUnitAIVal[UNITAI_CITY_DEFENSE] *= 4;
							...
							if (iValue > 0)
							{
								...
								if (isWorldWonderClass((BuildingClassTypes)iI))
								{
									if (iProductionRank <= std::min(3, ((GET_PLAYER(getOwnerINLINE()).getNumCities() + 2) / 3)))
									{
										...
										//Rhye/edead - start switch (wonders)
										
										iTempValue = 20;
										
										if (iI == HOUSE_OF_WISDOM && getX_INLINE() == 35 && getY_INLINE() == 41) {
											iTempValue *= 2;
										}
										if (iI == SPIRAL_MINARET && getX_INLINE() == 34 && getY_INLINE() == 45) {
											iTempValue *= 3;
											iTempValue /= 2;
										}
										if (iI == AL_AZHAR && getX_INLINE() == 9 && getY_INLINE() == 31) {
											iTempValue *= 3;
											iTempValue /= 2;										
										}
										if (iI == IMAM_REZA && (getX_INLINE() == 61 || getX_INLINE() == 62) && getY_INLINE() == 51) {
											iTempValue *= 2;
										}
										...
										{
										case BYZANTIUM:
											if (iI == BAGRATI_CATHEDRAL) {
												iTempValue *= 2;
												iTempValue /= 3;
											}
											break;
										case ABBASIDS:
											if (iI == HOUSE_OF_WISDOM || iI == INVENTORS_WORKSHOP || iI == SPIRAL_MINARET) {
												iTempValue *= 3;
											}
											if (iI == KRAK_DES_CHEVALIERS || iI == MINARET_OF_JAM || iI == AL_AZHAR || iI == BAGRATI_CATHEDRAL || iI == QUTB_MINAR || iI == SHALIMAR_GARDENS || iI == TAJ_MAHAL || iI == RED_FORT || iI == IBN_BATTUTA || iI == MEVLANAS_TOMB) {
												iTempValue /= 2;
											}
											if (iI == GUTLUK_TEMIR || iI == IMAM_REZA) {
												iTempValue *= 2;
												iTempValue /= 3;
											}
											break;
										case FATIMIDS:
											...
											if (iI == KRAK_DES_CHEVALIERS || iI == TOMB_OF_KHALID || iI == MINARET_OF_JAM || iI == BAGRATI_CATHEDRAL) {
												iTempValue /= 2;
											}
											if (iI == IMAM_REZA) {
												iTempValue /= 2;
										...
										case MALWA:
										case GUJARAT:
											if (iI == TOWER_OF_VICTORY) {
												iTempValue *= 2;
											}
											if (iI == MEVLANAS_TOMB || iI == IBN_BATTUTA || iI == GUTLUK_TEMIR) {
												iTempValue /= 3;
											}
											break;
										case SAMANIDS:
										...
										case ARMENIA:
										case GEORGIA:
											if (iI == BAGRATI_CATHEDRAL || iI == NAREKAVANK) {
												iTempValue *= 3;
												iTempValue /= 2;
											}
											break;
										case KHWAREZM:
											if (iI == GUTLUK_TEMIR || iI == SHAHNAMEH) {
												iTempValue *= 2;
											}
											break;
										case TURKS:
											if (iI == KIZIL_KULE || iI == MEVLANAS_TOMB) {
												iTempValue *= 2;
											}
											if (iI == KRAK_DES_CHEVALIERS || iI == BAGRATI_CATHEDRAL || iI == NAREKAVANK) {
												iTempValue /= 3;
											}
											if (iI == IMAM_REZA) {
												iTempValue /= 2;
										...
										case OUTREMER:
											...
											if (iI == IMAM_REZA || iI == MEVLANAS_TOMB || iI == SPIRAL_MINARET || iI == AL_AZHAR || iI == TOMB_OF_KHALID || iI == MINARET_OF_JAM) {
												iTempValue /= 3;
											}
											break;
										case GHORIDS:
											if (iI == MINARET_OF_JAM) {
												iTempValue *= 3;
											}
											if (iI == QUTB_MINAR || iI == IBN_BATTUTA) {
												iTempValue *= 2;
											}
											if (iI == IMAM_REZA) {
												iTempValue /= 2;
										...
										case OMAN:
											if (iI == KIZIL_KULE || iI == MEVLANAS_TOMB || iI == QUTB_MINAR || iI == MINARET_OF_JAM) {
												iTempValue /= 2;
											}
										case OTTOMANS:
											if (iI == TOPKAPI_PALACE || iI == BLUE_MOSQUE || iI == GRAND_BAZAAR) {
												iTempValue *= 2;
											}
											if (iI == KRAK_DES_CHEVALIERS || iI == BAGRATI_CATHEDRAL || iI == NAREKAVANK) {
												iTempValue /= 3;
											}
											if (iI == SHALIMAR_GARDENS || iI == TAJ_MAHAL || iI == RED_FORT || iI == IMAM_REZA || iI == SHAH_MOSQUE) {
												iTempValue /= 2;
											}
											break;
										case TIMURIDS:
											if (iI == SOLTANIYEH || iI == IMAM_REZA) {
												iTempValue *= 3;
												iTempValue /= 2;
											}
										case MAMLUKS:
											if (iI == SHAH_MOSQUE || iI == SHALIMAR_GARDENS || iI == TAJ_MAHAL || iI == RED_FORT || iI == IMAM_REZA) {
												iTempValue /= 2;
											}
										case SAFAVIDS:
											if (iI == SHAH_MOSQUE || iI == GRAND_BAZAAR || iI == IMAM_REZA || iI == ESFAHAN_BRIDGE) {
												iTempValue *= 2;
											}
											if (iI == BAGRATI_CATHEDRAL) {
												iTempValue /= 2;
											}
											if (iI == SHALIMAR_GARDENS || iI == TAJ_MAHAL || iI == RED_FORT) {
												iTempValue *= 2;
												iTempValue /= 3;
											}
											break;
										case MUGHALS:
											...
											if (iI == SHAH_MOSQUE || iI == IMAM_REZA) {
												iTempValue /= 2;
											}
											if (iI == TOPKAPI_PALACE || iI == GRAND_BAZAAR || iI == ESFAHAN_BRIDGE) {
												iTempValue *= 2;
												iTempValue /= 3;
											}											
		...
		if ((iFocusFlags == 0) || (iValue > 0) || (iPass == 0))
		{
		    ...
		    if ((iFocusFlags & BUILDINGFOCUS_WORLDWONDER) || (iPass > 0))
		    {
		        if (isWorldWonderClass(eBuildingClass))
		        {
					...
					//edead: historical cities to supplant aiYieldRank
					if (getX_INLINE() == 35 && getY_INLINE() == 41 && GC.getGameINLINE().getGameTurnYear() <= 1000) { // Baghdad
						iValue += 2;
					}
					if (eBuilding == SPIRAL_MINARET && getX_INLINE() == 34 && getY_INLINE() == 45) { // Samarra
						iValue++;
					}
					if (getX_INLINE() == 6 && getY_INLINE() == 63) { // Constantinople
						iValue++;
					}
					//edead: end
					
					//edead: start favorite wonder code by Kael
			...
			if (((iFocusFlags & BUILDINGFOCUS_HAPPY) || (iPass > 0)) && !isNoUnhappiness())
			{
				...
				// edead: start - count stability bonus of temples
				if (kBuilding.getHappiness() == 1 && kBuilding.getReligionType() == eStateReligion && NO_RELIGION != eStateReligion)
				{
					iValue += 20;
				}
				// edead: end
			...
			if (iPass > 0)
			{
				...
				for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
				{
					if (kBuilding.getReligionChange(iI) > 0)
					{
						...
						// edead: start - Wonders that spread state religion should count as well
						if (eStateReligion == (ReligionTypes)iI)
						{
							iValue += (kBuilding.getReligionChange(iI) * 10);
						}
						// edead: end
					}
				}
...
{
	// edead: start persecutor code, idea based on 3Miro's code from RFCE
	UnitTypes ePersecutor = NO_UNIT;
	int iI;
	// determine the persecutor's unit type
	for (iI = 0; iI < GC.getNumUnitInfos(); iI++)
	{
		if (GC.getUnitInfo((UnitTypes)iI).isPurgeReligion())
		{
			ePersecutor = (UnitTypes)iI;
			break;
		}
	}
	if (!isBarbarian() && !GET_PLAYER(getOwnerINLINE()).isMinorCiv() && ePersecutor != NO_UNIT)
	{
		if (((eUnitAI == UNITAI_SPY) || (eUnitAI == UNITAI_MISSIONARY)) && (canTrain(ePersecutor)) && (NO_RELIGION != GET_PLAYER(getOwnerINLINE()).getStateReligion()))
		{
			if ((GET_PLAYER(getOwnerINLINE()).getUnitClassCount((UnitClassTypes)GC.getUnitInfo(ePersecutor).getUnitClassType()) < 1) && (GC.getGameINLINE().getSorenRandNum(100," Shall we build a Prosecutor") < GET_PLAYER(getOwnerINLINE()).AI_getBuildPersecutorProb()) ) // changed to array - edead
			{
				// check whether the persecution is actually needed
				bool bFound = false;
				for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
				{
					if ((GET_PLAYER(getOwnerINLINE()).getStateReligion() != (ReligionTypes)iI) && (GET_PLAYER(getOwnerINLINE()).getHasReligionCount((ReligionTypes)iI) > 0))
					{
						pushOrder(ORDER_TRAIN, ePersecutor, eUnitAI, false, false, false);
						return true;
					}
				}
			}
