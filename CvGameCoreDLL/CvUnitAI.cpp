...
/*************************************************************************************************/
	...
	// edead: start Persecutor code (limited python callback for now)
	if (GC.getUnitInfo(getUnitType()).isPurgeReligion())
	{
		CyUnit* pyUnit = new CyUnit(this);
		CyArgsList argsList;
		argsList.add(gDLL->getPythonIFace()->makePythonObject(pyUnit));	// pass in unit class
...
/************************************************************************************************/
...

	{
		...
		if (area()->getAreaAIType(getTeam()) != AREAAI_OFFENSIVE)
		{
			...
			//Rhye - start + changed by edead 11.12.2009
			...
			if (area()->getCitiesPerPlayer((PlayerTypes)INDEPENDENT3) > 0)
			{
				if (AI_targetMinorCity(INDEPENDENT3))
				{
					return;
				}
			}
			if (area()->getCitiesPerPlayer((PlayerTypes)INDEPENDENT4) > 0)
			{
				if (AI_targetMinorCity(INDEPENDENT4))
				{
					return;
				}
			}
...
void CvUnitAI::AI_attackCityMove()
{
	...
	bool bHuntIndependents3 = false;
	bool bHuntIndependents4 = false;
	if (area()->getCitiesPerPlayer((PlayerTypes)INDEPENDENT) > 0)
	{
		if ((area()->getAreaAIType(getTeam()) != AREAAI_OFFENSIVE) && (area()->getAreaAIType(getTeam()) != AREAAI_DEFENSIVE))
		{
...
//			((area()->getAreaAIType(getTeam()) == AREAAI_DEFENSIVE) && (getGroup()->getNumUnits() >= AI_stackOfDoomExtra())))
	if (bReadyToAttack)
	{
		...
		else if (bHuntIndependents3 && AI_targetMinorCity(INDEPENDENT3))
		{
			return;
		}
		else if (bHuntIndependents4 && AI_targetMinorCity(INDEPENDENT4))
		{
			return;
		}
		//Rhye - end
...
void CvUnitAI::AI_counterMove()
{
	...
	//bool bDanger = (GET_PLAYER(getOwnerINLINE()).AI_getPlotDanger(plot(), 3) > 0);
	bool bDanger = (GET_PLAYER(getOwnerINLINE()).AI_getAnyPlotDanger(plot(), 3)); // BBAI
...
void CvUnitAI::AI_generalMove()
{
	...
	if (bOffenseWar) // edead
	{
		aeUnitAITypes.clear();
		aeUnitAITypes.push_back(UNITAI_ATTACK_CITY);
		aeUnitAITypes.push_back(UNITAI_ATTACK); // edead
		if (AI_lead(aeUnitAITypes))
		{
			return;
		}
...
//	}
	...
	for (int iI = 0; iI < GET_PLAYER(getOwnerINLINE()).AI_getNumCitySites(); iI++)
	{
		...
		// edead: start from Rhye's
		if (pCitySitePlot->getFoundModifier(getOwnerINLINE()) < 150)
			return false;
		// edead: end
...
bool CvUnitAI::AI_found_map(int modifier)
{
	...
	for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if (pLoopPlot != NULL)
		{
			if (pLoopPlot->getFoundModifier(getOwnerINLINE()) >= modifier) // edead: changed == to >= to allow gradation
			{
				if (AI_plotValid(pLoopPlot))
				{
					if (canFound(pLoopPlot))
		...
		for (iDY = -(iSearchRange); iDY <= iSearchRange; iDY++)
		{
			...
			if (pLoopPlot != NULL)
			{
				if (AI_plotValid(pLoopPlot) && (pLoopPlot != plot() || GET_PLAYER(getOwnerINLINE()).AI_getPlotDanger(pLoopPlot, 1) <= pLoopPlot->plotCount(PUF_canDefend, -1, -1, getOwnerINLINE())))
				{
					if (canFound(pLoopPlot))
					{
						if (GET_PLAYER(getOwnerINLINE()).isMinorCiv() || GC.getMapINLINE().plotINLINE(getX_INLINE() + iDX, getY_INLINE() + iDY)->getFoundModifier(getOwnerINLINE()) >= 60) // edead: from Rhye's
						{ //Rhye
...
int CvUnitAI::AI_getEspionageTargetValue(CvPlot* pPlot, int iMaxPath)
{
	...
	if (pPlot->getTeam() < NUM_MAJOR_PLAYERS && pPlot->isOwned() && pPlot->getTeam() != getTeam() && !GET_TEAM(getTeam()).isVassal(pPlot->getTeam())) // edead
	{
		if (AI_plotValid(pPlot))
		{
			if (pPlot->isCity())
...
bool CvUnitAI::AI_cityOffenseSpy(int iMaxPath)
{
	...
	for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; ++iPlayer)
	{
		...
		if (!kLoopPlayer.isMinorCiv() && !kLoopPlayer.isBarbarian() && kLoopPlayer.isAlive() && kLoopPlayer.getTeam() != getTeam() && !GET_TEAM(getTeam()).isVassal(kLoopPlayer.getTeam())) // edead: no AI spying vs. minor civs
		{
			int iLoop;
			for (CvCity* pLoopCity = kLoopPlayer.firstCity(&iLoop); NULL != pLoopCity; pLoopCity = kLoopPlayer.nextCity(&iLoop))
			{
