...
CvCity::CvCity()
{
	...
	m_aiBonusCommerceRateModifier = new int[NUM_COMMERCE_TYPES]; //BCM:Added 26.9.09
...
CvCity::~CvCity()
{
	...
	SAFE_DELETE_ARRAY(m_aiBonusCommerceRateModifier); //BCM:Added 26.9.09
...
/************************************************************************************************/
	...
	// edead: start (iFreePopulation from EraInfos is now used for late start civs)
	changePopulation(GC.getDefineINT("INITIAL_CITY_POPULATION") + GC.getEraInfo(GET_PLAYER(getOwnerINLINE()).getCurrentEra()).getFreePopulation());
...
/************************************************************************************************/
	...
	for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
	{
		...
		m_aiBonusCommerceRateModifier[iI] = 0; //BCM:Added 26.9.09
	}
...
bool CvCity::canTrain(UnitTypes eUnit, bool bContinue, bool bTestVisible, bool bIgnoreCost, bool bIgnoreUpgrades) const
{
	...
	// edead: inquisitors only in cities with state religion, based on 3Miro's
	if (GC.getUnitInfo(eUnit).isPurgeReligion())
	{
...
int CvCity::getConscriptPercentAnger(int iExtra) const
{
	...
	// edead: start Ottoman UP - no conscript anger
	if (getOwnerINLINE() == OTTOMANS)
	{
		return 0;
	}
	// edead: end
...
int CvCity::unhappyLevel(int iExtra) const
{
	...
	if (!isNoUnhappiness())
	{
		...
		iUnhappiness -= std::min(0, getCivicHappiness()); // edead
	}
...
int CvCity::happyLevel() const
{
	...
	iHappiness += std::max(0, getCivicHappiness()); // edead
...
int CvCity::calculateNumCitiesMaintenanceTimes100() const
{
	...
	// edead: higher maintenance - from Rhye
	iNumCitiesPercent *= (getPopulation() + 14);
	iNumCitiesPercent /= 15;
	// edead: end
...
int CvCity::calculateColonyMaintenanceTimes100() const
{
	...
	// edead: start no colony maint. for indeps - simplified from Rhye's
	if (getOwnerINLINE() >= NUM_MAJOR_PLAYERS) return 0;
	// edead: end
...
int CvCity::getReligionBadHappiness() const
{
	...
	return (getOwnerINLINE() == FATIMIDS ? (m_iReligionBadHappiness / 2) : m_iReligionBadHappiness); // edead - UP: Tolerance
}
...
void CvCity::updateCultureLevel(bool bUpdatePlotGroups)
{
	...
	// edead: limit the borders to 2 for Independents and Barbarians
...
// edead: start more gradual great merchant / caravan income
...
int CvCity::getProductionToCommerceModifier(CommerceTypes eIndex) const										 
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < NUM_COMMERCE_TYPES, "eIndex expected to be < NUM_COMMERCE_TYPES");
	// edead: start Sindh UP: Craftsmanship
	if (getOwnerINLINE() == SINDH) 
	{
		...
		if (!(kBuilding.isCommerceChangeOriginalOwner(eIndex)) || (getBuildingOriginalOwner(eBuilding) == getOwnerINLINE()))
		{
			...
			if (getNumActiveBuilding(eBuilding) > 0)
			{
				...
				if (GC.getBuildingInfo(eBuilding).getGlobalReligionCommerce() != NO_RELIGION)
				{
					...
					iCommerce += std::min(GC.getDefineINT("MAX_COM_SHRINE"), (GC.getReligionInfo((ReligionTypes)(GC.getBuildingInfo(eBuilding).getGlobalReligionCommerce())).getGlobalReligionCommerce(eIndex) * GC.getGameINLINE().countReligionLevels((ReligionTypes)(GC.getBuildingInfo(eBuilding).getGlobalReligionCommerce()))));
					//Rhye - end
				}
...
int CvCity::getRevoltTestProbability() const
{
	...
	// edead - start code adapted from RFC
	...
	if (getOwnerINLINE() == FATIMIDS) // Fatimid UP
	{
		result /= 2;
	}
	if (!isBarbarian() && !GET_PLAYER(getOwnerINLINE()).isMinorCiv())
	...
	return (result);
	// edead - end code adapted from RFC
}
...
int CvCity::getGreatPeopleUnitRate(UnitTypes eIndex) const																 
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < GC.getNumUnitInfos(), "eIndex expected to be < GC.getNumUnitInfos()");
	if (isBarbarian() || GET_PLAYER(getOwnerINLINE()).isMinorCiv()) return 0; // edead: no GPs for Independents and Barbarians
...
int CvCity::getGreatPeopleUnitProgress(UnitTypes eIndex) const														 
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < GC.getNumUnitInfos(), "eIndex expected to be < GC.getNumUnitInfos()");
	if (getOwnerINLINE() >= NUM_MAJOR_PLAYERS) return 0; // edead: no GPs for Independents and Barbarians
		...
		if (iChangeNumRealBuilding > 0)
		{
			if (bFirst)
			{
				...
				if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
				{
					...
					if (isWorldWonderClass((BuildingClassTypes)(GC.getBuildingInfo(eIndex).getBuildingClassType())))
					{
						//szBuffer = gDLL->getText("TXT_KEY_MISC_COMPLETES_WONDER", GET_PLAYER(getOwnerINLINE()).getNameKey(), GC.getBuildingInfo(eIndex).getTextKeyWide()); // edead
						szBuffer = gDLL->getText("TXT_KEY_MISC_COMPLETES_WONDER", GET_PLAYER(getOwnerINLINE()).getCivilizationDescription(), GC.getBuildingInfo(eIndex).getTextKeyWide()); // edead
						...
						for (iI = 0; iI < MAX_PLAYERS; iI++)
						{
							...
							if ((GET_PLAYER((PlayerTypes)iI).isAlive()) && (GC.getGameINLINE().getGameTurn() >= getTurnForYear(GET_PLAYER((PlayerTypes)iI).getStartYear()))) // edead
							{
								if (isRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false))
								{
									//szBuffer = gDLL->getText("TXT_KEY_MISC_WONDER_COMPLETED", GET_PLAYER(getOwnerINLINE()).getNameKey(), GC.getBuildingInfo(eIndex).getTextKeyWide()); // edead
									szBuffer = gDLL->getText("TXT_KEY_MISC_WONDER_COMPLETED", GET_PLAYER(getOwnerINLINE()).getCivilizationDescription(), GC.getBuildingInfo(eIndex).getTextKeyWide()); // edead
...
//Speed: End Modify
	...
	{
		changeCultureTimes100(getOwnerINLINE(), getCommerceRateTimes100(COMMERCE_CULTURE) * (GET_PLAYER(getOwnerINLINE()).getCulturePercent()) / 100, false, true);
	}
	
	// edead: end RFC balance
}
...
void CvCity::read(FDataStreamBase* pStream)
{
	...
	pStream->Read(NUM_COMMERCE_TYPES, m_aiBonusCommerceRateModifier); //BCM:Added 26.9.09
...
void CvCity::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(NUM_COMMERCE_TYPES, m_aiBonusCommerceRateModifier); //BCM:Added 26.9.09	
...
PlayerTypes CvCity::getLiberationPlayer(bool bConquest) const
{
	...
	for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; ++iPlayer)
	{
		...
		if (kLoopPlayer.isAlive() && !kLoopPlayer.isBarbarian() && !kLoopPlayer.isMinorCiv()) // edead
		{
