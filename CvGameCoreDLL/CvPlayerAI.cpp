...
CvPlayerAI::CvPlayerAI()
{
	...
	m_aiUnitAIModifier = new int[NUM_UNITAI_TYPES]; // edead
...
CvPlayerAI::~CvPlayerAI()
{
	...
	// Sanguo Mod Performance, end
	SAFE_DELETE_ARRAY(m_aiUnitAIModifier); // edead
}
...
void CvPlayerAI::AI_reset(bool bConstructor)
{
	...
	// edead: start
	m_iCompactEmpireModifier = 40; // BTS default
	m_iMassacreProb = 0;
	m_iBuildPersecutorProb = 0;
	m_iWarDistanceModifier = 1; // BTS default
	m_iWarCoastalModifier = 1; // BTS default
	m_iMaxTakenTiles = 7; // BTS default
	// edead: end
...
/************************************************************************************************/
									if ((pLoopUnit->getDomainType() != DOMAIN_LAND) || pLoopUnit->plot()->plotCount(PUF_isMilitaryHappiness, -1, -1, getID()) > 1)
									{
										...
										if ((calculateUnitCost() > 0) && (!AI_getAnyPlotDanger( pLoopUnit->plot(), 2, false))) // edead: bugfix!
/************************************************************************************************/
/* BETTER_BTS_AI_MOD                       END                                                  */
/************************************************************************************************/	
										{
...
void CvPlayerAI::AI_doCentralizedProduction()
{
	...
	if( isBarbarian() || isMinorCiv() ) // edead
	{
		return;
	}
...
void CvPlayerAI::AI_conquerCity(CvCity* pCity)
{
	CvCity* pNearestCity;
	bool bRaze = false;
	bool bMassacre = false; // edead: Massacre the Disbelievers
	int iRazeValue;
	int iI;
		...
		if (GC.getGameINLINE().getElapsedGameTurns() > 20)
		{
			...
			if (getNumCities() > 3 || isBarbarian()) //Rhye
			{
				if (!(pCity->isHolyCity()) && !(pCity->hasActiveWorldWonder()))
				{
                    ...
                    {
                    	//iRazeValue += 60; //edead
						if (!isBarbarian()) iRazeValue += 60; // edead
					}
					...
					if (iRazeValue > 0)
					{
						...
						// edead: start: less barb city razing before Mongol invasions
						if (((GC.getGameINLINE().getGameTurnYear() < 1215) || (GC.getGameINLINE().getGameTurnYear() > 1350)) && (isBarbarian()))
						{
							iRazeValue /= 4;
						}
						// edead: end
	...
	// edead: start Massacre the Disbelievers
	if (GC.getGameINLINE().getSorenRandNum(100, "AI Massacre Disbelievers") < AI_getMassacreProb())
	{
		bMassacre = true;
	}
	// edead: end
...
*/
		...
		CvEventReporter::getInstance().cityAcquiredAndKept(getID(), pCity, bMassacre); // edead: Massacre the Disbelievers
/************************************************************************************************/
...
int CvPlayerAI::AI_foundValue(int iX, int iY, int iMinRivalRange, bool bStartingLoc) const
{
	...
	if (iTakenTiles > (NUM_CITY_PLOTS *2/3) && iResourceValue < 250) // Rhye
	...
	// edead - start
	if (iTakenTiles > AI_getMaxTakenTiles())
	{
		return 0;
	}
	// edead - end
	...
	if (pNearestCity != NULL)
	{
		...
		else
		{
			...
			else if (getCapitalCity() != NULL)
			{
				...
				// edead: compact empire modifier from Rhye
				//iValue *= 100 + (((bAdvancedStart ? 80 : 50) * std::max(0, (iMaxDistanceFromCapital - iDistance))) / iMaxDistanceFromCapital);
				iValue *= 100 + (((bAdvancedStart ? 80 : AI_getCompactEmpireModifier()) * std::max(0, (iMaxDistanceFromCapital - iDistance))) / iMaxDistanceFromCapital);
				// edead: end
	...
	// edead: start settlers maps code based on Rhye's
	// randomization moved to DataLoader.py
	iValue *= pPlot->getFoundModifier(getID());
	iValue /= 100;
	
	if (pPlot->getFoundModifier(getID()) <= 3)
	{
		iValue = 0;
	}
	else if (pPlot->getFoundModifier(getID()) <= 60)
	{
		iValue /= 2;
	}
	// edead: end
...
int CvPlayerAI::AI_targetCityValue(CvCity* pCity, bool bRandomize, bool bIgnoreAttackers) const
{
	...
	if (pCity->getDefenseDamage() > 0)
	{
		...
		iValue += ((pCity->getDefenseDamage() / 50) + 1); // edead
	}

	if (pCity->isCoastal(GC.getMIN_WATER_SIZE_FOR_OCEAN()))
	{
		//iValue++;
		iValue += AI_getWarCoastalModifier(); // edead
	}
	...
	// edead: start based on Rhye's
	if (pCity->plot()->getFoundModifier(getID()) <= 3)
		iValue = iValue / 3 - 5;
	else if (pCity->plot()->getFoundModifier(getID()) <= 20)
		iValue -= 3;
	else if (pCity->plot()->getFoundModifier(getID()) >= 500) //500-700
		iValue = 2 + iValue*3;
	else if (pCity->plot()->getFoundModifier(getID()) >= 300) //300-400
		iValue = 2 + iValue*2;
	else if (pCity->plot()->getFoundModifier(getID()) >= 150) //150-200
		iValue += 3;
	else if (pCity->plot()->getFoundModifier(getID()) >= 40) //40-60-90
		iValue += 1;
	// edead: end
	...
	if (pNearestCity != NULL)
	{
		// edead: start from Rhye - "some civs will extend wars further"
		//iValue += std::max(1, ((GC.getMapINLINE().maxStepDistance() * 2) - GC.getMapINLINE().calculatePathDistance(pNearestCity->plot(), pCity->plot())));
		iValue += std::max(1, ((GC.getMapINLINE().maxStepDistance() * 2) - GC.getMapINLINE().calculatePathDistance(pNearestCity->plot(), pCity->plot())) / AI_getWarDistanceModifier());
		// edead: end
	}
	...
	// edead: start Franco-Mongol alliance
	if (isBarbarian() && (GC.getGameINLINE().getGameTurnYear() > 1220) && (GC.getGameINLINE().getGameTurnYear() < 1300))
	{
		if ((pCity->getOwner() == OUTREMER) || (pCity->getOwner() == ANTIOCH))
		{
			iValue /= 3;
		}
	}
...
/************************************************************************************************/
													...
													if (getTotalPopulation() > 5)
													{
														if (isWorldWonderClass((BuildingClassTypes)iJ))
														{
															if (!(GC.getGameINLINE().isBuildingClassMaxedOut((BuildingClassTypes)iJ)))
															{
																...
																//edead: start favorite wonder by Kael
								...
								//Rhye/edead - start switch
								if (iI == TECH_CASTE_SYSTEM) {
									iValue /= 4;
								}
								if (iI == TECH_CROP_ROTATION) {
									iValue *= 2;
									iValue /= 3;
								}
								if (iI == TECH_COMPASS || iI == TECH_CARTOGRAPHY) {
									iValue *= 4;
									iValue /= 5;
								}
								...
								{
								case BYZANTIUM:
									if (iI == TECH_RELIGIOUS_PHILOSOPHY || iI == TECH_ENGINEERING || iI == TECH_CIVIL_SERVICE || iI == TECH_ASYMMETRIC_WARFARE) {
										iValue *= 4;
										iValue /= 3;
									}
									break;
								case MAKURIA:
									if (iI == TECH_MACHINERY || iI == TECH_AGRICULTURAL_SCIENCE) {
										iValue *= 4;
										iValue /= 3;
									}
									if (iI == TECH_ASTROLABE || iI == TECH_STERN_RUDDER || iI == TECH_COMPASS || iI == TECH_NAVIGATION) {
										iValue /= 2;
									}
									break;
								case ABBASIDS:
									if (iI == TECH_EDUCATION || iI == TECH_ALGEBRA || iI == TECH_ALCHEMY || iI == TECH_MARKET_ECONOMY || iI == TECH_MECHANICAL_ENGINEERING || iI == TECH_MEDICINE ) {
										iValue *= 4;
										iValue /= 3;
									}
									if (iI == TECH_PATRONAGE || iI == TECH_RELIGIOUS_UNITY) {
										iValue /= 2;
									}
									break;
								case CHAUHAN:
								case MALWA:
									if (iI == TECH_CASTLE_BUILDING || iI == TECH_PATRONAGE) {
										iValue *= 4;
										iValue /= 3;
									}
									if (iI == TECH_MUSIC_THEORY || iI == TECH_CHEQUE_SYSTEM) {
										iValue *= 2;
										iValue /= 3;
									}
									if (iI == TECH_ASTROLABE || iI == TECH_STERN_RUDDER || iI == TECH_COMPASS || iI == TECH_NAVIGATION || iI == TECH_CROP_ROTATION) {
										iValue /= 2;
									}
									break;
								case SAMANIDS:
									if (iI == TECH_LITERATURE || iI == TECH_MOSAIC_ART) {
										iValue *= 5;
										iValue /= 4;
									}
									if (iI == TECH_ASTROLABE || iI == TECH_STERN_RUDDER || iI == TECH_COMPASS || iI == TECH_NAVIGATION) {
									...
									break;
								case ARMENIA:
									if (iI == TECH_MOSAIC_ART || iI == TECH_RELIGIOUS_UNITY || iI == TECH_LEVY_ARMIES) {
										iValue *= 5;
										iValue /= 4;
									}
									if (iI == TECH_ASTROLABE || iI == TECH_STERN_RUDDER || iI == TECH_COMPASS || iI == TECH_NAVIGATION) {
									...
									break;
								case YEMEN:
									if (iI == TECH_LITERATURE || iI == TECH_MARKET_ECONOMY || iI == TECH_COMPANIES) {
										iValue *= 4;
										iValue /= 3;
									}
									break;
								case BUYIDS:
									if (iI == TECH_CARTOGRAPHY) {
										iValue *= 2;
										iValue /= 3;
									}
									if (iI == TECH_ASTROLABE || iI == TECH_STERN_RUDDER || iI == TECH_COMPASS || iI == TECH_NAVIGATION) {
									...
									break;
								case GUJARAT:
									if (iI == TECH_WATER_MANAGEMENT || iI == TECH_CASTLE_BUILDING) {
										iValue *= 4;
										iValue /= 3;
									}
									if (iI == TECH_MUSIC_THEORY || iI == TECH_CHEQUE_SYSTEM || iI == TECH_CROP_ROTATION) {
										iValue *= 2;
										iValue /= 3;
									}
									break;
								case GHAZNAVIDS:
									if (iI == TECH_LONG_DISTANCE_TRADE || iI == TECH_CHEQUE_SYSTEM) {
										iValue *= 4;
										iValue /= 3;
									}
									if (iI == TECH_ASTROLABE || iI == TECH_STERN_RUDDER || iI == TECH_COMPASS || iI == TECH_NAVIGATION) {
									...
									break;
								case FATIMIDS:
									if (iI == TECH_RELIGIOUS_PHILOSOPHY || iI == TECH_EDUCATION) {
										iValue *= 3;
										iValue /= 2;
									}
									break;
								case GEORGIA:
									...
									break;
								case SELJUKS:
									if (iI == TECH_EDUCATION) {
										iValue *= 4;
										iValue /= 3;
									}
									if (iI == TECH_CHEQUE_SYSTEM || iI == TECH_MARKET_ECONOMY || iI == TECH_PATRONAGE || iI == TECH_RELIGIOUS_UNITY) {
										iValue /= 2;
									}
									if (iI == TECH_ASTROLABE || iI == TECH_STERN_RUDDER || iI == TECH_COMPASS || iI == TECH_NAVIGATION) {
										iValue /= 2;
									...
									break;
								case SINDH:
									if (iI == TECH_PATRONAGE || iI == TECH_RELIGIOUS_UNITY || iI == TECH_CROP_ROTATION) {
										iValue /= 3;
										iValue *= 2;
									}
									break;
								case TURKS:
									if (iI == TECH_GUNPOWDER) {
										iValue *= 4;
										iValue /= 3;
									}
									if (iI == TECH_STERN_RUDDER || iI == TECH_MUSIC_THEORY) {
										iValue *= 3;
										iValue /= 2;
									}
									break;
								...
								case ANTIOCH:
								case OUTREMER:
									if (iI == TECH_GUILDS || iI == TECH_CIVIL_SERVICE || iI == TECH_ASYMMETRIC_WARFARE) {
										iValue *= 5;
										iValue /= 4;
									}
									if (iI == TECH_WELFARE_STATE || iI == TECH_MECHANICAL_ENGINEERING) {
										iValue /= 2;
									}
									break;
								case ZENGIDS:
								...
								case GHORIDS:
									...
									break;
								case OMAN:
									if (iI == TECH_OPTICS || iI == TECH_NAVIGATION) {
										iValue *= 5;
										iValue /= 4;
									}
									if (iI == TECH_PATRONAGE || iI == TECH_RELIGIOUS_UNITY) {
									...
									break;
								case AYYUBIDS:
									if (iI == TECH_ASYMMETRIC_WARFARE) {
										iValue *= 4;
										iValue /= 3;
									}
									break;
								case MAMLUKS:
									if (iI == TECH_MILITARY_DRILL || iI == TECH_MATCHLOCK || iI == TECH_FLINTLOCK || iI == TECH_GUNPOWDER) {
										iValue *= 2;
										iValue /= 3;
									}									
									break;
								case OTTOMANS:
									...
									break;
								case TIMURIDS:
									if (iI == TECH_OPTICS || iI == TECH_ASTRONOMY) {
										iValue *= 5;
										iValue /= 4;
									}
									break;
								case AKKOYUNLU:
									if (iI == TECH_CIVIL_SERVICE) {
										iValue *= 2;
									}
									if (iI == TECH_PATRONAGE || iI == TECH_CITY_PLANNING || iI == TECH_ARCHITECTURE) {
										iValue *= 2;
										iValue /= 3;
									}
									if (iI == TECH_ASTROLABE || iI == TECH_STERN_RUDDER || iI == TECH_COMPASS || iI == TECH_NAVIGATION) {
									...
									break;
								case SAFAVIDS:
									if (iI == TECH_PATRONAGE || iI == TECH_CITY_PLANNING) {
										iValue *= 4;
										iValue /= 3;
									}
									if (iI == TECH_ARCHITECTURE || iI == TECH_GUNPOWDER || iI == TECH_MATCHLOCK) {
										iValue *= 5;
										iValue /= 4;
									}
									break;
								...
								case MUGHALS:
									...
									if (iI == TECH_ARCHITECTURE || iI == TECH_METALLURGY || iI == TECH_SYNCRETISM) {
										iValue *= 5;
										iValue /= 4;
									}
									break;
...
int CvPlayerAI::AI_getAttitudeVal(PlayerTypes ePlayer, bool bForced) const
{
	...
	iAttitude += getAttitudeModifier(ePlayer); // edead: permanent modifier
...
//	}
	...
	iAttitude += AI_getAttitudeCaliph(ePlayer); // edead
	iAttitude += AI_getAttitudeSharif(ePlayer); // edead
	iAttitude += AI_getAttitudeRaja(ePlayer); // edead
	iAttitude += GET_PLAYER(ePlayer).getDiplomacyModifier(); // edead
...
int CvPlayerAI::AI_getSameReligionAttitude(PlayerTypes ePlayer) const
{
	...
	// edead: begin religions - count Christians as one but with less bonus (below); should not apply to Sunni/Shia
	if ((getStateReligion() != NO_RELIGION) && (getStateReligion() == GET_PLAYER(ePlayer).getStateReligion()))
	{
		bSameReligion = true;
	}
	
	if (((getStateReligion() == RELIGION_CATHOLICISM) && (GET_PLAYER(ePlayer).getStateReligion() == RELIGION_ORTHODOXY)) || ((getStateReligion() == RELIGION_ORTHODOXY) && (GET_PLAYER(ePlayer).getStateReligion() == RELIGION_CATHOLICISM)))
	{
		bSameReligion = true;
	}
	...
	// edead: less love between Christians
	if (((getStateReligion() == RELIGION_CATHOLICISM) && (GET_PLAYER(ePlayer).getStateReligion() == RELIGION_ORTHODOXY)) || ((getStateReligion() == RELIGION_ORTHODOXY) && (GET_PLAYER(ePlayer).getStateReligion() == RELIGION_CATHOLICISM)))
	{
		iAttitude /= 2;
	}
...
int CvPlayerAI::AI_getDifferentReligionAttitude(PlayerTypes ePlayer) const
{
	...
	{
		// edead: begin religions - return 0 for Christians
		if (((getStateReligion() == RELIGION_CATHOLICISM) && (GET_PLAYER(ePlayer).getStateReligion() == RELIGION_ORTHODOXY)) || ((getStateReligion() == RELIGION_ORTHODOXY) && (GET_PLAYER(ePlayer).getStateReligion() == RELIGION_CATHOLICISM)))
		{
			return 0;
		}
	...
	// edead: less hatred between Sunni/Shia and Hindu/Buddhist
	if (((getStateReligion() == RELIGION_SUNNI) && (GET_PLAYER(ePlayer).getStateReligion() == RELIGION_SHIA)) || ((getStateReligion() == RELIGION_SHIA) && (GET_PLAYER(ePlayer).getStateReligion() == RELIGION_SUNNI)))
	{
		iAttitude /= 2;
	}
	else if (((getStateReligion() == RELIGION_HINDUISM) && (GET_PLAYER(ePlayer).getStateReligion() == RELIGION_BUDDHISM)) || ((getStateReligion() == RELIGION_BUDDHISM) && (GET_PLAYER(ePlayer).getStateReligion() == RELIGION_HINDUISM)))
	{
		iAttitude /= 2;
	}
	// edead: end
...
bool CvPlayerAI::AI_counterPropose(PlayerTypes ePlayer, const CLinkList<TradeData>* pTheirList, const CLinkList<TradeData>* pOurList, CLinkList<TradeData>* pTheirInventory, CLinkList<TradeData>* pOurInventory, CLinkList<TradeData>* pTheirCounter, CLinkList<TradeData>* pOurCounter) const
{
	...
	if (iAIDealWeight > iHumanDealWeight)
	{
		...
		for (pNode = pOurInventory->head(); pNode && iHumanDealWeight > iAIDealWeight; pNode = pOurInventory->next(pNode))
		{
			if (!pNode->m_data.m_bOffering && !pNode->m_data.m_bHidden)
			{
				if (pNode->m_data.m_eItemType == TRADE_RELIC)
				{
					...
					if (getTradeDenial(ePlayer, pNode->m_data) == NO_DENIAL)
					{
						...
						if (pUnit != NULL)
						{
							iWeight = GET_PLAYER(ePlayer).AI_relicTradeVal(pUnit);
							if (iWeight > 0)
							{
...
int CvPlayerAI::AI_bonusTradeVal(BonusTypes eBonus, PlayerTypes ePlayer, int iChange) const
{
	...
	// edead: start
	if ((eBonus == BONUS_PIG) || (eBonus == BONUS_WINE)) 
	{
		if ((GET_PLAYER(ePlayer).getStateReligion() == RELIGION_SUNNI) || (GET_PLAYER(ePlayer).getStateReligion() == RELIGION_SHIA))
		{
		iValue *= 2;
		iValue /= 3;
		}
...
int CvPlayerAI::AI_cityTradeVal(CvCity* pCity) const
{
	...
	if (GET_PLAYER(pCity->getOwnerINLINE()).getNumCities() > 12) 
		if (pCity->plot()->getFoundModifier(getID()) < 400) { //not in core area
			iValue *= 2;
			iValue /= 3;
		}
	//Rhye - end
...
//			iValue += ((iCombatValue * iCombatModifier) / 100);
	...
	case UNITAI_MISSIONARY:
		...
		if (getStateReligion() != NO_RELIGION)
		{
			...
			{
				//iValue += (5 * GC.getUnitInfo(eUnit).getReligionSpreads(getStateReligion())) / 2; // edead: more internal spread
				iValue += (7 * GC.getUnitInfo(eUnit).getReligionSpreads(getStateReligion())) / 2; // edead: more internal spread
			}
		}
		for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
		{
...
/************************************************************************************************/
	...
	// edead: start
	iValue *= 100 + AI_getUnitAIModifier(eUnitAI);
	iValue /= 100;
	// edead: end
...
int CvPlayerAI::AI_neededMissionaries(CvArea* pArea, ReligionTypes eReligion) const
{
            ...
            if (!bCultureVictory)
	{
                ...
                iCount = std::max(1, iCount / (bHoly ? 2 : 3)); // edead: more internal spread
            }
            return iCount;
        }
	}
...
/************************************************************************************************/	
	...
	iValue += -((kCivic.getDistanceMaintenanceModifier() * (getNumCities() - 4)) / 8); // edead: make it less attractive for small civs
	//iValue += -((kCivic.getNumCitiesMaintenanceModifier() * std::max(0, (getNumCities() - 3))) / 8);
	iValue += -((kCivic.getNumCitiesMaintenanceModifier() * (getNumCities() - 3)) / 8); // edead: make it less attractive for small civs
	...
	iValue += ((kCivic.getImprovementUpgradeRateModifier() * getNumCities()) / (kCivic.getImprovementUpgradeRateModifier() < 0 ? 25 : 50)); // edead
	...
	iValue += ((kCivic.getCoastalTradeRoutes() * std::max(0, iConnectedForeignCities - countNumCoastalCities() * 3) * 6) + (countNumCoastalCities() * 2)); // edead
...
/************************************************************************************************/
	...
	{
		iValue += (getNumCities() * 6 * AI_getHappinessWeight(kCivic.getCivicHappiness(), 1)) / 100;
	}
	// edead: end
...
/************************************************************************************************/
	...
	iValue += (kCivic.getNonStateReligionHappiness() * (iTotalReligonCount - iHighestReligionCount) * 4);
	...
	if (kCivic.isStateReligion())
	{
		if (iHighestReligionCount > 0)
		{
			...
			//iValue += ((kCivic.getStateReligionGreatPeopleRateModifier() * iHighestReligionCount) / 20); // edead: moved up
			//iValue += (kCivic.getStateReligionGreatPeopleRateModifier() / 4); // edead: moved up
	...
	for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		...
		if (pCapital) 
		{
			...
			if (getNumCities() > 6) 
			{
				iTempValue = (iTempValue * 6) / getNumCities();
			}
			// edead: end
		}
		iTempValue += ((kCivic.getYieldModifier(iI) * getNumCities()) / 2); // edead: moved here from the beginning
	...
	// stop vassals from switching away from master's civics
	...
	for (int iPlayer = 0; iPlayer < NUM_MAJOR_PLAYERS; iPlayer++)
	{
		...
		{
			eMaster = (PlayerTypes)iPlayer;
			break;
		}
	}
	if (eMaster != NO_PLAYER)
	{
		if (GET_PLAYER(eMaster).getCivics((CivicOptionTypes)GC.getCivicInfo(eCivic).getCivicOptionType()) == eCivic)
		{
			if (getCivics((CivicOptionTypes)GC.getCivicInfo(eCivic).getCivicOptionType()) == eCivic)
			{
				if (eCivic == CIVIC_PERSECUTION)
				{
					iValue *= 2;
				}
				else
	...
	switch (getID()) {
		...
		case ARMENIA:
		case GEORGIA:
			if (eCivic == CIVIC_TRIBAL_FEDERATION || eCivic == CIVIC_CASTE_SYSTEM || eCivic == CIVIC_THEOCRACY || eCivic == CIVIC_EMPIRE || eCivic == CIVIC_TRIBALISM || eCivic == CIVIC_TRIBAL_LAW)
			{
				iValue *= 2;
				iValue /= 3;
			}
			if (eCivic == CIVIC_MARKET_ECONOMY || eCivic == CIVIC_MERCHANT_CAPITALISM || eCivic == CIVIC_SLAVERY)
			{
				iValue *= 3;
				iValue /= 4;
			}
			if (eCivic == CIVIC_SERFDOM)
			{
				iValue *= 4;
				iValue /= 3;
			}
			break;
		case BYZANTIUM:
			if (eCivic == CIVIC_TRIBAL_FEDERATION || eCivic == CIVIC_CASTE_SYSTEM || eCivic == CIVIC_THEOCRACY || eCivic == CIVIC_TRIBALISM || eCivic == CIVIC_TRIBAL_LAW)
			{
				iValue *= 2;
				iValue /= 3;
			}
			if (eCivic == CIVIC_EMPIRE)
			{
				iValue *= 4;
				iValue /= 3;
			}
		...
		case AYYUBIDS:
		case ZENGIDS:
			if (eCivic == CIVIC_RELIGIOUS_LAW)
			{
				iValue *= 2;
			}
			if (eCivic == CIVIC_SERFDOM || eCivic == CIVIC_TRIBAL_FEDERATION || eCivic == CIVIC_CASTE_SYSTEM || eCivic == CIVIC_FREE_RELIGION || eCivic == CIVIC_TRIBALISM || eCivic == CIVIC_TRIBAL_LAW)
			{
				iValue *= 2;
				iValue /= 3;
			}
		...
		case MAMLUKS:
			...
			if (eCivic == CIVIC_SERFDOM || eCivic == CIVIC_TRIBAL_FEDERATION || eCivic == CIVIC_FREE_RELIGION || eCivic == CIVIC_TRIBALISM || eCivic == CIVIC_TRIBAL_LAW)
			{
				iValue *= 2;
				iValue /= 3;
			}
			...
			break;
		case FATIMIDS:
			if (eCivic == CIVIC_TRIBAL_FEDERATION || eCivic == CIVIC_CASTE_SYSTEM || eCivic == CIVIC_THEOCRACY || eCivic == CIVIC_PERSECUTION || eCivic == CIVIC_TRIBALISM || eCivic == CIVIC_TRIBAL_LAW)
			{
				iValue *= 2;
				iValue /= 3;
			}
		...
		case SAFAVIDS:
			...
			if (eCivic == CIVIC_PERSECUTION || eCivic == CIVIC_EMPIRE || eCivic == CIVIC_ABSOLUTISM || eCivic == CIVIC_STATE_MONOPOLY)
			{
				iValue *= 3;
				iValue /= 2;
			}
			if (eCivic == CIVIC_SERFDOM || eCivic == CIVIC_TRIBAL_FEDERATION || eCivic == CIVIC_CASTE_SYSTEM || eCivic == CIVIC_FREE_RELIGION)
			{
				iValue /= 2;
			}
			break;
		case OMAN:
			...
			if (eCivic == CIVIC_MERCHANT_CAPITALISM || eCivic == CIVIC_MARKET_ECONOMY || eCivic == CIVIC_ARISTOCRACY)
			{
				iValue *= 4;
				iValue /= 3;
			}
			if (eCivic == CIVIC_SERFDOM || eCivic == CIVIC_TRIBAL_FEDERATION || eCivic == CIVIC_CASTE_SYSTEM || eCivic == CIVIC_FREE_RELIGION || eCivic == CIVIC_EMPIRE || eCivic == CIVIC_ABSOLUTISM)
			{
				iValue /= 2;
			}
			break;
...
ReligionTypes CvPlayerAI::AI_bestReligion() const
{
	...
	for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
	{
		if (canDoReligion((ReligionTypes)iI))
		{
			...
			if (getStateReligion() == ((ReligionTypes)iI))
			{
				//iValue *= 4;
				//iValue /= 3;
				iValue *= 2; // edead: less likely to switch
			}
			...
			if (eMaster != NO_PLAYER)
			{
				if (GET_PLAYER(eMaster).getStateReligion() == iI && getStateReligion() == iI)
				{
					iValue *= 3;
					iValue /= 2;
				}
	...
	// edead: start comment (no atheism)
...
int CvPlayerAI::AI_religionValue(ReligionTypes eReligion) const
{
	...
	// edead: start
	if ((eReligion == RELIGION_BUDDHISM) || (eReligion == RELIGION_JUDAISM) || (eReligion == RELIGION_ZOROASTRIANISM))
	{
		return 0;
	}
	...
	switch (getID()) 
	{
		...
		case OUTREMER:
		case ANTIOCH:
			if (eReligion == RELIGION_CATHOLICISM)
				iValue += 10;
			else if (eReligion == RELIGION_ORTHODOXY)
				return iValue;
			else
				return 0;
			break;
		...
		case ARMENIA:
		case GEORGIA:
			if ((eReligion == RELIGION_ORTHODOXY) || (eReligion == RELIGION_CATHOLICISM))
			{
				iValue += 10;
			}
			else
		...
		case BYZANTIUM:
			if (eReligion == RELIGION_ORTHODOXY)
				iValue += 10;
			else if (eReligion == RELIGION_CATHOLICISM)
				iValue /= 2;
			else
				return 0;
			break;
		...
		case KHWAREZM:
		case TIMURIDS:
			if (eReligion == RELIGION_SUNNI)
				iValue += 10;
			else if (eReligion == RELIGION_SHIA)
				return iValue;
			else
				iValue /= 4;
			break;
		...
		case FATIMIDS:
		case SAFAVIDS:
			if (eReligion == RELIGION_SHIA)
				iValue += 10;
			else if (eReligion == RELIGION_SUNNI)
				return iValue;			
			else
				iValue /= 4;
			break;
		...
		case MUGHALS:
		case OMAN:
			if ((eReligion == RELIGION_SUNNI) || (eReligion == RELIGION_SHIA))
				iValue += 10;
			else
				iValue /= 4;
			break;
		...
		case CHAUHAN:
			if (eReligion != RELIGION_HINDUISM)
				iValue /= 2;
			break;
...
int CvPlayerAI::AI_espionageVal(PlayerTypes eTargetPlayer, EspionageMissionTypes eMission, CvPlot* pPlot, int iData) const
{
	...
	if (eTargetPlayer == NO_PLAYER || eTargetPlayer >= NUM_MAJOR_PLAYERS) // edead: no espionage vs. minor civs
	{
		return 0;
	}
...
// edead: start titles
int CvPlayerAI::AI_getAttitudeCaliph(PlayerTypes ePlayer) const
{
	if (GET_TEAM(GET_PLAYER(ePlayer).getTeam()).getProjectCount(PROJECT_CALIPH) > 0)
	{
		if (getStateReligion() == GET_PLAYER(ePlayer).getStateReligion())
		{
			return 1;
		}
	}
...
int CvPlayerAI::AI_getAttitudeSharif(PlayerTypes ePlayer) const
{
	if (GET_TEAM(GET_PLAYER(ePlayer).getTeam()).getProjectCount(PROJECT_SHARIF_OF_MECCA) > 0)
	{
		if ((getStateReligion() == RELIGION_SUNNI) || (getStateReligion() == RELIGION_SHIA))
		{
			return 1;
		}
	}
...
int CvPlayerAI::AI_getAttitudeRaja(PlayerTypes ePlayer) const
{
	if (GET_TEAM(GET_PLAYER(ePlayer).getTeam()).getProjectCount(PROJECT_RAJA) > 0)
	{
		if ((getStateReligion() == RELIGION_HINDUISM))
		{
			return 2;
		}
	}
...
/********************************************************************************/
		...
		*/
		...
		int iEspionageTargetRate = 0;
		int* piTarget = new int[NUM_MAJOR_PLAYERS];
		int* piWeight = new int[NUM_MAJOR_PLAYERS];
...
/********************************************************************************/
		...
		else
		{
			...
			iEspionageTargetRate /= 2; //edead: from Rhye, to counterbalance the number of players
					...
					if (iI != getID())
					{
						...
						if (canContact((PlayerTypes)iI) && AI_isWillingToTalk((PlayerTypes)iI))
						{
							if (GET_PLAYER((PlayerTypes)iI).getTeam() == getTeam() || GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isVassal(getTeam()))
							{
								...
								// edead: start human vassal balance
								if (GET_PLAYER((PlayerTypes)iI).isHuman() && iBestValue > GC.getDefineINT("HUMAN_AS_VASSAL_BONUS_GIFT_CHANCE") * 100)
								{
									eBestGiveBonus = NO_BONUS;
								}
								// edead: end
								...
								if (eBestGiveBonus != NO_BONUS)
								{
									...
									if (GET_PLAYER((PlayerTypes)iI).isHuman())
									{
										// edead: human vassal fix
										//if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
										if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]) && (GET_PLAYER((PlayerTypes)iI).canTradeNetworkWith(getID()))) // edead
										{
											pDiplo = new CvDiploParameters(getID());
											FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
							...
							if (GET_PLAYER((PlayerTypes)iI).getTeam() != getTeam() && GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isVassal(getTeam()))
							{
								...
								// edead: start human vassal balance
								if (GET_PLAYER((PlayerTypes)iI).isHuman() && iBestValue > GC.getDefineINT("HUMAN_AS_VASSAL_TECH_GIFT_CHANCE") * 100)
								{
									eBestGiveTech = NO_TECH;
								}
								// edead: end
								...
								if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isVassal(getTeam()))
								{
									...
									if (eBestGiveBonus != NO_BONUS)
									{
										...
										if (GET_PLAYER((PlayerTypes)iI).isHuman())
										{
											if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
											{
												if (GET_PLAYER((PlayerTypes)iI).canTradeNetworkWith(getID())) { // edead: human vassal fix
...
/************************************************************************************************/
									...
									//if (GET_TEAM(getTeam()).getLeaderID() == getID())
									{
										if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_TRADE_RELIC) == 0)
										{
											if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_TRADE_RELIC), "AI Diplo Trade Relic") == 0)
											{
												...
												//{
													...
													for (CvUnit* pLoopUnit = GET_PLAYER((PlayerTypes)iI).firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = GET_PLAYER((PlayerTypes)iI).nextUnit(&iLoop))
													{
														...
														{
															setTradeItem(&item, TRADE_RELIC, pLoopUnit->getID());
													...
													if (pRelic != NULL)
													{
														iTheirValue = GET_PLAYER((PlayerTypes)iI).AI_relicTradeVal(pRelic);
														...
														if (iGold >= iTheirValue && iTheirValue > 0)
														{
															...
															if (canTradeItem((PlayerTypes)iI, item, true))
															{
																...
																setTradeItem(&item, TRADE_RELIC, pWorker->getID());
																theirList.insertAtEnd(item);
																...
																if (GET_PLAYER((PlayerTypes)iI).isHuman())
																{
																	if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
																	{
																		AI_changeContactTimer(((PlayerTypes)iI), CONTACT_TRADE_RELIC, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_TRADE_RELIC));
																		pDiplo = new CvDiploParameters(getID());
																		FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
																		pDiplo->setDiploComment((DiploCommentTypes)GC.getInfoTypeForString("AI_DIPLOCOMMENT_OFFER_DEAL"));
																		pDiplo->setAIContact(true);
...
void CvPlayerAI::read(FDataStreamBase* pStream)
{
	...
	// edead: start
	pStream->Read(&m_iCompactEmpireModifier);
	pStream->Read(&m_iMassacreProb);
	pStream->Read(&m_iBuildPersecutorProb);
	pStream->Read(&m_iWarDistanceModifier);
	pStream->Read(&m_iWarCoastalModifier);
	pStream->Read(&m_iMaxTakenTiles);
	pStream->Read(NUM_UNITAI_TYPES, m_aiUnitAIModifier); 
	// edead: end
...
void CvPlayerAI::write(FDataStreamBase* pStream)
{
	...
	// edead: start
	pStream->Write(m_iCompactEmpireModifier);
	pStream->Write(m_iMassacreProb);
	pStream->Write(m_iBuildPersecutorProb);
	pStream->Write(m_iWarDistanceModifier);
	pStream->Write(m_iWarCoastalModifier);
	pStream->Write(m_iMaxTakenTiles);
	pStream->Write(NUM_UNITAI_TYPES, m_aiUnitAIModifier);
	// edead: end
...
bool CvPlayerAI::AI_disbandUnit(int iExpThreshold, bool bObsolete)
{
	...
	// edead: no disbanding during civ birth - added 5.07.2010
	if (GC.getGameINLINE().getGameTurn() < getTurnForYear(getStartYear())+10)
	{
		return false;
	}
	// edead: end
...
void CvPlayerAI::AI_updateCitySites(int iMinFoundValueThreshold, int iMaxSites) const
{
	...
	while (iPass < iMaxSites)
	{
		...
		for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
		{
			...
			if (pLoopPlot->isRevealed(getTeam(), false))
			{
				if (pLoopPlot->getFoundModifier(getID()) >= 90) //Rhye
				{ //Rhye
...
// BEGIN: Show Hidden Attitude Mod - modified for RFC by edead
int CvPlayerAI::AI_getFirstImpressionAttitude(PlayerTypes ePlayer) const
{
	...
	iAttitude += GC.getLeaderHeadInfo(getPersonalityType()).getBaseAttitude();
	iAttitude += getAttitudeModifier(ePlayer); // edead: permanent attitude modifier
	...
	if (GC.getGameINLINE().isDebugMode())
	{
		if (!kPlayer.isHuman())
		{
			...
			// }
			iAttitude += (4 - abs(AI_getPeaceWeight() - kPlayer.AI_getPeaceWeight())); // edead: this spoiler is OK
...
int CvPlayerAI::AI_getKnownPlayerRank(PlayerTypes ePlayer) const
{
	...
	int iRank = 0;
	for (int iI = 0; iI < NUM_MAJOR_PLAYERS; iI++) // edead: MAX_CIV_PLAYERS -> NUM_MAJOR_PLAYERS 
	{
...
int CvPlayerAI::AI_getLostWarAttitude(PlayerTypes ePlayer) const
{
	...
	if (!GC.getGameINLINE().isDebugMode() && NO_PLAYER != GC.getGameINLINE().getActivePlayer())
	{
...
int CvPlayerAI::AI_getCompactEmpireModifier() const
{
	return m_iCompactEmpireModifier;
}

void CvPlayerAI::AI_setCompactEmpireModifier(int iNewValue)
{
	m_iCompactEmpireModifier = iNewValue;
}

int CvPlayerAI::AI_getMassacreProb() const
{
	return m_iMassacreProb;
}

void CvPlayerAI::AI_setMassacreProb(int iNewValue)
{
	m_iMassacreProb = iNewValue;
}

int CvPlayerAI::AI_getBuildPersecutorProb() const
{
	return m_iBuildPersecutorProb;
}

void CvPlayerAI::AI_setBuildPersecutorProb(int iNewValue)
{
	m_iBuildPersecutorProb = iNewValue;
}

int CvPlayerAI::AI_getWarDistanceModifier() const
{
	return m_iWarDistanceModifier;
}

void CvPlayerAI::AI_setWarDistanceModifier(int iNewValue)
{
	m_iWarDistanceModifier = std::max(1, iNewValue);
}

int CvPlayerAI::AI_getWarCoastalModifier() const
{
	return m_iWarCoastalModifier;
}

void CvPlayerAI::AI_setWarCoastalModifier(int iNewValue)
{
	m_iWarCoastalModifier = iNewValue;
}

int CvPlayerAI::AI_getMaxTakenTiles() const
{
	return m_iMaxTakenTiles;
}

void CvPlayerAI::AI_setMaxTakenTiles(int iNewValue)
{
	m_iMaxTakenTiles = iNewValue;
}

int CvPlayerAI::AI_getUnitAIModifier(UnitAITypes eUnitAI) const
{
	FAssertMsg((eUnitAI >= 0), "eUnitAI is less than zero");
	FAssertMsg((eUnitAI < NUM_UNITAI_TYPES), "eUnitAI is higher than NUM_UNITAI_TYPES");
	return m_aiUnitAIModifier[eUnitAI];
}

void CvPlayerAI::AI_setUnitAIModifier(UnitAITypes eUnitAI, int iValue)
{
	FAssertMsg((eUnitAI >= 0), "eUnitAI is less than zero");
	FAssertMsg((eUnitAI < NUM_UNITAI_TYPES), "eUnitAI is higher than NUM_UNITAI_TYPES");
	m_aiUnitAIModifier[eUnitAI] = iValue;
}

int CvPlayerAI::AI_relicTradeVal(CvUnit* pUnit) const
{
	if (!(GC.getUnitInfo(pUnit->getUnitType()).isRelic()))
	{
		return 0;
	}

	/*int iValue = GC.getDefineINT("AI_RELIC_VALUE");
	...
	return GC.getDefineINT("AI_RELIC_VALUE");
}
