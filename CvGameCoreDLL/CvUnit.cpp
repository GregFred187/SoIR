...
void CvUnit::init(int iID, UnitTypes eUnit, UnitAITypes eUnitAI, PlayerTypes eOwner, int iX, int iY, DirectionTypes eFacingDirection)
{
	...
	{
		//int iOffset = GC.getGameINLINE().getSorenRandNum(iNumNames, "Unit name selection"); // edead: historical names!

		for (iI = 0; iI < iNumNames; iI++)
		{
			//int iIndex = (iI + iOffset) % iNumNames; // edead: historical names!
			int iIndex = (iI + iUnitName) % iNumNames; // edead: historical names!
...
void CvUnit::reset(int iID, UnitTypes eUnit, PlayerTypes eOwner, bool bConstructorCall)
{
	...
	m_eFacingDirection = DIRECTION_SOUTH;
	m_iImmobileTimer = 0;
	m_iBarbarianPercent = 0; // edead
	m_bCannotUpgrade = false; // edead
...
void CvUnit::kill(bool bDelay, PlayerTypes ePlayer)
{
	...
	if (ePlayer != NO_PLAYER)
	{
		...
		if (NO_UNIT != getLeaderUnitType())
		{
			...
			{
				//if (GET_PLAYER((PlayerTypes)iI).isAlive())
				if (GET_PLAYER((PlayerTypes)iI).isAlive() && GC.getGameINLINE().getGameTurn() > getTurnForYear(GET_PLAYER((PlayerTypes)iI).getStartYear())) // edead
				{
...
void CvUnit::resolveCombat(CvUnit* pDefender, CvPlot* pPlot, CvBattleDefinition& kBattle)
{
	...
	// edead: start AI luck part 1
	int iCombatDieSides = GC.getCOMBAT_DIE_SIDES();
	if (!isHuman() && !pDefender->isHuman())
	{
		iCombatDieSides += GET_PLAYER(getOwnerINLINE()).getLuck();
		iCombatDieSides -= GET_PLAYER(pDefender->getOwnerINLINE()).getLuck();
	}
	// edead: end part 1

	while (true)
	{
		// edead: start AI luck part 2
		//if (GC.getGameINLINE().getSorenRandNum(GC.getDefineINT("COMBAT_DIE_SIDES"), "Combat") < iDefenderOdds)
		if (GC.getGameINLINE().getSorenRandNum(iCombatDieSides, "Combat") < iDefenderOdds)
		// edead: end part 2
		{
			if (getCombatFirstStrikes() == 0)
			{
				if (getDamage() + iAttackerDamage >= maxHitPoints() && GC.getGameINLINE().getSorenRandNum(100, "Withdrawal") < withdrawalProbability())
				{
					...
					// edead: start global XP from Mongols
					bool bUpdateGlobal = true;
					if (pDefender->isBarbarian() && !pDefender->isAlwaysFullXP()) bUpdateGlobal = false;
					changeExperience(GC.getDefineINT("EXPERIENCE_FROM_WITHDRAWL"), pDefender->maxXPValue(), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), bUpdateGlobal);
		...
		if (isDead() || pDefender->isDead())
		{
			if (isDead())
			{
				...
				// edead: start global XP from Mongols
				bool bUpdateGlobal = true;
				if (isBarbarian() && !isAlwaysFullXP()) bUpdateGlobal = false;
				pDefender->changeExperience(iExperience, maxXPValue(), true, pPlot->getOwnerINLINE() == pDefender->getOwnerINLINE(), bUpdateGlobal);
			...
			else
			{
				...
				bool bUpdateGlobal = true;
				if (pDefender->isBarbarian() && !pDefender->isAlwaysFullXP()) bUpdateGlobal = false;
				changeExperience(iExperience, pDefender->maxXPValue(), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), bUpdateGlobal);
...
void CvUnit::updateCombat(bool bQuick)
{
	...
	if (!bFinish)
	{
		if (!isFighting())
		{
			...
			else
			{
				...
				{
					//szMessage = gDLL->getText("TXT_KEY_MISC_YOU_UNITS_UNDER_ATTACK", GET_PLAYER(getOwnerINLINE()).getNameKey()); // edead
					szMessage = gDLL->getText("TXT_KEY_MISC_YOU_UNITS_UNDER_ATTACK", GET_PLAYER(getOwnerINLINE()).getCivilizationDescription()); // edead
				}
				else
				{
	...
	if (bFinish)
	{
		...
		// edead: start IDW suppression during AI autoplay
		bool bInfluenceEnabled = bool(GC.getIDW_ENABLED());
		if (GC.getGameINLINE().getGameTurn() < getTurnForYear(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getStartYear()))
		{
			bInfluenceEnabled = false;
		}
		// edead: end
		if (isDead())
		{
			...
			szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_UNIT_DIED_ATTACKING", wcrtrim(getNameKey()), wcrtrim(pDefender->getNameKey()));
			//szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_UNIT_DIED_ATTACKING", getNameKey(), (pDefender->isBarbarian() ? L"" : pDefender->getNameKey())); // edead
			// ------ BEGIN InfluenceDrivenWar -------------------------------
			float fInfluenceRatio = 0.0;
			if (bInfluenceEnabled)
			{
				fInfluenceRatio = pDefender->doVictoryInfluence(this, false, false);
				if (!GC.getIDW_NO_COMBAT_SPAM()) // edead
				{
			...
			szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_KILLED_ENEMY_UNIT", wcrtrim(pDefender->getNameKey()), wcrtrim(getNameKey()), getVisualCivAdjective(pDefender->getTeam()));
			//szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_KILLED_ENEMY_UNIT", (pDefender->isBarbarian() ? L"" : pDefender->getNameKey()), getNameKey(), getVisualCivAdjective(pDefender->getTeam())); // edead
			// ------ BEGIN InfluenceDrivenWar -------------------------------
			if (bInfluenceEnabled)
			{
				if (!GC.getIDW_NO_COMBAT_SPAM()) // edead
		...
		else if (pDefender->isDead())
		{
			...
			szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_UNIT_DESTROYED_ENEMY", wcrtrim(getNameKey()), wcrtrim(pDefender->getNameKey()));
			//szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_UNIT_DESTROYED_ENEMY", getNameKey(), (pDefender->isBarbarian() ? L"" : pDefender->getNameKey())); // edead
			// ------ BEGIN InfluenceDrivenWar -------------------------------
			float fInfluenceRatio = 0.0;			
			if (bInfluenceEnabled)
			{
			...
			{
				szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_UNIT_WAS_DESTROYED_UNKNOWN", wcrtrim(pDefender->getNameKey()), wcrtrim(getNameKey()));
			}
			else
			{
				szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_UNIT_WAS_DESTROYED", wcrtrim(pDefender->getNameKey()), wcrtrim(getNameKey()), getVisualCivAdjective(pDefender->getTeam()));
				//szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_UNIT_WAS_DESTROYED", (pDefender->isBarbarian() ? L"" : pDefender->getNameKey()), getNameKey(), getVisualCivAdjective(pDefender->getTeam())); // edead
			}
			// ------ BEGIN InfluenceDrivenWar -------------------------------
			if ((bInfluenceEnabled) && (!GC.getIDW_NO_COMBAT_SPAM()))
			{
				CvWString szTempBuffer;
				szTempBuffer.Format(L" Influence: -%.1f%%", fInfluenceRatio);
				szBuffer += szTempBuffer;
		...
		{
			szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_UNIT_WITHDRAW", wcrtrim(getNameKey()), wcrtrim(pDefender->getNameKey()));
			...
			szBuffer = gDLL->getText("TXT_KEY_MISC_ENEMY_UNIT_WITHDRAW", wcrtrim(getNameKey()), wcrtrim(pDefender->getNameKey()));
...
void CvUnit::move(CvPlot* pPlot, bool bShow)
{
	...
	// edead: start auto-pillage
	if (GC.getUnitInfo(getUnitType()).getAutoPillageProb() > 0)
	{
		quickPillage();
	}
	// edead: end
...
bool CvUnit::canGift(bool bTestVisible, bool bTestTransport)
{
	...
	// edead: no gifting on missionaries
	...
	// edead: only allow gifting between masters and vassals
	if (!GET_TEAM(pPlot->getTeam()).isVassal(getTeam()) && !GET_TEAM(getTeam()).isVassal(pPlot->getTeam()))
	{
		return false;
	}	
	// edead: end
	...
	if (pPlot->getImprovementType() != NO_IMPROVEMENT)
	{
		...
		if (pPlot->getTeam() != getTeam())
		{
			...
			if (iPillageGold > 0)
			{
				...
				if (GC.getIDW_ENABLED() && GC.getIDW_PILLAGE_INFLUENCE_ENABLED() && (GC.getGameINLINE().getGameTurn() >= getTurnForYear(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getStartYear())))
				{
				...
				// ------ BEGIN InfluenceDrivenWar -------------------------------
				if ((fInfluenceRatio > 0.0f) && (!GC.getIDW_NO_COMBAT_SPAM())) // edead
				{
				...
				if (pPlot->isOwned())
				{
					...
					// ------ BEGIN InfluenceDrivenWar -------------------------------
					if ((fInfluenceRatio > 0.0f) && (!GC.getIDW_NO_COMBAT_SPAM()))
					{
						CvWString szInfluence;
						szInfluence.Format(L" Tile influence: -%.1f%%", fInfluenceRatio);
						szBuffer += szInfluence;
...
bool CvUnit::canFound(const CvPlot* pPlot, bool bTestVisible) const
{
	...
	// edead: bugfix
	if (!GET_PLAYER(getOwnerINLINE()).isHuman() && pPlot->getFoundModifier(getOwnerINLINE()) <= 3)
	{
		if (GET_PLAYER(getOwnerINLINE()).isFoundedFirstCity())
		{
			return false;
		}
	}
...
bool CvUnit::canConstruct(const CvPlot* pPlot, BuildingTypes eBuilding, bool bTestVisible) const
{
	...
	// edead: Mughal & Safavid UHV fix (same code as in CvPlayer::canConstruct())
	if (!GET_PLAYER(getOwnerINLINE()).isHuman())
	{
		if (eBuilding == SHALIMAR_GARDENS || eBuilding == TAJ_MAHAL || eBuilding == RED_FORT) {
			if (GET_PLAYER(MUGHALS).isHuman()) {
				if (GC.getGameINLINE().getGameTurn() < getTurnForYear(GET_PLAYER(MUGHALS).getStartYear()+50)) {
					return false;
				}
			}
			else if (GET_PLAYER(SAFAVIDS).isHuman()) {
				if (GC.getGameINLINE().getGameTurn() < getTurnForYear(GET_PLAYER(SAFAVIDS).getStartYear()+50)) {
					return false;
				}
			}
		}
		else if (eBuilding == SHAH_MOSQUE || eBuilding == GRAND_BAZAAR || eBuilding == ESFAHAN_BRIDGE) {
			if (GET_PLAYER(SAFAVIDS).isHuman()) {
				if (GC.getGameINLINE().getGameTurn() < getTurnForYear(GET_PLAYER(SAFAVIDS).getStartYear()+50)) {
					return false;
				}
			}
		}
		else if (eBuilding == MINARET_OF_JAM || eBuilding == QUTB_MINAR) {
			if (GET_PLAYER(GHORIDS).isHuman()) {
				if (GC.getGameINLINE().getGameTurn() < getTurnForYear(GET_PLAYER(GHORIDS).getStartYear()+50)) {
					return false;
				}
			}
		}
...
int CvUnit::getTradeGold(const CvPlot* pPlot) const
{
	...
	pCity = pPlot->getPlotCity();
	//pCapitalCity = GET_PLAYER(getOwnerINLINE()).getCapitalCity(); // edead
	...
	// edead: start (closest city, not capital city)
	pCapitalCity = NULL;
	int iClosestDistance = MAX_INT;
	int iLoop;
	for (CvCity* pLoopCity = GET_PLAYER(getOwnerINLINE()).firstCity(&iLoop); NULL != pLoopCity; pLoopCity = GET_PLAYER(getOwnerINLINE()).nextCity(&iLoop))
	{
		int iDistance = ::plotDistance(pPlot->getX_INLINE(), pPlot->getY_INLINE(), pLoopCity->getX_INLINE(), pLoopCity->getY_INLINE());
	...
	iGold = (m_pUnitInfo->getBaseTrade() + (m_pUnitInfo->getTradeMultiplier() * ((pCapitalCity != NULL) ? pCity->calculateTradeProfitTimes100(pCapitalCity) : 0))); // edead: mroe gradual income
	...
	iGold /= 100; // edead: because calculateTradeProfitTimes100 is used
...
bool CvUnit::espionage(EspionageMissionTypes eMission, int iData)
{
	...
	{
		// SuperSpies: edead start assassination bonus
		//if (testSpyIntercepted(eTargetPlayer, GC.getEspionageMissionInfo(eMission).getDifficultyMod()))
		int iSkill = 0;
		if (GC.getEspionageMissionInfo(eMission).getDestroyUnitCostFactor() > 0 || GC.getEspionageMissionInfo(eMission).getPlayerAnarchyCounter() > 0)
		{
			iSkill = getCollateralDamageProtection();
		}
		if (testSpyIntercepted(eTargetPlayer, GC.getEspionageMissionInfo(eMission).getDifficultyMod() - iSkill))
		// edead: end
		{
			return false;
		}
		...
		if (GET_PLAYER(getOwnerINLINE()).doEspionageMission(eMission, eTargetPlayer, plot(), iData, this))
		{
			...
			if (!testSpyIntercepted(eTargetPlayer, GC.getDefineINT("ESPIONAGE_SPY_MISSION_ESCAPE_MOD")))
			{
				...
				// ALN BetterSpyEscape Start
				CvCity* aeBestCities[3];
				int aiBestDist[3];
				int aiCityWeight[3];
				for (int iI = 0; iI < 3; iI++)
				{
					aeBestCities[iI] = NULL;
					aiBestDist[iI] = MAX_INT;
				}
				int iLoop;
				CvPlayerAI& kPlayer = GET_PLAYER(getOwnerINLINE());
				for (CvCity* pLoopCity = kPlayer.firstCity(&iLoop); pLoopCity != NULL; pLoopCity = kPlayer.nextCity(&iLoop))
				{
					// this will create an ordered list of the three closest cities
					// if less than three cities exist, the last one or two will remain NULL and MAX_INT
					int iDistance = plotDistance(getX_INLINE(), getY_INLINE(), pLoopCity->getX_INLINE(), pLoopCity->getY_INLINE());
					if (iDistance < aiBestDist[0])
					{
						if (aeBestCities[0] != NULL)
						{
							if (aeBestCities[1] != NULL)
							{
								aeBestCities[2] = aeBestCities[1];
								aiBestDist[2] = aiBestDist[1];
							}
							aeBestCities[1] = aeBestCities[0];
							aiBestDist[1] = aiBestDist[0];
						}
						aeBestCities[0] = pLoopCity;
						aiBestDist[0] = iDistance;
					}
					else if (iDistance < aiBestDist[1])
					{
						if (aeBestCities[1] != NULL)
						{
							aeBestCities[2] = aeBestCities[1];
						...
						aeBestCities[1] = pLoopCity;
						aiBestDist[1] = iDistance;
					}
					else if (iDistance < aiBestDist[2])
					{
						aeBestCities[2] = pLoopCity;
						aiBestDist[2] = iDistance;
					}
				}
				// assign weighted values to each city based on distance then a random factor
				// essentially the closer the next two cities to the first, the more randomness we'll have
				for (int iI = 0; iI < 3; iI++)
				{
					if (aeBestCities[iI] != NULL)
					{
						int iWeight = (aiBestDist[iI] * 100 / aiBestDist[0]);
						iWeight = ((iWeight - 100) / 2) + 100;
						iWeight += GC.getGameINLINE().getSorenRandNum(50, "Spy Escape");
						iWeight = 100 * 100 / iWeight;
						aiCityWeight[iI] = iWeight;
					}
					else
					{
						aiCityWeight[iI] = 0;
				...
				// and pick the one with the highest weight
				int iBestWeight = 0;
				for (int iI = 0; iI < 3; iI++)
				{
					if (aeBestCities[iI] != NULL && aiCityWeight[iI] > iBestWeight)
					{
						pCapital = aeBestCities[iI];
						iBestWeight = aiCityWeight[iI];
					}
				}
				// ALN BetterSpyEscape End
				if (NULL != pCapital)
				{
				...
				// assassination skill added by edead
				awardSpyExperience(GET_PLAYER(eTargetPlayer).getTeam(),GC.getEspionageMissionInfo(eMission).getDifficultyMod() - iSkill);
				//SuperSpies: TSHEEP end
			}
...
bool CvUnit::testSpyIntercepted(PlayerTypes eTargetPlayer, int iModifier)
{
	...
	if (GC.getGameINLINE().getSorenRandNum(10000, "Spy Interception") >= getSpyInterceptPercent(kTargetPlayer.getTeam()) * (100 + iModifier) + std::max(0, iModifier) * 10) // edead
	{
		return false;
	}
...
int CvUnit::getSpyInterceptPercent(TeamTypes eTargetTeam) const
{
	...
	if(getExtraEvasion() && iSuccess > 0)
	{
		// edead: this had to be done because 99% of the time the chance is 1%; therefore 1 point of deception (-10%) reduces the
		// chances of getting caught to 0%; 2 points makes all missions go to 100% and makes invincible even with counter-espionage
		// Deception values have been changed accordingly from 10% per level to 20%,25%,30% per level
		if (iSuccess > 1) iSuccess = std::max(1, iSuccess - getExtraEvasion());
		//iSuccess -= getExtraEvasion();
		//iSuccess *= 100 - getExtraEvasion();
		//iSuccess /= 100;
	}
	//SuperSpies: TSHEEP end
...
// edead: start
bool CvUnit::isAlwaysFullXP() const
{
	return m_pUnitInfo->isAlwaysFullXP();
}

int CvUnit::getBarbarianPercent() const
{
	return m_iBarbarianPercent;
}

void CvUnit::changeBarbarianPercent(int iChange)
{
	if (iChange != 0)
	{
		m_iBarbarianPercent += iChange;

		setInfoBarDirty(true);
	}
}

void CvUnit::changeCannotUpgrade(int iChange)			
{
	if (iChange > 0)
	{
		m_bCannotUpgrade = true;
	}
	else if (iChange < 0)
	{
		m_bCannotUpgrade = false;
	}
}

bool CvUnit::isMinorCiv() const
{
	return GET_PLAYER(getOwnerINLINE()).isMinorCiv();
}
// edead: end
...
int CvUnit::workRate(bool bMax) const
{
	...
	// edead: slower the independents' worker rate - added 7.12.2009
	...
	if (pAttacker != NULL)
	{
		...
		if (isBarbarian())
		{
			if (pAttacker->isHuman())
			{
				...
				iExtraModifier -= getBarbarianPercent(); // edead
				iModifier += iExtraModifier;
				if (pCombatDetails != NULL)
				{
...
const CvWString CvUnit::getName(uint uiForm) const
{
	...
	if ((NO_UNIT != getLeaderUnitType()) || (m_pUnitInfo->getNumUnitNames() > 0)) 
	{ 
		szBuffer.Format(L"%s (%s)", m_szName.GetCString(), m_pUnitInfo->getDescription(uiForm));
	}
	else 
...
void CvUnit::setHasPromotion(PromotionTypes eIndex, bool bNewValue)
{
	...
	if (isHasPromotion(eIndex) != bNewValue)
	{
		...
		changeBarbarianPercent(GC.getPromotionInfo(eIndex).getBarbarianPercent() * iChange); // edead
		changeCannotUpgrade((GC.getPromotionInfo(eIndex).isCannotUpgrade()) ? iChange : 0); // edead
...
void CvUnit::read(FDataStreamBase* pStream)
{
	...
	pStream->Read(&m_iBarbarianPercent); // edead
	pStream->Read(&m_bCannotUpgrade); // edead
	...
	pStream->ReadString(m_szArtDefineTag); // edead
...
void CvUnit::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(m_iBarbarianPercent); // edead
	pStream->Write(m_bCannotUpgrade); // edead
	...
	pStream->WriteString(m_szArtDefineTag); // edead
...
{
	// edead: start
	if (m_szArtDefineTag && m_szArtDefineTag != "")
	{
		return ARTFILEMGR.getUnitArtInfo(m_szArtDefineTag);
	}	
	// edead: end
...
// quick pillage, for use with barbarian auto-pillage
// this will pillage the tile without checking for all kinds of conditions that would slow it down
bool CvUnit::quickPillage()
{
	CvWString szBuffer;
	ImprovementTypes eTempImprovement = NO_IMPROVEMENT;
	RouteTypes eTempRoute = NO_ROUTE;
	...
	int iPillageImprovementProb = GC.getUnitInfo(getUnitType()).getAutoPillageProb();
	int iPillageRouteProb = iPillageImprovementProb / 2;
	if (pPlot->getRegionID() == 39) // Iraq
	{
		iPillageImprovementProb *= 2;
		iPillageRouteProb *= 2;
	}
	
	if (pPlot->getImprovementType() != NO_IMPROVEMENT)
	{
		if (GC.getASyncRand().get(100) < iPillageImprovementProb)
		{
			eTempImprovement = pPlot->getImprovementType();
			if (pPlot->isOwned())
			{
	...
	else if (pPlot->isRoute() && (GC.getASyncRand().get(100) < iPillageRouteProb))
	{
		eTempRoute = pPlot->getRouteType();
		pPlot->setRouteType(NO_ROUTE, true); // XXX downgrade rail???
	}
...
void CvUnit::setArtDefineTag(std::string szNewValue)
{
	m_szArtDefineTag = szNewValue;
	reloadEntity();
	gDLL->getInterfaceIFace()->setDirty(UnitInfo_DIRTY_BIT, true);
}
...
{
	if (GC.getIDW_NO_BARBARIAN_INFLUENCE())
	{
	...
	if (GC.getIDW_NO_NAVAL_INFLUENCE())
	{
	...
	float fWinnerPlotMultiplier = GC.getIDW_WINNER_PLOT_MULTIPLIER(); // by default: same influence in WinnerPlot and LoserPlot (1.0f)

	float fLoserPlotMultiplier = GC.getIDW_LOSER_PLOT_MULTIPLIER(); // by default: same influence in WinnerPlot and LoserPlot (1.0f)
	...
	{         	
		...
		{			
			// last city defender is dead -> influence is increased (default: 2.5f)
			...
			// {
				...
				// {
					...
					// gDLL->getInterfaceIFace()->addHumanMessage(pLoserUnit->getOwnerINLINE(), false, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, "AS2D_UNIT_BUILD_UNIT", MESSAGE_TYPE_INFO, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pLoserPlot->getX_INLINE(), pLoserPlot->getY_INLINE(), true, true);
					// gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), true, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, "AS2D_UNIT_BUILD_UNIT", MESSAGE_TYPE_INFO, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pLoserPlot->getX_INLINE(), pLoserPlot->getY_INLINE());
	...
	{				
		...
		{		
			// fort captured - influence is increased (default: 2.0f)
...
void CvUnit::influencePlots(CvPlot* pCentralPlot, PlayerTypes eTargetPlayer, float fLocationMultiplier)
{
	...
	// default: each point of experience increases influence by 1%
	...
	int iInfluenceRadius = GC.getIDW_INFLUENCE_RADIUS(); // default: like 2square city workable radius
	if (iInfluenceRadius < 0)
		return;

	float fPlotDistanceFactor = GC.getIDW_PLOT_DISTANCE_FACTOR(); // default 0.2f: influence decreases by 20% with plot distance

//	CvWString szBuffer;
//	szBuffer.Format(L"Factors: %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %.3f, %d", GC.getIDW_BASE_COMBAT_INFLUENCE(), fLocationMultiplier, fGameSpeedMultiplier, fPlotDistanceFactor, fExperienceMultiplier, fWarlordMultiplier, fBaseMultiplier, iInfluenceRadius);	
//	gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), true, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, "AS2D_UNIT_BUILD_UNIT", MESSAGE_TYPE_INFO, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pCentralPlot->getX_INLINE(), pCentralPlot->getY_INLINE());
...
{
	if (isBarbarian() && GC.getIDW_NO_BARBARIAN_INFLUENCE())
	{
		return 0.0f;
	}
	if ((DOMAIN_SEA == getDomainType()) && GC.getIDW_NO_NAVAL_INFLUENCE())
	{
		return 0.0f;
	}
	...
	int iCultureTransfer = int (GC.getIDW_BASE_PILLAGE_INFLUENCE() * fGameSpeedMultiplier * sqrt((float) iTargetCulture));
	if (iTargetCulture < iCultureTransfer)
	{
		// cannot transfer more culture than remaining target culure
		iCultureTransfer = iTargetCulture;
...
//	CvWString szBuffer;
//	szBuffer.Format(L"Factors: %.1f, %.1f, %d, Result: %.3f, ", fGameSpeedMultiplier, GC.getIDW_BASE_PILLAGE_INFLUENCE(), iTargetCulture, fInfluenceRatio);	
//	gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), true, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, "AS2D_UNIT_BUILD_UNIT", MESSAGE_TYPE_INFO, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), plot()->getX_INLINE(), plot()->getY_INLINE());
...
void CvUnit::tradeUnit(PlayerTypes eReceivingPlayer)
{
	...
	if (eReceivingPlayer != NO_PLAYER)
	{
		...
		gDLL->getInterfaceIFace()->addHumanMessage(pTradeUnit->getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_UNITGIFTED", MESSAGE_TYPE_INFO, pTradeUnit->getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), pTradeUnit->getX_INLINE(), pTradeUnit->getY_INLINE(), true, true);
	 }
}
