...
CvPlayer::CvPlayer()
{
	...
	// edead: start dynamic civ names (jdog)
	CvWString m_szName;
	...
	m_paiReligionSpreadPercent = NULL; // edead
	m_aiBorders = new int[MAX_PLAYERS]; // edead
	m_aiAttitudeModifier = new int[MAX_PLAYERS]; // edead
...
CvPlayer::~CvPlayer()
{
	...
	SAFE_DELETE_ARRAY(m_aiBorders); // edead
	SAFE_DELETE_ARRAY(m_aiAttitudeModifier); // edead
}
	...
	if ((GC.getInitCore().getSlotStatus(getID()) == SS_TAKEN) || (GC.getInitCore().getSlotStatus(getID()) == SS_COMPUTER))
	{
		...
		for (iI = 0; iI < GC.getNumTraitInfos(); iI++)
		{
			if (hasTrait((TraitTypes)iI))
			{
				...
				changeDiplomacyModifier(GC.getTraitInfo((TraitTypes)iI).getDiplomacyModifier()); // edead - bonus to diplomacy
...
void CvPlayer::uninit()
{
	...
	SAFE_DELETE_ARRAY(m_paiReligionSpreadPercent); // edead
...
void CvPlayer::reset(PlayerTypes eID, bool bConstructorCall)
{
	...
	// edead - start
	m_iCivicHappiness = 0;
	m_iDiplomacyModifier = 0;
	m_iLuck = 0;
	m_iStartYear = 0;
	m_iGrowthPercent = 100;
	m_iProductionPercent = 100;
	m_iResearchPercent = 100;
	m_iCulturePercent = 100;
	m_iEspionagePercent = 100;
	m_iGreatPeoplePercent = 100;
	m_bTurnPlayed = false;
	// edead - end
...
/************************************************************************************************/
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		m_aiBorders[iI] = 0; // edead
		m_aiAttitudeModifier[iI] = 0; // edead
	...
	if (!bConstructorCall)
	{
		...
		// edead: start
		FAssertMsg(m_paiReligionSpreadPercent==NULL, "about to leak memory, CvPlayer::m_paiReligionSpreadPercent");
		m_paiReligionSpreadPercent = new int[GC.getNumReligionInfos()];
		for (iI = 0;iI < GC.getNumReligionInfos();iI++)
		{
			m_paiReligionSpreadPercent[iI] = 100;
		}
		// edead: end
...
void CvPlayer::acquireCity(CvCity* pOldCity, bool bConquest, bool bTrade, bool bUpdatePlotGroups)
{
	...
	if (bConquest)
	{
		...
		argsList.add(getID()); // edead: for Raider trait
	...
	for (iI = 0; iI < GC.getNumBuildingInfos(); iI++)
	{
		...
		if (paiNumRealBuilding[iI] > 0)
		{
			...
			if (eBuilding != NO_BUILDING)
			{
				...
				if (!(GC.getBuildingInfo((BuildingTypes)iI).isNeverCapture())) // edead: bNeverCapture should always work
				{
	...
	if (bConquest)
	{
		...
		{
			//Rhye - start UP (Babylonian/Khwarezmian)
...
void CvPlayer::doTurn()
{
	...
	if (isTurnPlayed()) return; // Rhye / edead
...
/************************************************************************************************/
   ...
   case TRADE_RELIC:
		...
		if (pUnitTraded != NULL)
		{
			if (GC.getUnitInfo(pUnitTraded->getUnitType()).isRelic() && (pTheirCapitalCity != NULL))
			{
				return true;
			}
		}
...
DenialTypes CvPlayer::getTradeDenial(PlayerTypes eWhoTo, TradeData item) const
{
	...
	case TRADE_RELIC:
		...
		if (pUnit != NULL)
		{
			return AI_relicTrade(pUnit, eWhoTo);
		}
		break;
	// edead: end
...
void CvPlayer::found(int iX, int iY)
{
	...
	for (iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
	{
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			...
			{
				// edead: start - FreeStartEra buildings are now available to late start civs
...
bool CvPlayer::canTrain(UnitTypes eUnit, bool bContinue, bool bTestVisible, bool bIgnoreCost) const
{
	...
	if (GC.getUnitInfo(eUnit).getStateReligion() != NO_RELIGION)
	{
		...
		{
			//edead: start
			if (GC.getUnitInfo(eUnit).getOrStateReligion() != NO_RELIGION)
			{
				if (getStateReligion() != GC.getUnitInfo(eUnit).getOrStateReligion())
				{
					return false;
				}
			}
...
bool CvPlayer::canConstruct(BuildingTypes eBuilding, bool bContinue, bool bTestVisible, bool bIgnoreCost) const
{
	...
	// edead: start
	if ((getID() == PORTUGAL) && (eBuilding >= GREAT_OBSERVATORY))
	{
		return false;
	}
	// edead: end
	...
	if (GC.getBuildingInfo(eBuilding).getMaxStartEra() != NO_ERA)
	{
		...
		if (GC.getGameINLINE().getCurrentEra() > GC.getBuildingInfo(eBuilding).getMaxStartEra() || getCurrentEra() > GC.getBuildingInfo(eBuilding).getMaxStartEra()) // edead
		{
			return false;
		}
	}
	...
	// edead: Mughal & Safavid UHV fix
	if (!isHuman()) 
	{
		if (eBuilding == SHALIMAR_GARDENS || eBuilding == TAJ_MAHAL || eBuilding == RED_FORT) {
			if (GET_PLAYER(MUGHALS).isHuman()) {
				if (GC.getGameINLINE().getGameTurn() < getTurnForYear(GET_PLAYER(MUGHALS).getStartYear()+25)) {
					return false;
				}
			}
			else if (GET_PLAYER(SAFAVIDS).isHuman()) {
				if (GC.getGameINLINE().getGameTurn() < getTurnForYear(GET_PLAYER(SAFAVIDS).getStartYear()+25)) {
					return false;
				}
			}
		}
			...
			if (GET_PLAYER(GHORIDS).isHuman()) {
				if (GC.getGameINLINE().getGameTurn() < getTurnForYear(GET_PLAYER(GHORIDS).getStartYear()+25)) {
					return false;
				}
			}
		}
...
int CvPlayer::getProductionNeeded(UnitTypes eUnit) const
{
	...
	// edead: start
	iProductionNeeded *= getProductionPercent();
...
int CvPlayer::getProductionModifier(UnitTypes eUnit) const
{
	...
	// edead: start UP: Ghazis
	if (getID() == TURKS)
	{
		if ((GC.getUnitInfo(eUnit).getDomainType() == DOMAIN_LAND) && (GC.getUnitInfo(eUnit).isMilitaryProduction()))
		{
			iMultiplier += 25;
		}
	}
...
int CvPlayer::getImprovementUpgradeRate() const
{
	...
	//iRate = 1; // XXX
	iRate = 100; // edead (allow negative bonus to work properly; related changes are in CvPlot::doImprovementUpgrade())

	if (isBarbarian() || isMinorCiv()) iRate = 50; // edead: handicap for minor civs
...
int CvPlayer::calculateUnitCost(int& iFreeUnits, int& iFreeMilitaryUnits, int& iPaidUnits, int& iPaidMilitaryUnits, int& iBaseUnitCost, int& iMilitaryCost, int& iExtraCost) const
{
	...
	// edead: stop minor civs from disbanding most units
	if (isMinorCiv())
	{
		iFreeMilitaryUnits += getNumCities();
	}
	// edead: end
	...
	// edead: extra free upkeep point for AI with 1 city to help with spawns
	if (!isHuman() && (getNumCities() <= 1) && (iSupport > 0))
		iSupport--;
	// edead: end
...
int CvPlayer::greatPeopleThreshold(bool bMilitary) const
{
	...
	iThreshold *= getGreatPeoplePercent();
	iThreshold /= 100;
	// edead: end
...
{
	if (getID() != AKKOYUNLU && getStateReligion() != RELIGION_SUNNI) return 0; // edead: Prophet's Mosque hack
...
{
	// edead: start UP: Philosophy
	if ((getID() == ABBASIDS) && (getCurrentEra() < 2)) {
		return (m_iGreatPeopleRateModifier + 100);
	}
	// edead: end
...
{
	// edead: start UP: devshirme
	if (getID() == OTTOMANS) {
		return (m_iMaxConscript + 3);
	}
	// edead: end
...
// edead - start general civic happiness for Slavery etc.
...
// edead - start bonus to Diplomacy for the new Diplomatic trait

int CvPlayer::getDiplomacyModifier() const
{
	return m_iDiplomacyModifier;
}


void CvPlayer::changeDiplomacyModifier(int iChange)
{
	if (iChange != 0)
	{
		m_iDiplomacyModifier = (m_iDiplomacyModifier + iChange);
	}
}

// edead - AI luck - for AI "cheating" in combat (AI vs AI combat only!)

int CvPlayer::getLuck() const
{
	return m_iLuck;
}


void CvPlayer::setLuck(int iNewValue)
{
	m_iLuck = iNewValue;
}
...
int CvPlayer::getStartYear() const
{
	return m_iStartYear;
}

void CvPlayer::setStartYear(int iNewValue)
{
...
void CvPlayer::changeInflationModifier(int iChange)
{
	if (iChange != 0)
	{
		m_iInflationModifier = (m_iInflationModifier + iChange);
	}
}

int CvPlayer::getGrowthPercent() const
{
	return m_iGrowthPercent;
}

void CvPlayer::setGrowthPercent(int iNewValue)
{
	m_iGrowthPercent = iNewValue;
}

int CvPlayer::getProductionPercent() const
{
	return m_iProductionPercent;
}

void CvPlayer::setProductionPercent(int iNewValue)
{
	m_iProductionPercent = iNewValue;
}

int CvPlayer::getResearchPercent() const
{
	return m_iResearchPercent;
}

void CvPlayer::setResearchPercent(int iNewValue)
{
	m_iResearchPercent = iNewValue;
}

int CvPlayer::getCulturePercent() const
{
	return m_iCulturePercent;
}

void CvPlayer::setCulturePercent(int iNewValue)
{
	m_iCulturePercent = iNewValue;
}

int CvPlayer::getEspionagePercent() const
{
	return m_iEspionagePercent;
}

void CvPlayer::setEspionagePercent(int iNewValue)
{
	m_iEspionagePercent = iNewValue;
}

int CvPlayer::getGreatPeoplePercent() const
{
	return m_iGreatPeoplePercent;
}

void CvPlayer::setGreatPeoplePercent(int iNewValue)
{
	m_iGreatPeoplePercent = iNewValue;
}

bool CvPlayer::isTurnPlayed() const
{
	return m_bTurnPlayed;
}

void CvPlayer::setTurnPlayed(bool bNewValue)
{
	m_bTurnPlayed = bNewValue;
}
		...
		else
		{
			...
			if (GC.getGameINLINE().getElapsedGameTurns() > 0)
			{
				...
				if (!isBarbarian() && !isMinorCiv()) //Rhye
				{
					...
					szBuffer = gDLL->getText("TXT_KEY_MISC_CIV_DESTROYED", getCivilizationDescriptionKey()); // edead
...
void CvPlayer::verifyAlive()
{
	...
	if (isAlive())
	{
		if ((getID() == PORTUGAL) && (GC.getGameINLINE().getGameTurn() >= getTurnForYear(GET_PLAYER(PORTUGAL).getStartYear()))) return; // edead: Portugal can't die
		...
		if (!bKill)
		{
			if (!isBarbarian())
			{
				if (getNumCities() == 0 && getAdvancedStartPoints() < 0)
				{
					...
					if ((getNumUnits() == 0) || (isFoundedFirstCity() && (GC.getGameINLINE().getGameTurn() > getTurnForYear(getStartYear()) + 10))) // edead
					{
						bKill = true;
					}
				}
...
LeaderHeadTypes CvPlayer::getLeaderType() const
{
	...
	return GC.getInitCore().getLeader((PlayerTypes)getID()); // edead
}
...
void CvPlayer::setLastStateReligion(ReligionTypes eNewValue)
{
	...
	if (getLastStateReligion() != eNewValue)
	{
		...
		if (GC.getGameINLINE().isFinalInitialized())
		{
			...
			if (!isBarbarian() && !isMinorCiv()) //Rhye
			{
				if (getLastStateReligion() != NO_RELIGION) // edead
				{
					for (iI = 0; iI < MAX_PLAYERS; iI++)
					{
						if (GET_PLAYER((PlayerTypes)iI).isAlive())
						{
							if (GET_TEAM(getTeam()).isHasMet(GET_PLAYER((PlayerTypes)iI).getTeam()))
							{
								//szBuffer = gDLL->getText("TXT_KEY_MISC_PLAYER_CONVERT_RELIGION", getNameKey(), GC.getReligionInfo(getLastStateReligion()).getTextKeyWide()); // edead
								szBuffer = gDLL->getText("TXT_KEY_MISC_PLAYER_CONVERT_RELIGION", getCivilizationDescriptionKey(), GC.getReligionInfo(getLastStateReligion()).getTextKeyWide()); // edead
...
void CvPlayer::doGold()
{
	...
	//edead: start comment (moved to calculateGoldRate())
	// CyArgsList argsList;
	// argsList.add(getID());
	// long lResult=0;
	// gDLL->getPythonIFace()->callFunction(PYGameModule, "doGold", argsList.makeFunctionArgs(), &lResult);
...
bool CvPlayer::canDoEspionageMission(EspionageMissionTypes eMission, PlayerTypes eTargetPlayer, const CvPlot* pPlot, int iExtraData, const CvUnit* pUnit) const
{
	...
	// edead: start: Assassination of Heir only in the Capital / Summer Palace
	if (GC.getEspionageMissionInfo(eMission).getPlayerAnarchyCounter() > 0 && !pPlot->getPlotCity()->isGovernmentCenter()) 
	{
		return false;
	}
	// edead:
...
int CvPlayer::getEspionageMissionBaseCost(EspionageMissionTypes eMission, PlayerTypes eTargetPlayer, const CvPlot* pPlot, int iExtraData, const CvUnit* pSpyUnit) const
{
	...
	else if (kMission.getDestroyUnitCostFactor() > 0)
	{
			...
			if (NULL != pPlot)
			{
				...
				while (pUnitNode != NULL)
				{
					...
					if (canSpyDestroyUnit(eTargetPlayer, *pLoopUnit))
					{
						...
						if (GC.getUnitInfo(pLoopUnit->getUnitType()).isGoldenAge() || GC.getUnitInfo(pLoopUnit->getUnitType()).getLeaderPromotion() != NO_PROMOTION) iValue = GC.getDefineINT("GREAT_PEOPLE_ASSASSINATION_COST"); // edead: SuperSpies - for Great People killing
						if (iValue < iCost)
						{
							iCost = iValue;
							pUnit = pLoopUnit;
		...
		{
			// edead: start SuperSpies assassination
			if (canSpyDestroyUnit(eTargetPlayer, *pUnit)) 
			{
				iCost = getProductionNeeded(pUnit->getUnitType());
				if (GC.getUnitInfo(pUnit->getUnitType()).isGoldenAge() || GC.getUnitInfo(pUnit->getUnitType()).getLeaderPromotion() != NO_PROMOTION) iCost = GC.getDefineINT("GREAT_PEOPLE_ASSASSINATION_COST"); // Great People
			}
			else 
			{
				return -1;
		...
		if (NO_BUILDING != eBuilding)
		{
			if (NULL != pCity && pCity->getNumRealBuilding(eBuilding) > 0)
			{
				if (canSpyDestroyBuilding(eTargetPlayer, eBuilding))
				{
					...
					if (NULL != pSpyUnit)
					{
						iMissionCost *= 100 - pSpyUnit->getExtraCityAttackPercent();
						iMissionCost /= 100;
					}
					// edead: end
				}
	...
	else if (kMission.getPlayerAnarchyCounter() > 0)
	{
		...
		// edead: start
		iMissionCost *= 100 + 5 * (GET_PLAYER(eTargetPlayer).getNumCities() - 1);
		iMissionCost /= 100;
		// edead: end
	}
	...
	// edead: start
	iMissionCost *= getEspionagePercent();
	iMissionCost /= 100;
	// edead: end
...
bool CvPlayer::doEspionageMission(EspionageMissionTypes eMission, PlayerTypes eTargetPlayer, CvPlot* pPlot, int iExtraData, CvUnit* pSpyUnit)
{
	...
	if (kMission.getPlayerAnarchyCounter() > 0)
	{
		if (NO_PLAYER != eTargetPlayer)
		{
			...
			GET_PLAYER(eTargetPlayer).changeAnarchyTurns(1 + GC.getGameINLINE().getSorenRandNum(iTurns, "Random Anarchy Length")); // edead
	...
	if (kMission.getCounterespionageNumTurns() > 0 && kMission.getCounterespionageMod() > 0)
	{
		...
		if (NO_TEAM != eTargetTeam)
		{
			...
			GET_TEAM(getTeam()).changeCounterespionageModAgainstTeam(eTargetTeam, (kMission.getCounterespionageMod() + (5 * pSpyUnit->getExtraCityDefensePercent()))); // edead
...
void CvPlayer::processCivics(CivicTypes eCivic, int iChange)
{
	...
	// edead - start
	changeCivicHappiness(GC.getCivicInfo(eCivic).getCivicHappiness() * iChange); 
...
//Rhye - start
// edead: changed Rhye's x,y loop to plotByIndex loop
	...
	for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if (pLoopPlot != NULL)
		{
			if (canFound(pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE()))
			{
				...
				{
					if (pLoopPlot->getFoundModifier(getID()) >= threshold)
					{
						//count++;
						if (pLoopPlot->isRevealed(getTeam(), false))
						{
...
// edead - changes the flag and updates graphical data
...
// edead / jdog / rhye - updates the leader & traits
...
void CvPlayer::read(FDataStreamBase* pStream)
{
	...
	pStream->Read(&m_iDiplomacyModifier);
	pStream->Read(&m_iLuck);
	pStream->Read(&m_iStartYear);
	pStream->Read(&m_iGrowthPercent);
	pStream->Read(&m_iProductionPercent);
	pStream->Read(&m_iResearchPercent);
	pStream->Read(&m_iCulturePercent);
	pStream->Read(&m_iEspionagePercent);
	pStream->Read(&m_iGreatPeoplePercent);
	pStream->Read(GC.getNumReligionInfos(), m_paiReligionSpreadPercent);
	pStream->Read(MAX_PLAYERS, m_aiBorders);
	pStream->Read(MAX_PLAYERS, m_aiAttitudeModifier);
	pStream->Read(&m_bTurnPlayed);
	// edead - end
...
void CvPlayer::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(m_iDiplomacyModifier);
	pStream->Write(m_iLuck);
	pStream->Write(m_iStartYear);
	pStream->Write(m_iGrowthPercent);
	pStream->Write(m_iProductionPercent);
	pStream->Write(m_iResearchPercent);
	pStream->Write(m_iCulturePercent);
	pStream->Write(m_iEspionagePercent);
	pStream->Write(m_iGreatPeoplePercent);
	pStream->Write(GC.getNumReligionInfos(), m_paiReligionSpreadPercent);
	pStream->Write(MAX_PLAYERS, m_aiBorders);
	pStream->Write(MAX_PLAYERS, m_aiAttitudeModifier);
	pStream->Write(m_bTurnPlayed);
	// edead - end
...
void CvPlayer::createGreatPeople(UnitTypes eGreatPersonUnit, bool bIncrementThreshold, bool bIncrementExperience, int iX, int iY)
{
	...
	if (pPlot)
	{
		if (pCity)
		{
			//CvWString szCity; // edead
			//szCity.Format(L"%s (%s)", pCity->getName().GetCString(), GET_PLAYER(pCity->getOwnerINLINE()).getName()); // edead
			//szReplayMessage = gDLL->getText("TXT_KEY_MISC_GP_BORN", pGreatPeopleUnit->getName().GetCString(), szCity.GetCString()); // edead
			szReplayMessage = gDLL->getText("TXT_KEY_MISC_GP_BORN", pGreatPeopleUnit->getName().GetCString(), pCity->getName().GetCString()); // edead
		}
		else
		{
...
EventTriggeredData* CvPlayer::initTriggeredData(EventTriggerTypes eEventTrigger, bool bFire, int iCityId, int iPlotX, int iPlotY, PlayerTypes eOtherPlayer, int iOtherPlayerCityId, ReligionTypes eReligion, CorporationTypes eCorporation, int iUnitId, BuildingTypes eBuilding, CvWString szCustomText) // edead
{
	...
	if (aszTexts.size() > 0)
	{
		...
		pTriggerData->m_szText = gDLL->getText(aszTexts[iText].GetCString(), 
			...
			,szCustomText.GetCString() // edead: custom global text %s12_city
			);
	...
	if (kTrigger.getNumWorldNews() > 0)
	{
		...
		pTriggerData->m_szGlobalText = gDLL->getText(kTrigger.getWorldNews(iText).GetCString(),
			...
			,szCustomText.GetCString() // edead: custom global text %s7_city
			);
	}
	else
	{
...
bool CvPlayer::isValidEventTech(TechTypes eTech, EventTypes eEvent, PlayerTypes eOtherPlayer) const
{
	...
	if (!canResearch(eTech) && (NO_TECH == kEvent.getTech())) // edead: start (enable events to grant any tech)
	{
		return false;
	}
	...
	if (NO_PLAYER != eOtherPlayer && !GET_TEAM(GET_PLAYER(eOtherPlayer).getTeam()).isHasTech(eTech) && !GET_PLAYER(eOtherPlayer).isMinorCiv()) // edead: fix for triggers w/ rebels
	{
		return false;
	}
...
bool CvPlayer::canTrigger(EventTriggerTypes eTrigger, PlayerTypes ePlayer, ReligionTypes eReligion) const
{
	...
	// edead: start - don't trigger minors civs
...
bool CvPlayer::splitEmpire(int iAreaId)
{
	...
	//if (!bPlayerExists)
	if (eNewPlayer >= INDEPENDENT && eNewPlayer <= INDEPENDENT4)
	//Rhye - end
	{
		//Rhye - start comment
		/*
...
bool CvPlayer::canSpyDestroyUnit(PlayerTypes eTarget, CvUnit& kUnit) const
{
	...
	// edead: SuperSpies start - only assassinate special units, excl. other spies
	if (GC.getUnitInfo(kUnit.getUnitType()).getSpecialUnitType() == NO_SPECIALUNIT || kUnit.isSpy())
	{
		return false;
	}
	// edead: SuperSpies end
	...
	for (int i = NUM_BASIC_ITEMS; i < NUM_TRADEABLE_HEADINGS; i++)
	{
		...
		case TRADE_PEACE:
			if (!isHuman())
			{
				...
				for (int j = 0; j < NUM_MAJOR_PLAYERS; j++) // edead
				{
...
bool CvPlayer::getHeadingTradeString(PlayerTypes eOtherPlayer, TradeableItems eItem, CvWString& szString, CvString& szIcon) const
{
	...
	// edead: start Relic trade
	case TRADE_RELIC:
        szString = gDLL->getText("TXT_KEY_TRADE_RELIC");
        break;
	// edead: end
...
// edead: start
int CvPlayer::getBorders(PlayerTypes ePlayer) const
{
	if (ePlayer < 0 || ePlayer >= NUM_MAJOR_PLAYERS) 
	{
		return -1;
	}
	
	return m_aiBorders[ePlayer];
}

void CvPlayer::setBorders(PlayerTypes ePlayer, int iNewValue)
{
	FAssertMsg(ePlayer >= 0, "ePlayer is expected to be non-negative (invalid Index)");
	FAssertMsg(ePlayer < MAX_PLAYERS, "ePlayer is expected to be within maximum bounds (invalid Index)");
	m_aiBorders[ePlayer] = iNewValue;
}

int CvPlayer::getAttitudeModifier(PlayerTypes ePlayer) const
{
	if (ePlayer < 0 || ePlayer >= NUM_MAJOR_PLAYERS) 
	{
		return -1;
	...
	return m_aiAttitudeModifier[ePlayer];
}

void CvPlayer::setAttitudeModifier(PlayerTypes ePlayer, int iNewValue)
{
	FAssertMsg(ePlayer >= 0, "ePlayer is expected to be non-negative (invalid Index)");
	FAssertMsg(ePlayer < MAX_PLAYERS, "ePlayer is expected to be within maximum bounds (invalid Index)");
	m_aiAttitudeModifier[ePlayer] = iNewValue;
}

int CvPlayer::getReligionSpreadPercent(ReligionTypes eIndex) const
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < GC.getNumReligionInfos(), "eIndex is expected to be within maximum bounds (invalid Index)");
	return m_paiReligionSpreadPercent[eIndex];
}

void CvPlayer::setReligionSpreadPercent(ReligionTypes eIndex, int iNewValue)
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < GC.getNumReligionInfos(), "eIndex is expected to be within maximum bounds (invalid Index)");
	m_paiReligionSpreadPercent[eIndex] = iNewValue;
}

// Relic trade based on Afforess' Advanced Diplomacy
DenialTypes CvPlayer::AI_relicTrade(CvUnit* pUnit, PlayerTypes ePlayer) const
{
...
// Civ & Religion specific soundtracks
EraTypes CvPlayer::getSoundtrackEra() const
{
	switch (getID())
	{
		case BYZANTIUM:
		...
		case OUTREMER:
			...
			{
				return ERA_CHRISTIAN_EARLY;
			}
			else
			{
				return ERA_CHRISTIAN_LATE;
			}
			break;
		...
		case MUGHALS:
			if (getCurrentEra() <= ERA_CLASSICAL)
			{
				return ERA_INDIA_EARLY;
			}
			else
			{
				return ERA_INDIA_LATE;
			}
			break;
