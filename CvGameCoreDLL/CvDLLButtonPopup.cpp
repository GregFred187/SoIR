	...
	case BUTTONPOPUP_RAZECITY:
		...
		else if (pPopupReturn->getButtonClicked() == 4)
		{
			...
			if (NULL != pCity)
			{
				...
				CvEventReporter::getInstance().cityAcquiredAndKept(GC.getGameINLINE().getActivePlayer(), pCity, true);
			}
		}
		// edead: end
...
bool CvDLLButtonPopup::launchRazeCityPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	...
	// edead: start massacre of population part I
	bool bMassacre = false;
	if (pNewCity->getReligionCount() > 0)
	{
		if ((player.getStateReligion() == NO_RELIGION) || (!pNewCity->isHasReligion(player.getStateReligion())) || (pNewCity->getReligionCount() > 1))
		{
			bMassacre = true;
			if (player.getStateReligion() == RELIGION_ORTHODOXY)
			{
				if ((pNewCity->getReligionCount() == 1) && (pNewCity->isHasReligion(RELIGION_CATHOLICISM)))
				{
					bMassacre = false;
				}
				if ((pNewCity->getReligionCount() == 2) && (pNewCity->isHasReligion(RELIGION_CATHOLICISM)) && (pNewCity->isHasReligion(RELIGION_ORTHODOXY)))
				{
					bMassacre = false;
				}
			}
			else if (player.getStateReligion() == RELIGION_CATHOLICISM)
			{
				if ((pNewCity->getReligionCount() == 1) && (pNewCity->isHasReligion(RELIGION_ORTHODOXY)))
				{
					bMassacre = false;
				}
				if ((pNewCity->getReligionCount() == 2) && (pNewCity->isHasReligion(RELIGION_CATHOLICISM)) && (pNewCity->isHasReligion(RELIGION_ORTHODOXY)))
	...
	// edead: start massacre of population part II
	if (bMassacre)
	{
		gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_EXTERMINATE").c_str(), NULL, 4, WIDGET_GENERAL);
	}
	// edead: end
	...
	else if (kMission.getDestroyUnitCostFactor() > 0)
	{
		...
		while (pUnitNode != NULL)
		{
			...
			if (NULL != pLoopUnit)
			{
				...
				if (kPlayer.canDoEspionageMission(eMission, eTargetPlayer, pPlot, pLoopUnit->getID(), pUnit)) // edead
				{
					if (pLoopUnit->getTeam() == GET_PLAYER(eTargetPlayer).getTeam())
					{
						...
						int iCost = kPlayer.getEspionageMissionCost(eMission, eTargetPlayer, pPlot, pLoopUnit->getID(), pUnit); // edead
						...
						gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, szBuffer, pLoopUnit->getButton(), pLoopUnit->getID(), WIDGET_HELP_ESPIONAGE_COST, eMission, pLoopUnit->getID()); // edead
					}
				}
			}
		}
