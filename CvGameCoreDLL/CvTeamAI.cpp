...
DenialTypes CvTeamAI::AI_techTrade(TechTypes eTech, TeamTypes eTeam) const
{
	...
	if (GC.getGameINLINE().getGameTurn() <= getTurnForYear(GET_PLAYER((PlayerTypes)getID()).getStartYear())+2) // edead
	{
		return DENIAL_TECH_MONOPOLY;
	}
	//if (GC.getGameINLINE().getGameTurn() <= startingTurn[eTeam]+2)
	if (GC.getGameINLINE().getGameTurn() <= getTurnForYear(GET_PLAYER((PlayerTypes)eTeam).getStartYear())+2) // edead
	{
...
DenialTypes CvTeamAI::AI_vassalTrade(TeamTypes eTeam) const
{
	...
	for (int iLoopTeam = 0; iLoopTeam < NUM_MAJOR_PLAYERS; iLoopTeam++) // edead: ignore minor players
	{
...
DenialTypes CvTeamAI::AI_surrenderTrade(TeamTypes eTeam, int iPowerMultiplier) const
{
	...
	// edead: start
	if (getID() == PORTUGAL || getID() == BYZANTIUM)
	{
		return DENIAL_JOKING;
	}
	// edead: end
	
	// edead: start - no vassal trading before city flipping takes place, since it's essentially an exploit
	if (GC.getGameINLINE().getGameTurn() <= getTurnForYear(GET_PLAYER((PlayerTypes)getID()).getStartYear())+2)
	{
		return DENIAL_JOKING;
	}
	// edead: end
...
/************************************************************************************************/
	...
	if (!GET_TEAM(eTeam).isParent(getID()))
	{
		...
		for (std::vector<PlayerTypes>::const_iterator iter = m_aePlayerMembers.begin(); iter != m_aePlayerMembers.end(); ++iter)
		{
			// edead: start Crusaders vassalage fix
			if ((int(*iter) == ANTIOCH) || (int(*iter) == OUTREMER)) 
			{
				if (GET_PLAYER(kMasterTeam.getLeaderID()).getStateReligion() == RELIGION_CATHOLICISM)
					iPersonalityModifier -= 5;
				else if (GET_PLAYER(kMasterTeam.getLeaderID()).getStateReligion() == RELIGION_ORTHODOXY)
					iPersonalityModifier += GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getVassalPowerModifier()/3;
				else
		...
		for (int iLoopTeam = 0; iLoopTeam < MAX_CIV_TEAMS; iLoopTeam++)
		{
			if (iLoopTeam != getID())
			{
				...
				if (kLoopTeam.isAlive() && !kLoopTeam.isMinorCiv()) // edead: exclude independents
				{
...
/************************************************************************************************/
	...
	if (!isAtWar(eTeam))
	{
		if (!GET_TEAM(eTeam).isParent(getID()))
		{
			...
			if (GET_PLAYER(kMasterTeam.getLeaderID()).getBorders(getLeaderID()) >= 5)
			{
				return DENIAL_TOO_FAR;
			}
			// edead: end
