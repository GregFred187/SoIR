					...
					if (GET_PLAYER((PlayerTypes)iJ).isAlive())
					{
						if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
						{
							...
							else if (GET_PLAYER((PlayerTypes)iJ).getTeam() != getID())
							{
								...
								{
									// edead: start Independent friend fix - just skip them
									//if (GET_TEAM(GET_PLAYER((PlayerTypes)iJ).getTeam()).AI_getAttitude(eTeam) >= ATTITUDE_PLEASED)
									if (!GET_TEAM(eTeam).isMinorCiv() && GET_TEAM(GET_PLAYER((PlayerTypes)iJ).getTeam()).AI_getAttitude(eTeam) >= ATTITUDE_PLEASED)
									// edead: end
									{
				  ...
				  !isMinorCiv() && !(GET_TEAM(eTeam).isMinorCiv()))
			{
				for (iI = 0; iI < MAX_PLAYERS; iI++)
				{
					if (GET_PLAYER((PlayerTypes)iI).isAlive())
					{
						if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
						{
							//szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_DECLARED_WAR_ON", GET_TEAM(eTeam).getName().GetCString()); // edead
							szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_DECLARED_WAR_ON", GET_PLAYER((PlayerTypes)eTeam).getCivilizationDescription()); // edead
						...
						{
							//szBuffer = gDLL->getText("TXT_KEY_MISC_DECLARED_WAR_ON_YOU", getName().GetCString()); // edead
							szBuffer = gDLL->getText("TXT_KEY_MISC_DECLARED_WAR_ON_YOU", GET_PLAYER((PlayerTypes)getID()).getCivilizationDescription()); // edead
							gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_DECLAREWAR", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"));
						}
						...
						{
							//szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_DECLARED_WAR", getName().GetCString(), GET_TEAM(eTeam).getName().GetCString()); // edead
							szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_DECLARED_WAR", GET_PLAYER((PlayerTypes)getID()).getCivilizationDescription(), GET_PLAYER((PlayerTypes)eTeam).getCivilizationDescription()); // edead
				...
				//szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_DECLARES_WAR", getName().GetCString(), GET_TEAM(eTeam).getName().GetCString()); // edead
				szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_DECLARES_WAR", GET_PLAYER((PlayerTypes)getID()).getCivilizationDescription(), GET_PLAYER((PlayerTypes)eTeam).getCivilizationDescription()); // edead
...
/************************************************************************************************/
		...
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (GET_PLAYER((PlayerTypes)iI).isAlive())
			{
				if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
				{
					//szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_MADE_PEACE_WITH", GET_TEAM(eTeam).getName().GetCString()); // edead
					szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_MADE_PEACE_WITH", GET_PLAYER((PlayerTypes)eTeam).getCivilizationDescription()); // edead
				...
				{
					//szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_MADE_PEACE_WITH", getName().GetCString()); // edead
					szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_MADE_PEACE_WITH", GET_PLAYER((PlayerTypes)getID()).getCivilizationDescription()); // edead
					gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_MAKEPEACE", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
				}
				else if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(getID()) && GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(eTeam))
				{
					//szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_MADE_PEACE", getName().GetCString(), GET_TEAM(eTeam).getName().GetCString()); // edead
					szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_MADE_PEACE", GET_PLAYER((PlayerTypes)getID()).getCivilizationDescription(), GET_PLAYER((PlayerTypes)eTeam).getCivilizationDescription()); // edead
...
int CvTeam::getResearchCost(TechTypes eTech) const
{
	...
	// edead: start
	iCost *= GET_PLAYER((PlayerTypes)getID()).getResearchPercent();
	iCost /= 100;
	// edead: end
	...
	int iMultiplier = 4; // 4-4-3-3
	if (GET_PLAYER((PlayerTypes)getID()).isHuman())
		iMultiplier = 7; // 7-7-6-5
	if (GET_PLAYER((PlayerTypes)getID()).getCurrentEra() == 2)
		iMultiplier -= 1;
	else if (GET_PLAYER((PlayerTypes)getID()).getCurrentEra() > 2)
	{
		iMultiplier *= 3;
		iMultiplier /= 4;
	}
	...
	if (getID() < NUM_MAJOR_PLAYERS) {		
		if (iNumCities < 5)
		{
			iMultiplier = 3*(int)GET_PLAYER((PlayerTypes)getID()).getCurrentEra(); //x-3-6-9
			if (iMultiplier > 0) {
		...
		int iTurnModifier = 10*(int)GET_PLAYER((PlayerTypes)getID()).getCurrentEra(); //0-10-20-30
		...
		if (GC.getGameINLINE().getGameTurn() <= getTurnForYear(GET_PLAYER((PlayerTypes)getID()).getStartYear()) + iTurnModifier) { // edead
			...
			int iAmount = (getTurnForYear(GET_PLAYER((PlayerTypes)getID()).getStartYear()) + iTurnModifier) - GC.getGameINLINE().getGameTurn();
			iAmount *= 100;
...
CvWString CvTeam::getName() const
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID()) // edead: fix for names of dead players
		{
...
void CvTeam::setDefensivePact(TeamTypes eIndex, bool bNewValue)
{
	...
	if (isDefensivePact(eIndex) != bNewValue)
	{
		...
		{
			//CvWString szBuffer = gDLL->getText("TXT_KEY_MISC_PLAYERS_SIGN_DEFENSIVE_PACT", getName().GetCString(), GET_TEAM(eIndex).getName().GetCString()); // edead
			CvWString szBuffer = gDLL->getText("TXT_KEY_MISC_PLAYERS_SIGN_DEFENSIVE_PACT", GET_PLAYER((PlayerTypes)getID()).getCivilizationDescription(), GET_PLAYER((PlayerTypes)eIndex).getCivilizationDescription()); // edead
			...
			for (int iI = 0; iI < MAX_PLAYERS; iI++)
			{
				...
				if (kPlayer.isAlive())
				{
					...
					{
						gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_WELOVEKING", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
					}
				}
			}
		}
...
void CvTeam::setVassal(TeamTypes eIndex, bool bNewValue, bool bCapitulated)
{
	...
	// edead: start possible bugfix for vassals being at war with their masters
	if (bNewValue && !bCapitulated && isAtWar(eIndex))
		return;
	// edead: end
	...
	if (isVassal(eIndex) != bNewValue)
	{
		...
		if (isVassal(eIndex))
		{
			...
			if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
			{
				...
				if (bCapitulated)
				{
					//szReplayMessage = gDLL->getText("TXT_KEY_MISC_CAPITULATE_AGREEMENT", getName().GetCString(), GET_TEAM(eIndex).getName().GetCString()); // edead
					szReplayMessage = gDLL->getText("TXT_KEY_MISC_CAPITULATE_AGREEMENT", GET_PLAYER((PlayerTypes)getID()).getCivilizationDescription(), GET_PLAYER((PlayerTypes)eIndex).getCivilizationDescription()); // edead
				}
				else
				{
					//szReplayMessage = gDLL->getText("TXT_KEY_MISC_VASSAL_AGREEMENT", getName().GetCString(), GET_TEAM(eIndex).getName().GetCString()); // edead
					szReplayMessage = gDLL->getText("TXT_KEY_MISC_VASSAL_AGREEMENT", GET_PLAYER((PlayerTypes)getID()).getCivilizationDescription(), GET_PLAYER((PlayerTypes)eIndex).getCivilizationDescription()); // edead
				}
		...
		else
		{
			...
			if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()) && isAlive() && GET_TEAM(eIndex).isAlive())
			{
				...
				if (m_bCapitulated)
				{
					//szReplayMessage = gDLL->getText("TXT_KEY_MISC_SURRENDER_REVOLT", getName().GetCString(), GET_TEAM(eIndex).getName().GetCString()); // edead
					szReplayMessage = gDLL->getText("TXT_KEY_MISC_SURRENDER_REVOLT", GET_PLAYER((PlayerTypes)getID()).getCivilizationDescription(), GET_PLAYER((PlayerTypes)eIndex).getCivilizationDescription()); // edead
				}
				else
				{
					//szReplayMessage = gDLL->getText("TXT_KEY_MISC_VASSAL_REVOLT", getName().GetCString(), GET_TEAM(eIndex).getName().GetCString()); // edead
					szReplayMessage = gDLL->getText("TXT_KEY_MISC_VASSAL_REVOLT", GET_PLAYER((PlayerTypes)getID()).getCivilizationDescription(), GET_PLAYER((PlayerTypes)eIndex).getCivilizationDescription()); // edead
				}
	...
	if (iChange != 0)
	{
		...
		if (iChange > 0)
		{
			...
			if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
			{
				...
				if (getID() == ZENGIDS) szTemp = gDLL->getText("TXT_KEY_LEADER_ZENGI"); // edead: stupid hack
				if (getID() == BUYIDS) szTemp = gDLL->getText("TXT_KEY_LEADER_AL_DAWLA"); // edead: stupid hack
				szBuffer = gDLL->getText("TXT_KEY_MISC_COMPLETES_PROJECT", szTemp.GetCString(), GC.getProjectInfo(eIndex).getTextKeyWide()); // edead
				//szBuffer = gDLL->getText("TXT_KEY_MISC_COMPLETES_PROJECT", getName().GetCString(), GC.getProjectInfo(eIndex).getTextKeyWide());
				//szBuffer = gDLL->getText("TXT_KEY_MISC_COMPLETES_PROJECT", GET_PLAYER((PlayerTypes)getID()).getCivilizationDescription(), GC.getProjectInfo(eIndex).getTextKeyWide()); // edead
				GC.getGameINLINE().addReplayMessage(REPLAY_MESSAGE_MAJOR_EVENT, getLeaderID(), szBuffer, -1, -1, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));

				for (iI = 0; iI < MAX_PLAYERS; iI++)
				{
					//if (GET_PLAYER((PlayerTypes)iI).isAlive()) // edead
					if ((GET_PLAYER((PlayerTypes)iI).isAlive()) && (GC.getGameINLINE().getGameTurn() > 1) && (GC.getGameINLINE().getGameTurn() >= getTurnForYear(GET_PLAYER((PlayerTypes)iI).getStartYear()))) // edead
					{
						szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_HAS_COMPLETED", szTemp.GetCString(), GC.getProjectInfo(eIndex).getTextKeyWide()); // edead GC.getInitCore().getLeaderName((PlayerTypes)getID(), 0)
						//szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_HAS_COMPLETED", getName().GetCString(), GC.getProjectInfo(eIndex).getTextKeyWide());
						//szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_HAS_COMPLETED", GET_PLAYER((PlayerTypes)getID()).getCivilizationDescription(), GC.getProjectInfo(eIndex).getTextKeyWide()); // edead
						//gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_PROJECT_COMPLETED", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT")); // edead
						gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_FEAT_ACCOMPLISHED", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT")); // edead
						
					}
				}
			}
		}
		// edead: start project lost
		else
		{
			if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
			{
				for (iI = 0; iI < MAX_PLAYERS; iI++)
				{
					if (GET_PLAYER((PlayerTypes)iI).isAlive() && (GC.getGameINLINE().getGameTurn() >= getTurnForYear(GET_PLAYER((PlayerTypes)iI).getStartYear())))
					{
						szTemp = GC.getInitCore().getLeaderName((PlayerTypes)getID(), 0);
						if (getID() == ZENGIDS) szTemp = gDLL->getText("TXT_KEY_LEADER_ZENGI"); // edead: stupid hack
						if (getID() == BUYIDS) szTemp = gDLL->getText("TXT_KEY_LEADER_AL_DAWLA"); // edead: stupid hack
						szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_HAS_LOST_PROJECT", szTemp.GetCString(), GC.getProjectInfo(eIndex).getTextKeyWide());
						gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_DEAL_CANCELLED", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
					}
				}
			}
		}
					...
					if (GC.getTechInfo(eIndex).getFirstFreeTechs() > 0)
					{
						...
						for (iI = 0; iI < MAX_PLAYERS; iI++)
						{
							if (GET_PLAYER((PlayerTypes)iI).isAlive())
							{
								...
								{
									//szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_FIRST_TO_TECH", GET_PLAYER(ePlayer).getNameKey(), GC.getTechInfo(eIndex).getTextKeyWide()); // edead
									szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_FIRST_TO_TECH", GET_PLAYER(ePlayer).getCivilizationDescriptionKey(), GC.getTechInfo(eIndex).getTextKeyWide()); // edead
								}
								else
								{
		...
		if (GC.getDefineINT("CIRCUMNAVIGATE_FREE_MOVES") != 0)
		{
			...
			for (int iI = 0; iI < MAX_PLAYERS; iI++)
			{
				if (GET_PLAYER((PlayerTypes)iI).isAlive())
				{
					...
					{
						//szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_CIRC_GLOBE", getName().GetCString()); // edead
						szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_CIRC_GLOBE", GET_PLAYER((PlayerTypes)getID()).getCivilizationDescription()); // edead
					}
					else
					{
