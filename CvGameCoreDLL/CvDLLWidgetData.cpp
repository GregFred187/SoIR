...
void CvDLLWidgetData::parseHelp(CvWStringBuffer &szBuffer, CvWidgetDataStruct &widgetDataStruct)
{
	...
	// edead: start
	case WIDGET_PIETY_LEVEL:
		parsePietyLevelHelp(widgetDataStruct, szBuffer);
		break;
	// edead: end
...
void CvDLLWidgetData::parseContactCivHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	...
	if (GC.getGameINLINE().getActivePlayer() == widgetDataStruct.m_iData1)
	{
		...
		{
			if (GET_TEAM(GET_PLAYER((PlayerTypes)widgetDataStruct.m_iData1).getTeam()).getProjectCount((ProjectTypes)iI) > 0)
			{
				szBuffer.append(gDLL->getText("TXT_KEY_MISC_YOUR_TITLE", GC.getProjectInfo((ProjectTypes)iI).getDescription()));
				szBuffer.append(NEWLINE);
	...
	//szBuffer.append(NEWLINE); // edead
	//GAMETEXT.parsePlayerTraits(szBuffer, (PlayerTypes)widgetDataStruct.m_iData1); // edead
	...
	else
	{
		...
		for (iI = 0; iI < GC.getNumProjectInfos(); iI++)
		{
			...
			{
				szBuffer.append(NEWLINE);
				szBuffer.append(gDLL->getText("TXT_KEY_MISC_CONTACT_TITLE", GC.getProjectInfo((ProjectTypes)iI).getDescription()));
			}
		}
		// edead: end
		...
		if (!(GET_PLAYER((PlayerTypes)widgetDataStruct.m_iData1).isHuman()))
		{
			...
			// edead: start espionage tech research info (moved from CvMainInterface.py)
			bool bEspionageCanSeeResearch = false;
			int iI;
			for (iI = 0; iI < GC.getNumEspionageMissionInfos(); iI++)
			{
				if (GC.getEspionageMissionInfo((EspionageMissionTypes)iI).isSeeResearch())
				{
					bEspionageCanSeeResearch = GET_PLAYER(GC.getGameINLINE().getActivePlayer()).canDoEspionageMission((EspionageMissionTypes)iI, (PlayerTypes)widgetDataStruct.m_iData1, NULL, -1, NULL);
					break;
				}
			}
			
			if (((GET_PLAYER((PlayerTypes)widgetDataStruct.m_iData1).getTeam() == GC.getGameINLINE().getActiveTeam()) && (GET_TEAM(GC.getGameINLINE().getActiveTeam()).getNumMembers() > 1)) || (GET_TEAM((TeamTypes)widgetDataStruct.m_iData1).isVassal(GC.getGameINLINE().getActiveTeam())) || GC.getGameINLINE().isDebugMode() || bEspionageCanSeeResearch)
			{
				if (GET_PLAYER((PlayerTypes)widgetDataStruct.m_iData1).getCurrentResearch() != -1) 
				{
					szBuffer.append(gDLL->getText("TXT_KEY_MISC_RESEARCH_ESPIONAGE"));
					CvWString szTempBuffer = CvWString::format(L" %s (%d)", GC.getTechInfo(GET_PLAYER((PlayerTypes)widgetDataStruct.m_iData1).getCurrentResearch()).getDescription(), GET_PLAYER((PlayerTypes)widgetDataStruct.m_iData1).getResearchTurnsLeft(GET_PLAYER((PlayerTypes)widgetDataStruct.m_iData1).getCurrentResearch(), true));
					szBuffer.append(szTempBuffer);
					szBuffer.append(NEWLINE);
				}
			}
...
void CvDLLWidgetData::parseCompleteStabilityInfo(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer) 
{
	...
	// edead: start full display in cheat mode
	if (gDLL->getChtLvl() > 0)
	{
		CvWString szTempBuffer = CvWString::format(L" (%d)", iResult);
		szBuffer.append(szTempBuffer);
	}
	// edead: end
...
void CvDLLWidgetData::parseDescriptionHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer, bool bMinimal)
{
	...
	case CIVILOPEDIA_PAGE_NATIONAL_WONDER: // edead
	case CIVILOPEDIA_PAGE_RELIC: // edead
		{
...
// edead: start piety / favor level help
void CvDLLWidgetData::parsePietyLevelHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{

	szBuffer.append(gDLL->getText("TXT_KEY_MISC_PIETY_TITLE"));
	szBuffer.append(" ");

	switch (widgetDataStruct.m_iData1)
	{
	case 0:
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_HERETIC"));
		szBuffer.append(NEWLINE);
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_HERETIC_EFFECTS"));
		break;
	case 1:
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_APOSTATE"));
		szBuffer.append(NEWLINE);
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_APOSTATE_EFFECTS"));
		break;
	case 2:
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_HEATHEN"));
		szBuffer.append(NEWLINE);
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_HEATHEN_EFFECTS"));
		break;
	case 3:
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_UNRELIABLE"));
		szBuffer.append(NEWLINE);
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_UNRELIABLE_EFFECTS"));
		break;
	case 4:
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_BELIEVER"));
		szBuffer.append(NEWLINE);
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_BELIEVER_EFFECTS"));
		break;
	case 5:
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_FAITHFUL"));
		szBuffer.append(NEWLINE);
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_FAITHFUL_EFFECTS"));
		break;
	case 6:
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_RIGHTEOUS"));
		szBuffer.append(NEWLINE);
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_RIGHTEOUS_EFFECTS"));
		break;
	case 7:
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_DEVOTED"));
		szBuffer.append(NEWLINE);
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_DEVOTED_EFFECTS"));
		break;
	case 8:
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_BLESSED"));
		szBuffer.append(NEWLINE);
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_BLESSED_EFFECTS"));
		break;
	case 9:
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_SAINT"));
		szBuffer.append(NEWLINE);
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_SAINT_EFFECTS"));
		break;
	case 10:
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_ROYAL_SAINT"));
		szBuffer.append(NEWLINE);
		szBuffer.append(gDLL->getText("TXT_KEY_FAVOR_LEVEL_ROYAL_SAINT_EFFECTS"));
		break;
	}
}
// edead: end