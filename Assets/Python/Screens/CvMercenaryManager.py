...
class CvMercenaryManager:
	...
	def __init__(self, iScreenId):
		...
		# edead: ui fix for excessive padding
		self.iAvailableCount = 0
		self.iHiredCount = 0
...
##			g_bRequireCityUnitContractCreation = config.getboolean("Mercenaries Mod", "Require City Unit Contract Creation", true)
	...
	def populateAvailableMercenariesPanel(self, screen, availableCount=-1):
		
		# edead: UI fix
		for i in range(6):
			screen.deleteWidget("dummyPanelHire"+str(i))
		if AVAILABLE_MERCENARIES_INNER_PANEL_X not in self.screenWidgetData:
			self.calculateScreenWidgetData(screen)
		...
		for mercenaryName in mercenaries:
			...
			mercenaryName = mercenaryName.encode('ascii', 'xmlcharrefreplace')
            # mercenaryName = CyTranslator().getText( mercenaryName[1] , ())
            
			# Don't add the mercenary to the list if they were built by the current player
			if(mercenary.getBuilder() == iPlayer):
				continue
			
			# edead - regional recruitment
			...
			screen.attachLabel( mercenaryName+"Text", mercenaryName  + "text3", "     " + localText.getText("TXT_KEY_MERCENARY_SCREEN_LEVEL", (mercenary.getLevel(),)) )
			screen.attachLabel( mercenaryName+"Text", mercenaryName  + "text4", "     " + localText.getText("TXT_KEY_MERCENARY_SCREEN_COST", (strHCost,)) + "  " + localText.getText("TXT_KEY_MERCENARY_SCREEN_MAINTENANCE", (strMCost,)) )
			screen.attachLabel( mercenaryName+"Text", mercenaryName  + "text5", "     " + localText.getText("TXT_KEY_MERCENARY_SCREEN_LOCATION", (objMercenaryUtils.getMercenaryStartingCityName(iPlayer, mercenary),)) )
		...
		if availableCount == -1:
			self.iAvailableCount = mercenaryCount
		else:
			self.iAvailableCount = availableCount
		
		# the amount of blank entries depends on screen height
		iHeight = screen.getYResolution()
		iPadding = 4
		if iHeight >= 1000: iPadding = 5
		if iHeight >= 1200: iPadding = 6
		
		# apply padding to Available panel
		if (iPadding - self.iAvailableCount) > 0:
			for i in range(iPadding - self.iAvailableCount):
				...
				if iHeight > 768 or i > 0:
					screen.attachLabel( "dummyPanelHire"+str(i), "", "     ")
					screen.attachLabel( "dummyPanelHire"+str(i), "", "     ")
		
		sd.setVal('iAvailableCount', self.iAvailableCount)


	# Populates the panel that shows all of the players hired mercenaries
	def populateHiredMercenariesPanel(self, screen, hiredCount=-1):
		
		# edead: UI fix
		for i in range(6):
			screen.deleteWidget("dummyPanelFire"+str(i))
		if HIRED_MERCENARIES_INNER_PANEL_X not in self.screenWidgetData:
			self.calculateScreenWidgetData(screen)
		screen.deleteWidget(HIRED_MERCENARIES_INNER_PANEL_ID)
		...
		for mercenaryName in mercenaries:
			...
			screen.attachLabel( mercenaryName+"Text", mercenaryName  + "text3", "     " + localText.getText("TXT_KEY_MERCENARY_SCREEN_LEVEL", (mercenary.getLevel(),)) + "      " + localText.getText("TXT_KEY_MERCENARY_SCREEN_XP", (strXP,)))
			screen.attachLabel( mercenaryName+"Text", mercenaryName  + "text4", "     " + localText.getText("TXT_KEY_MERCENARY_SCREEN_MAINTENANCE", (strCost,)) )
			screen.attachPanel(mercenaryName, mercenaryName+"hireButtonPanel", "", "", False, True, PanelStyles.PANEL_STYLE_EMPTY)

			# Only show the fire button if the mercenary is in the game
			if(mercenary.objUnit != None):
				# edead: this doesn't work anyway
		...
		if hiredCount == -1:
			self.iHiredCount = mercenaryCount
		else:
			self.iHiredCount = hiredCount
		...
		# apply padding to Hired panel
		if (iPadding - self.iHiredCount) > 0:
			for i in range(iPadding - self.iHiredCount):
		...
		sd.setVal('iHiredCount', self.iHiredCount)
	...
	def populateAvailableUnitsPanel(self, screen):
		...
		for unitID in unitDict:
			...
			screen.attachLabel( unitID+"Text", unitID  + "text3", "     " + localText.getText("TXT_KEY_MERCENARY_SCREEN_LEVEL", (unit.getLevel(),)) + "      " + localText.getText("TXT_KEY_MERCENARY_SCREEN_XP", (strXP,)))
			...
			if(bCreateContract):
				screen.attachLabel( unitID+"Text", unitID  + "text4", "     " + localText.getText("TXT_KEY_MERCENARY_SCREEN_XP", (unit.plot().getPlotCity().getName(),)))
	...
	def populateContractedOutUnitsPanel(self, screen):
		...
		for unitID in unitDict:
			...
			if(unit.isHired()):
				screen.attachLabel( unitID+"Text", unitID  + "text4", "     " + localText.getText("TXT_KEY_MERCENARY_SCREEN_HIRED_BY", (gc.getPlayer(unit.getOwner()).getName(),)))
	...
	def clearUnitInformation(self, screen):
		...
		screen.deleteWidget("UnitSpecialAbilities")
		screen.deleteWidget("UnitInnerSpecialAbilities")
		screen.deleteWidget("UnitSpecialAbilitiesList")
		

	# Populates the unit information panel with the unit information details		
	def populateUnitInformation(self, screen, mercenary):
		
		# edead: Add Special Abilities
		screen.addPanel( "UnitSpecialAbilities", "", "", true, false, self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_X] + 5, self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_Y] + 5, self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_WIDTH] - 10, self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_HEIGHT], PanelStyles.PANEL_STYLE_STANDARD )
		screen.addPanel( "UnitInnerSpecialAbilities", localText.getText("TXT_KEY_PEDIA_SPECIAL_ABILITIES", ()), "", true, false, self.screenWidgetData[UNIT_INFORMATION_INNER_PROMOTION_PANEL_X]+5, self.screenWidgetData[UNIT_INFORMATION_INNER_PROMOTION_PANEL_Y] + 5, self.screenWidgetData[UNIT_INFORMATION_INNER_PROMOTION_PANEL_WIDTH] - 10, self.screenWidgetData[UNIT_INFORMATION_INNER_PROMOTION_PANEL_HEIGHT], PanelStyles.PANEL_STYLE_EMPTY )
		szSpecialText = CyGameTextMgr().getUnitHelp( mercenary.getUnitInfoID(), True, False, False, None )[1:]
		screen.addMultilineText( "UnitSpecialAbilitiesList", szSpecialText, self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_X]+10, self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_Y]+40, self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_WIDTH]-10, self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_HEIGHT]-45, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)	
		
		screen.addPanel(UNIT_INFORMATION_PROMOTION_PANEL_ID, "", "", True, True, self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_X] + 5, self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_Y] + self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_HEIGHT] + 15, self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_WIDTH] - 10, self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_HEIGHT], PanelStyles.PANEL_STYLE_STANDARD)
		screen.addPanel(UNIT_INFORMATION_INNER_PROMOTION_PANEL_ID, "Promotions", "", True, True, self.screenWidgetData[UNIT_INFORMATION_INNER_PROMOTION_PANEL_X] + 5, self.screenWidgetData[UNIT_INFORMATION_INNER_PROMOTION_PANEL_Y] + self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_HEIGHT] + 15, self.screenWidgetData[UNIT_INFORMATION_INNER_PROMOTION_PANEL_WIDTH] - 10, self.screenWidgetData[UNIT_INFORMATION_INNER_PROMOTION_PANEL_HEIGHT], PanelStyles.PANEL_STYLE_EMPTY)
		...
		screen.appendListBoxString( UNIT_INFORMATION_DETAILS_LIST_ID, "  " + localText.getText("TXT_KEY_MERCENARY_SCREEN_UNIT_TYPE", (mercenary.getUnitInfo().getDescription(),)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
		screen.appendListBoxString( UNIT_INFORMATION_DETAILS_LIST_ID, "  " + localText.getText("TXT_KEY_MERCENARY_SCREEN_LEVEL", (mercenary.getLevel(),)) + "     " + localText.getText("TXT_KEY_MERCENARY_SCREEN_XP", (strXP,)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
		...
		if(mercenary.isHired()):
			screen.appendListBoxString( UNIT_INFORMATION_DETAILS_LIST_ID, "  " + localText.getText("TXT_KEY_MERCENARY_SCREEN_HIRED_BY", (gc.getPlayer(mercenary.getOwner()).getName(),)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )

		# Otherwise show their hire cost information			
		else:
			# Build the unit hire cost string
			strHCost = u"%d%c" %(mercenary.getHireCost(), gc.getCommerceInfo(CommerceTypes.COMMERCE_GOLD).getChar())
			screen.appendListBoxString( UNIT_INFORMATION_DETAILS_LIST_ID, "  " + localText.getText("TXT_KEY_MERCENARY_SCREEN_CONTRACT_INCOME", (strHCost,)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
			#screen.appendListBoxString( UNIT_INFORMATION_DETAILS_LIST_ID, "  " + localText.getText("TXT_KEY_MERCENARY_SCREEN_LOCATION", (objMercenaryUtils.getMercenaryStartingCityName(gc.getGame().getActivePlayer(), mercenary),)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
		
		# Show the contract income for the unit if it is in the game			
		if(mercenary.isPlaced()):
			strMCost = u"%d%c" %(mercenary.getMercenaryMaintenanceCost(), gc.getCommerceInfo(CommerceTypes.COMMERCE_GOLD).getChar())
			screen.appendListBoxString( UNIT_INFORMATION_DETAILS_LIST_ID, "  " + localText.getText("TXT_KEY_MERCENARY_SCREEN_INCOME_PER_TURN", (strMCost,)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
		...
		# Otherwise, show the number of turns until the unit is placed in the game
			...
			if(placementTurns <= 1):
				strPlacement = localText.getText("TXT_KEY_MERCENARY_ARRIVES_NEXT_TURN", ())
			else:
				strPlacement = localText.getText("TXT_KEY_MERCENARY_ARRIVES_IN", (placementTurns,))
		...
		# edead: Add Free Promotions
		unitInfo = gc.getUnitInfo(mercenary.getUnitInfoID())
		promotionTypes = []
		for promotion in promotionList:
			promotionTypes.append(gc.getInfoTypeForString(promotion.getType()))
		for iPromotion in range(gc.getNumPromotionInfos()):
			if unitInfo.getFreePromotions(iPromotion) and iPromotion not in promotionTypes:
				promotionList.insert(0, gc.getPromotionInfo(iPromotion))
	...
	def clearMercenaryInformation(self, screen):
		...
		screen.deleteWidget("SpecialAbilities")
		screen.deleteWidget("InnerSpecialAbilities")
		screen.deleteWidget("SpecialAbilitiesList")
	...
	def populateMercenaryInformation(self, screen, mercenary):
		...
		# edead: Add Special Abilities
		screen.addPanel( "SpecialAbilities", "", "", true, false, self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_X] + 5, self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_Y] + 5, self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_WIDTH] - 10, self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_HEIGHT], PanelStyles.PANEL_STYLE_STANDARD )
		screen.addPanel( "InnerSpecialAbilities", localText.getText("TXT_KEY_PEDIA_SPECIAL_ABILITIES", ()), "", true, false, self.screenWidgetData[MERCENARY_INFORMATION_INNER_PROMOTION_PANEL_X]+5, self.screenWidgetData[MERCENARY_INFORMATION_INNER_PROMOTION_PANEL_Y] + 5, self.screenWidgetData[MERCENARY_INFORMATION_INNER_PROMOTION_PANEL_WIDTH] - 10, self.screenWidgetData[MERCENARY_INFORMATION_INNER_PROMOTION_PANEL_HEIGHT], PanelStyles.PANEL_STYLE_EMPTY )
		szSpecialText = CyGameTextMgr().getUnitHelp( mercenary.getUnitInfoID(), True, False, False, None )[1:]
		screen.addMultilineText( "SpecialAbilitiesList", szSpecialText, self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_X]+10, self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_Y]+40, self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_WIDTH]-10, self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_HEIGHT]-45, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)	
		
		screen.addPanel(MERCENARY_INFORMATION_PROMOTION_PANEL_ID, "", "", True, True, self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_X] + 5, self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_Y] + self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_HEIGHT] + 15, self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_WIDTH] - 10, self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_HEIGHT], PanelStyles.PANEL_STYLE_STANDARD)
		screen.addPanel(MERCENARY_INFORMATION_INNER_PROMOTION_PANEL_ID, "Promotions", "", True, True, self.screenWidgetData[MERCENARY_INFORMATION_INNER_PROMOTION_PANEL_X] + 5, self.screenWidgetData[MERCENARY_INFORMATION_INNER_PROMOTION_PANEL_Y] + self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_HEIGHT] + 15, self.screenWidgetData[MERCENARY_INFORMATION_INNER_PROMOTION_PANEL_WIDTH] - 10, self.screenWidgetData[MERCENARY_INFORMATION_INNER_PROMOTION_PANEL_HEIGHT], PanelStyles.PANEL_STYLE_EMPTY)
		...
		screen.appendListBoxString( MERCENARY_INFORMATION_DETAILS_LIST_ID, "  " + localText.getText("TXT_KEY_MERCENARY_SCREEN_UNIT_TYPE", (mercenary.getUnitInfo().getDescription(),)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
		screen.appendListBoxString( MERCENARY_INFORMATION_DETAILS_LIST_ID, "  " + localText.getText("TXT_KEY_MERCENARY_SCREEN_LEVEL", (mercenary.getLevel(),)) + "     " + localText.getText("TXT_KEY_MERCENARY_SCREEN_XP", (strXP,)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
		...
		screen.appendListBoxString( MERCENARY_INFORMATION_DETAILS_LIST_ID, "  " + localText.getText("TXT_KEY_MERCENARY_SCREEN_COST", (strHCost,)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )

		# Show the mercenary costs if it is in the game			
		if(mercenary.isPlaced()):
			screen.appendListBoxString( MERCENARY_INFORMATION_DETAILS_LIST_ID, "  " + localText.getText("TXT_KEY_MERCENARY_SCREEN_MAINTENANCE", (strMCost,)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
			if not mercenary.isHired():
				screen.appendListBoxString( MERCENARY_INFORMATION_DETAILS_LIST_ID, "  " + localText.getText("TXT_KEY_MERCENARY_SCREEN_LOCATION", (objMercenaryUtils.getMercenaryStartingCityName(gc.getGame().getActivePlayer(), mercenary),)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
		# Otherwise, show the number of turns until the unit is placed in the game
		else:
			# Get the number of turns until the unit/mercenary is placed in the game
			placementTurns = mercenary.getPlacementTurns()
	...
	def drawGoldInformation(self, screen):
		...
		if(self.currentScreen == MERCENARY_CONTRACT_MANAGER):
			...
			strCost = u"%s %c: %d" %(localText.getText("TXT_KEY_MERCENARY_INCOME", ()), gc.getCommerceInfo(CommerceTypes.COMMERCE_GOLD).getChar(),iCost)
		...
		# Otherwise get the mercenary maintenance information
			...
			strCost = u"%s %c: %d" %(localText.getText("TXT_KEY_MERCENARY_MAINTENANCE", ()), gc.getCommerceInfo(CommerceTypes.COMMERCE_GOLD).getChar(),iCost)
	...
	def drawScreenTop(self, screen):
		...
		screen.setLabel(SCREEN_TITLE_TEXT_PANEL_ID, "Background", self.screenWidgetData[SCREEN_TITLE_TEXT_PANEL], CvUtil.FONT_CENTER_JUSTIFY, self.screenWidgetData[SCREEN_TITLE_TEXT_PANEL_X], self.screenWidgetData[SCREEN_TITLE_TEXT_PANEL_Y], self.Z_CONTROLS, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
	...
	def getGoldText(self, iPlayer):
		...
		strDelta = localText.getText("TXT_KEY_MERCENARY_SCREEN_PER_TURN", (iDelta,))
		
		# Set the color for the gold/turn.
		if iDelta > 0:
			strDelta = u"%s" % localText.changeTextColor("(+" + strDelta + ")", gc.getInfoTypeForString("COLOR_GREEN"))
		elif iDelta < 0:
			strDelta = u"%s" % localText.changeTextColor("(" + strDelta + ")", gc.getInfoTypeForString("COLOR_RED"))
		else: strDelta = u""
	...
	def contractOutMercenary(self, screen, panelID, unitID):
		...
		screen.attachLabel( unitID+"Text", unitID  + "text3", "     " + localText.getText("TXT_KEY_MERCENARY_SCREEN_LEVEL", (mercenary.getLevel(),)) + "      " + localText.getText("TXT_KEY_MERCENARY_SCREEN_XP", (strXP,)) )
	...
	def cancelContract(self, screen, panelID, unitID, mercenaryName):
		...
		screen.attachLabel( unitID+"Text", unitID  + "text3", "     " + localText.getText("TXT_KEY_MERCENARY_SCREEN_LEVEL", (level,)) + "      " + localText.getText("TXT_KEY_MERCENARY_SCREEN_XP", (strXP,)) )
		...
		# Show the create contract button if the player can contract out the unit
		if(bCreateContract):
			screen.attachLabel( unitID+"Text", unitID  + "text4", "     " + localText.getText("TXT_KEY_MERCENARY_SCREEN_IN", (unit.plot().getPlotCity().getName(),)) )
			screen.attachImageButton( unitID, unitID+"_CreateContractButton", 
										"Art/Interface/Buttons/Actions/StealPlans.dds", GenericButtonSizes.BUTTON_SIZE_32, WidgetTypes.WIDGET_GENERAL, -1, -1, False )		
	...
	def hireMercenary(self, screen, mercenaryName):
		...
		self.iHiredCount = sd.getVal('iHiredCount')
		self.iAvailableCount = sd.getVal('iAvailableCount')
		...
		# edead: UI fix
		self.iHiredCount = self.iHiredCount + 1
		self.iAvailableCount = self.iAvailableCount - 1
		self.populateAvailableMercenariesPanel(screen, self.iAvailableCount)
		sd.setVal('iHiredCount', self.iHiredCount)
		...
		#Rhye - start Carthaginian UP
		#strCost = u"%d%c" %(mercenary.getMercenaryMaintenanceCost(), gc.getCommerceInfo(CommerceTypes.COMMERCE_GOLD).getChar())
		strCost = u"%d%c" %(mercenary.getMercenaryMaintenanceCostXPlayer(iPlayer), gc.getCommerceInfo(CommerceTypes.COMMERCE_GOLD).getChar())
		#Rhye - end UP
	...
	# by edead
	def addPadding(self, screen):
		"""One method to handle all padding in scrolling panels."""
	...
	def calculateScreenWidgetData(self, screen):
		...
		self.screenWidgetData[AVAILABLE_MERCENARIES_PANEL_WIDTH] = 480 # edead (450)
		...
		self.screenWidgetData[HIRED_MERCENARIES_PANEL_WIDTH] = 480 # edead (450)
		...
		self.screenWidgetData[MERCENARY_ANIMATION_WIDTH] = 270
		self.screenWidgetData[MERCENARY_ANIMATION_HEIGHT] = 196
		...
		self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_WIDTH] = self.screenWidgetData[MERCENARY_INFORMATION_PANEL_WIDTH] - (self.screenWidgetData[BORDER_WIDTH]*6)
		self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_HEIGHT] = 128 + (self.screenWidgetData[BORDER_WIDTH]*9)

		self.screenWidgetData[MERCENARY_INFORMATION_INNER_PROMOTION_PANEL_X] = self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_X] + self.screenWidgetData[BORDER_WIDTH] + 5
		...
		self.screenWidgetData[MERCENARY_INFORMATION_DETAILS_PANEL_X] = self.screenWidgetData[MERCENARY_ANIMATION_X] + self.screenWidgetData[MERCENARY_ANIMATION_WIDTH] + (self.screenWidgetData[BORDER_WIDTH]*2) - 8
		self.screenWidgetData[MERCENARY_INFORMATION_DETAILS_PANEL_Y] = self.screenWidgetData[MERCENARY_ANIMATION_Y] - 10
		...
		self.screenWidgetData[MERCENARY_INFORMATION_DETAILS_PANEL_HEIGHT] = self.screenWidgetData[MERCENARY_ANIMATION_HEIGHT] + 19
		...
		self.screenWidgetData[UNITS_CONTRACTED_OUT_PANEL_WIDTH] = 480 # edead (450)
		...
		self.screenWidgetData[AVAILABLE_UNITS_PANEL_WIDTH] = 480 # edead (450)
		...
		self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_X] = self.screenWidgetData[MERCENARY_INFORMATION_TEXT_BACKGROUND_PANEL_X]
		self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_Y] = self.screenWidgetData[MERCENARY_ANIMATION_Y] + self.screenWidgetData[MERCENARY_ANIMATION_HEIGHT] + self.screenWidgetData[BORDER_WIDTH]
		self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_WIDTH] = self.screenWidgetData[MERCENARY_INFORMATION_PANEL_WIDTH] - (self.screenWidgetData[BORDER_WIDTH]*6)
		self.screenWidgetData[UNIT_INFORMATION_PROMOTION_PANEL_HEIGHT] = 128 + (self.screenWidgetData[BORDER_WIDTH]*9)

		self.screenWidgetData[UNIT_INFORMATION_INNER_PROMOTION_PANEL_X] = self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_X] + self.screenWidgetData[BORDER_WIDTH] + 5
		self.screenWidgetData[UNIT_INFORMATION_INNER_PROMOTION_PANEL_Y] = self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_Y] + self.screenWidgetData[BORDER_WIDTH]
		self.screenWidgetData[UNIT_INFORMATION_INNER_PROMOTION_PANEL_WIDTH] = self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_WIDTH] + (self.screenWidgetData[BORDER_WIDTH]*2)
		self.screenWidgetData[UNIT_INFORMATION_INNER_PROMOTION_PANEL_HEIGHT] = self.screenWidgetData[MERCENARY_INFORMATION_PROMOTION_PANEL_HEIGHT] - (self.screenWidgetData[BORDER_WIDTH]*2)

		self.screenWidgetData[UNIT_INFORMATION_DETAILS_PANEL_X] = self.screenWidgetData[MERCENARY_ANIMATION_X] + self.screenWidgetData[MERCENARY_ANIMATION_WIDTH] + (self.screenWidgetData[BORDER_WIDTH]*2)
		self.screenWidgetData[UNIT_INFORMATION_DETAILS_PANEL_Y] = self.screenWidgetData[MERCENARY_ANIMATION_Y]
		self.screenWidgetData[UNIT_INFORMATION_DETAILS_PANEL_WIDTH] = self.screenWidgetData[SCREEN_WIDTH] - (self.screenWidgetData[MERCENARY_ANIMATION_X] + self.screenWidgetData[MERCENARY_ANIMATION_WIDTH] + (self.screenWidgetData[BORDER_WIDTH])*6)
		self.screenWidgetData[UNIT_INFORMATION_DETAILS_PANEL_HEIGHT] = self.screenWidgetData[MERCENARY_ANIMATION_HEIGHT]
		...
		self.screenWidgetData[AVAILABLE_MERCENARY_GROUPS_PANEL_WIDTH] = 480 # edead (450)
		...
		self.screenWidgetData[HIRED_MERCENARY_GROUPS_PANEL_WIDTH] = 480 # edead (450)
