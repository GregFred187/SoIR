...
from Consts import iNumPlayers, relics # edead
...
#		self.bShowAllPlayers = false
	...
	def showScreen(self, iTurn, iTabID, iEndGame):
		...
		for iLoopPlayer in range(iNumPlayers): # edead
...
#############################################################################################################
	...
	def drawTextChart(self):
		...
		#for iPlayerLoop in range(gc.getMAX_PLAYERS()): #Rhye
                for iPlayerLoop in range(iNumPlayers): #Rhye
			...
			if (gc.getPlayer(iPlayerLoop).isAlive() and not gc.getPlayer(iPlayerLoop).isBarbarian() and not gc.getPlayer(iPlayerLoop).isMinorCiv()): #Rhye
				...
				iValue = pCurrPlayer.getPower() * 100 # edead (1000)
				if iPlayerLoop == self.iActivePlayer:
					iMilitary = iValue
				else:
		...
		for iRankLoop in range(5):
			...
			if (pCity):
				...
				if (pCity.isRevealed(gc.getGame().getActiveTeam()) or gc.getTeam(pPlayer.getTeam()).isHasMet(gc.getGame().getActiveTeam())):
					...
					self.szCityDescs[iRankLoop] = ("%s" %(pPlayer.getCivilizationAdjective(0))) #edead
				else:
					...
					self.szCityDescs[iRankLoop] = '' #edead
...
#############################################################################################################
	...
	def drawWondersDropdownBox(self):
		...
		if (self.szWonderDisplayMode == "Relics"):
			bDefault = true
		else:
			bDefault = false
		screen.addPullDownString(self.szWondersDropdownWidget, localText.getText("TXT_KEY_PEDIA_CATEGORY_RELIC", ()), 3, 3, bDefault )
	...
	############### Create ListBox for Projects ###############
		...
		elif (self.szWonderDisplayMode == "Relics"):

	############### Create ListBox for Relics ###############
...
############################################### DISPLAY PROJECT MODE ###############################################
			...
			if (self.szWonderDisplayMode == "Projects"):
				...
				szWonderDesc = "%s" %(self.aiWonderBuiltBy[self.iActiveWonderCounter]) # edead
				...
				for pCity in apCityList:
					...
					# Loop through buildings
						...
						for iBuildingLoop in range(gc.getNumBuildingInfos()):
							...
							# seperate relics and wonders
							relicList = con.relics.keys()
							if (iBuildingLoop in relicList):
								isRelic = 1
							else:
								isRelic = 0

							# World Wonder Mode
							if (self.szWonderDisplayMode == "World Wonders" and isWorldWonderClass(gc.getBuildingInfo(iBuildingLoop).getBuildingClassType()) and isRelic == 0):
	...
	#								print("Adding National wonder to list: %s, %d, %s" %(pCity.getBuildingOriginalTime(iBuildingLoop),iBuildingLoop,pPlayer.getCivilizationAdjective(0)))
							...
							# Relics Mode
							elif (self.szWonderDisplayMode == "Relics" and isWorldWonderClass(gc.getBuildingInfo(iBuildingLoop).getBuildingClassType()) and isRelic == 1):
	...
	def drawStatsTab(self):
		...
		for i in range(iNumUnits):
			if not gc.getUnitInfo(i).isGraphicalOnly():
				...
				unitDict[i] = j
		iNumBuildings = gc.getNumBuildingInfos() - len(relics) # excluse relics
		# edead: end
		...
		self.iNumUnitStatsChartRows = iNumUnitTypes
			...
			if (self.iActiveTab == self.iTopCitiesID):
				...
				if (szWidgetName == self.szWondersDropdownWidget):
					...
					if (iSelected == 0):
						...
						self.szWonderDisplayMode = "Relics"
