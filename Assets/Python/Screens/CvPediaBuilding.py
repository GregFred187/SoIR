...
from Consts import relics # edead
...
class CvPediaBuilding:
	...
	def interfaceScreen(self, iBuilding):	
		...
		# Unit animation
		# edead: start
		if self.getBuildingType(self.iBuilding) == 3:
			#screen.addDDSGFC(self.top.getNextWidgetName(), gc.getBuildingInfo(self.iBuilding).getArtInfo().getNIF(), self.X_BUILDING_ANIMATION, self.Y_BUILDING_ANIMATION, self.W_BUILDING_ANIMATION, self.H_BUILDING_ANIMATION, WidgetTypes.WIDGET_GENERAL, -1, -1 )
			screen.addUnitGraphicGFC(self.top.getNextWidgetName(), relics[self.iBuilding][0], self.X_BUILDING_ANIMATION, self.Y_BUILDING_ANIMATION, self.W_BUILDING_ANIMATION, self.H_BUILDING_ANIMATION, WidgetTypes.WIDGET_GENERAL, -1, -1, self.X_ROTATION_BUILDING_ANIMATION, self.Z_ROTATION_BUILDING_ANIMATION, self.SCALE_ANIMATION, True)
		else:
	...
	def placeRequires(self):
		...
		# edead: start
		iPrereq = gc.getBuildingInfo(self.iBuilding).getOrPrereqReligion()
		if (iPrereq >= 0):
			screen.attachLabel(panelName, "", localText.getText("TXT_KEY_OR", ()))
			screen.attachImageButton( panelName, "", gc.getReligionInfo(iPrereq).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_RELIGION, iPrereq, -1, False )
		# edead: end
...
# edead: replaced functions, borrowed from Sevopedia
...
##		return false
	...
	def getBuildingType(self, iBuilding):
		if gc.getBuildingClassInfo((gc.getBuildingInfo(iBuilding).getBuildingClassType())).isMonument():
			return 3
		elif (isWorldWonderClass(gc.getBuildingInfo(iBuilding).getBuildingClassType())):
			return 2
...
##		return listSorted
	...
	def getBuildingSortedList(self, iBuildingType):
		...
		for iBuilding in range(gc.getNumBuildingInfos()):
			...
			if self.getBuildingType(iBuilding) == iBuildingType and not gc.getBuildingInfo(iBuilding).isGraphicalOnly(): # edead
