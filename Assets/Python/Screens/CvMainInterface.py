...
import CvMercenaryModGameUtils
...
#		screen.setImageButton( "MercenaryManagerButton", ArtFileMgr.getInterfaceArtInfo("INTERFACE_MERCENARIES_MANAGER").getPath(), iBtnX, iBtnY - 1, iBtnWidth, iBtnWidth, WidgetTypes.WIDGET_ACTION, gc.getControlInfo(ControlTypes.CONTROL_TURN_LOG).getActionInfoIndex(), -1 )
		...
		if (not CyEngine().isGlobeviewUp() and pHeadSelectedCity):
			...
			if (CyInterface().getInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_SELECTION):
				...
				if ( pHeadSelectedUnit.getOwner() == gc.getGame().getActivePlayer() and g_pSelectedUnit != pHeadSelectedUnit ):
					...
					# edead: start Charlemagne inquisitor code & custom unit effect
					
					pUnit = g_pSelectedUnit
					
					# Using an Inquisitor
					if pUnit.getUnitType() == con.iInquisitor and not pUnit.isDelayedDeath():
					...
					# Great Saint
					if pUnit.getUnitType() in [con.iGreatProphet, con.iGreatProphet2, con.iGreatProphet3] and not pUnit.isDelayedDeath():
						iReligion = gc.getPlayer(pUnit.getOwner()).getStateReligion()
						if iReligion != -1:
							screen.appendMultiListButton( "BottomButtonContainer", gc.getReligionInfo(iReligion).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 667, 667, False )
							screen.show( "BottomButtonContainer" )
							iCount = iCount + 1
					
					# Relics
					if pUnit.getUnitType() in [con.iCatholicMissionary, con.iOrthodoxMissionary] and not pUnit.isDelayedDeath():
						plot = CyMap().plot(pUnit.getX(), pUnit.getY())
						if plot.isCity():
							city = plot.getPlotCity()
							if city.getOwner() == pUnit.getOwner():
								for iRelic in con.relics.keys():
									if city.getNumRealBuilding(iRelic):
										if gc.getBuildingInfo(iRelic).getStateReligion() in [con.iCatholicism, con.iOrthodoxy] or gc.getBuildingInfo(iRelic).getOrStateReligion() in [con.iCatholicism, con.iOrthodoxy]:
											screen.appendMultiListButton( "BottomButtonContainer", gc.getBuildingInfo(iRelic).getButton(), 0, WidgetTypes.WIDGET_GENERAL, iRelic + 700, iRelic + 700, False )
											screen.show( "BottomButtonContainer" )
											iCount = iCount + 1

					if pUnit.getUnitType() in [con.iSunniMissionary, con.iShiaMissionary] and not pUnit.isDelayedDeath():
						plot = CyMap().plot(pUnit.getX(), pUnit.getY())
						if plot.isCity():
							city = plot.getPlotCity()
							if city.getOwner() == pUnit.getOwner():
								for iRelic in con.relics.keys():
									if city.getNumRealBuilding(iRelic):
										if gc.getBuildingInfo(iRelic).getStateReligion() in [con.iSunni, con.iShia] or gc.getBuildingInfo(iRelic).getOrStateReligion() in [con.iSunni, con.iShia]:
											screen.appendMultiListButton( "BottomButtonContainer", gc.getBuildingInfo(iRelic).getButton(), 0, WidgetTypes.WIDGET_GENERAL, iRelic + 700, iRelic + 700, False )
											screen.show( "BottomButtonContainer" )
											iCount = iCount + 1
	...
	def updateGameDataStrings( self ):
		...
		if ( CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY  and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_ADVANCED_START):
			...
			if (gc.getPlayer(ePlayer).isAlive()):
				...
				#szText = CyGameTextMgr().getGoldStr(ePlayer) # edead
				...
				# edead: start stability icon
				iCount += 1
				if not CyInterface().isCityScreenUp() and CyGame().getGameTurn() >= getTurnForYear(con.tBirth[ePlayer]):
				...
				if iPiety >= 0 and gc.getPlayer(ePlayer).getStateReligion() >= 0 and not CyInterface().isCityScreenUp():
					...
					screen.setLabel("PietyButton", "Background", szPietyButton, CvUtil.FONT_RIGHT_JUSTIFY, 31, 50 + (iCount * 19), -0.1, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_PIETY_LEVEL, utils.getFavorLevel(ePlayer), iPiety)
					screen.setLabel("PietyText", "Background", szPietyText, CvUtil.FONT_LEFT_JUSTIFY, 31, 50 + (iCount * 19), -0.1, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_PIETY_LEVEL, utils.getFavorLevel(ePlayer), iPiety)
					screen.show("PietyButton")
					screen.show("PietyText")
					iCount += 1
				else:
					screen.hide("PietyButton")
					screen.hide("PietyText")
				...
				if not CyInterface().isCityScreenUp() and gc.getPlayer(ePlayer).getCombatExperience() > 0:
					eCombatXPButton = u"<font=2>%c</font>" %(CyGame().getSymbolID(FontSymbols.POWER_CHAR) + 12)
		...
		if ( CyInterface().isCityScreenUp() ):
			if ( pHeadSelectedCity ):
				...
				for i in range( gc.getNumBuildingInfos() ):
					...
					if gc.getBuildingInfo(i).isGraphicalOnly(): continue # edead - do not display hidden buildings (plague)
					...
					if (gc.getTeam(gc.getGame().getActiveTeam()).isHasMet(eTeam) or gc.getTeam(eTeam).isHuman() or gc.getGame().isDebugMode()):
						...
						while (j > -1):
							...
							if (not CyInterface().isScoresMinimized() or gc.getGame().getActivePlayer() == ePlayer):
								if (gc.getPlayer(ePlayer).isAlive() and not gc.getPlayer(ePlayer).isMinorCiv()):
								#if (gc.getPlayer(ePlayer).isEverAlive() and not gc.getPlayer(ePlayer).isMinorCiv()): # Rhye
									...
									if (gc.getPlayer(ePlayer).getTeam() == eTeam):
										...
										# edead - start (scoreboard with civ names instead of leaders)
										gsName = gc.getPlayer(ePlayer).getCivilizationDescription(0) # getCivilizationShortDescription(0)
										if (not CyInterface().isFlashingPlayer(ePlayer) or CyInterface().shouldFlash(ePlayer)):
											if (ePlayer == gc.getGame().getActivePlayer()):
												#szTempBuffer = u"%d: [<color=%d,%d,%d,%d>%s</color>] " %(gc.getGame().getPlayerScore(ePlayer), gc.getPlayer(ePlayer).getPlayerTextColorR(), gc.getPlayer(ePlayer).getPlayerTextColorG(), gc.getPlayer(ePlayer).getPlayerTextColorB(), gc.getPlayer(ePlayer).getPlayerTextColorA(), gsName)
												szTempBuffer = u"[<color=%d,%d,%d,%d>%s</color>] " %(gc.getPlayer(ePlayer).getPlayerTextColorR(), gc.getPlayer(ePlayer).getPlayerTextColorG(), gc.getPlayer(ePlayer).getPlayerTextColorB(), gc.getPlayer(ePlayer).getPlayerTextColorA(), gsName)
											else:
												#szTempBuffer = u"%d: <color=%d,%d,%d,%d>%s</color> " %(gc.getGame().getPlayerScore(ePlayer), gc.getPlayer(ePlayer).getPlayerTextColorR(), gc.getPlayer(ePlayer).getPlayerTextColorG(), gc.getPlayer(ePlayer).getPlayerTextColorB(), gc.getPlayer(ePlayer).getPlayerTextColorA(), gsName)
												szTempBuffer = u"<color=%d,%d,%d,%d>%s</color> " %(gc.getPlayer(ePlayer).getPlayerTextColorR(), gc.getPlayer(ePlayer).getPlayerTextColorG(), gc.getPlayer(ePlayer).getPlayerTextColorB(), gc.getPlayer(ePlayer).getPlayerTextColorA(), gsName)
										else:
											szTempBuffer = u"%d: %s " %(gc.getGame().getPlayerScore(ePlayer), gsName)
										szBuffer = szBuffer + szTempBuffer
										# edead - end
										...
										if (gc.getTeam(eTeam).isAlive()):
											...
											if (gc.getPlayer(ePlayer).getStateReligion() != -1):
												...
												# if iPiety >= 0:
													# szTempBuffer = ":%d" %(iPiety)
													# szTempBuffer += "/%d" %(iBasePiety)
													# szBuffer = szBuffer + szTempBuffer
												# edead: end
											#Rhye - start plague
										...
										#Rhye - start
										if (not gc.getTeam(eTeam).isAlive() and CyGame().getGameTurn() >= getTurnForYear(con.tBirth[eTeam])):
												szBuffer = szBuffer + "-" 
										else:
												szBuffer = szBuffer + u" - %d" %(CyGame().getPlayerScore(ePlayer))
										#Rhye - end
										...
										if (gc.getPlayer(ePlayer).isAlive()):
											if (ePlayer < con.iNumPlayers): #edead
	...
	def setMinimapButtonVisibility( self, bVisible):
		...
		#kMainButtons = ["UnitIcons", "Grid", "BareMap", "Yields", "ScoresVisible", "ResourceIcons"] #edead
	...
	def createMinimapButtons( self ):
		...
		#ArtFileMgr.getInterfaceArtInfo("INTERFACE_STABILITY_OVERLAY").getPath()
	...
	def handleInput (self, inputClass):
		...
		# edead: start Persecution button
		if inputClass.getNotifyCode() == 11 and inputClass.getData1() == 666 and inputClass.getData2() == 666:
			if gc.getGame().getActivePlayer() == utils.getHumanID() and not g_pSelectedUnit.isDelayedDeath():
				...
				if not pCity.isNone():
					...
					if len(religionList) == 1:
						utils.doPersecution(g_pSelectedUnit.getX(), g_pSelectedUnit.getY(), g_pSelectedUnit.getID())
		...
		# edead: start Great Saint button
		if inputClass.getNotifyCode() == 11 and inputClass.getData1() == 667 and inputClass.getData2() == 667:
			if not g_pSelectedUnit.isDelayedDeath():
				utils.doGreatSaintEffect(g_pSelectedUnit)
		# edead: end
		
		# edead: start Relic button
		if inputClass.getNotifyCode() == 11 and inputClass.getData1() > 700 and inputClass.getData1() == inputClass.getData2():
			if not g_pSelectedUnit.isDelayedDeath():
				iRelic = inputClass.getData1() - 700
				plot = CyMap().plot(g_pSelectedUnit.getX(), g_pSelectedUnit.getY())
				if plot.isCity():
					city = plot.getPlotCity()
					if city.getNumRealBuilding(iRelic):
						city.setNumRealBuilding(iRelic, 0)
						utils.makeUnit(con.relics[iRelic][0], g_pSelectedUnit.getOwner(), (g_pSelectedUnit.getX(), g_pSelectedUnit.getY()), 1, UnitAITypes.UNITAI_MERCHANT)
						g_pSelectedUnit.kill(False, con.iBarbarian)
		# edead: end
	...
	# edead: start code from Main Interface Mod (Impaler)
