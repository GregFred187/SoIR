...
class CvPediaMain( CvPediaScreen.CvPediaScreen ):
	...
	def __init__(self):
		...
		self.mapCategories = { 
			CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT	: self.placeConcepts, # edead
			...
			CivilopediaPageTypes.CIVILOPEDIA_PAGE_NATIONAL_WONDER	: self.placeNationalWonders, # edead
			CivilopediaPageTypes.CIVILOPEDIA_PAGE_WONDER	: self.placeWonders, 
			CivilopediaPageTypes.CIVILOPEDIA_PAGE_RELIC	: self.placeRelics, # edead
	...
	def setPediaCommonWidgets(self):
		...
		self.szCategoryNationalWonder = localText.getText("TXT_KEY_PEDIA_CATEGORY_NATIONAL_WONDER", ()) # edead
		self.szCategoryWonder = localText.getText("TXT_KEY_PEDIA_CATEGORY_WORLD_WONDER", ())		
		self.szCategoryRelic = localText.getText("TXT_KEY_PEDIA_CATEGORY_RELIC", ()) # edead
		...
		self.listCategories = [
								self.szCategoryConcept, # edead
								self.szCategoryTech, 
								self.szCategoryUnit, 
								self.szCategoryBuilding,
								self.szCategoryNationalWonder, # edead
								self.szCategoryWonder,
								self.szCategoryRelic, # edead
	...
	def placeBuildings(self):
		...
		list = self.pediaBuildingScreen.getBuildingSortedList(0) # edead
	...
	def placeWonders(self):
		...
		list = self.pediaBuildingScreen.getBuildingSortedList(2) # edead
	...
	def placeNationalWonders(self):
		...
		# Create and place a tech pane									
		#list = self.pediaBuildingScreen.getBuildingSortedList(true)
		list = self.pediaBuildingScreen.getBuildingSortedList(1) # edead
	...
	def placeRelics(self):
		screen = self.getScreen()
		
		# Create and place a tech pane									
		#list = self.pediaBuildingScreen.getBuildingSortedList(true)
		list = self.pediaBuildingScreen.getBuildingSortedList(3) # edead
		...
		for item in list:
			...
			if iRow >= iNumRows:
				iNumRows += 1
				screen.appendTableRow(tableName)
			screen.setTableText(tableName, iColumn, iRow, u"<font=3>" + item[0] + u"</font>", "", WidgetTypes.WIDGET_PEDIA_JUMP_TO_BUILDING, item[1], 1, CvUtil.FONT_LEFT_JUSTIFY)
			iCounter += 1
