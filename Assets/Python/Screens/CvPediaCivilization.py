...
class CvPediaCivilization:
	...
	def placeUnit(self):
		...
		for iUnit in range(gc.getNumUnitClassInfos()):
			...
			#if (iDefaultUnit > -1 and iUniqueUnit > -1 and iDefaultUnit != iUniqueUnit): # edead: bGraphicalOnly fix
	...
	def placeText(self):
		...
		if self.iCivilization != con.iNumPlayers and self.iCivilization != con.iNumPlayers+1: # rebels + barbs in XML
			...
			szText += localText.getText("TXT_KEY_CIVILOPEDIA_BORDER_PROVINCES", ())
			for i in range(len(lNormalRegions)):
				if i > 0: szText += ", "
				szText += gc.getGame().getRegionName(lNormalRegions[i], False)
			for i in range(len(lBroaderRegions)):
				if (lNormalRegions and i == 0) or (i > 0):
					szText += ", "
				szText += gc.getGame().getRegionName(lBroaderRegions[i], False)
		szText += "\n\n"
	...
	def placeLinks(self, bRedraw):
		...
		for iI in range(gc.getNumCivilizationInfos()):
			...
			if (not gc.getCivilizationInfo(listSorted[iI][1]).isGraphicalOnly()): # edead
