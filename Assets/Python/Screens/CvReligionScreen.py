...
class CvReligionScreen:
	...
	def __init__(self):
		...
		self.HOLY_WAR_AREA_ID = "HolyWarWidget" # edead
		self.HOLY_WAR = "HolyWarButton" # edead
		...
		self.LEFT_EDGE_TEXT = 20 # edead (10)
		self.X_RELIGION_START = 120 # edead (180)
		self.DX_RELIGION = 95 # edead (98)
		self.Y_RELIGION = 40 # edead (35)
		self.Y_FOUNDED = 95 # edead (90)
		self.Y_HOLY_CITY = 106 # edead (100)
		self.Y_INFLUENCE = 138 # edead (130)
		self.Y_RELIGION_NAME = 70 # edead (65)
		...
		self.H_CITY_AREA_SMALLER = 345 # edead

		# edead: start
		self.X_HOLY_WAR_AREA = 45
		self.Y_HOLY_WAR_AREA = 647
		self.W_HOLY_WAR_AREA = 457 * 2 + 20
		self.H_HOLY_WAR_AREA = 50
		# edead: end
		...
		self.ReligionScreenInputMap = {
			...
			self.HOLY_WAR			: self.HolyWarButton,
			}	
...
#			szLabel = localText.changeTextColor(szLabel, gc.getInfoTypeForString("COLOR_YELLOW"))
	...
	def drawCityInfo(self, iReligion):
		...
		# edead: start dynamic area allocation for holy war
		iCityAreaHeight = self.H_CITY_AREA
		if utils.getCaliphController() == self.iActivePlayer:
			iCityAreaHeight = self.H_CITY_AREA_SMALLER
		screen.addPanel(self.AREA1_ID, "", "", True, True, self.X_CITY1_AREA, self.Y_CITY_AREA, self.W_CITY_AREA, iCityAreaHeight, PanelStyles.PANEL_STYLE_MAIN)
		...
		screen.addPanel(self.AREA2_ID, "", "", True, True, self.X_CITY2_AREA, self.Y_CITY_AREA, self.W_CITY_AREA, iCityAreaHeight, PanelStyles.PANEL_STYLE_MAIN)
		...
		screen.addMultilineText("Child" + self.AREA1_ID, szLeftCities, self.X_CITY1_AREA+5, self.Y_CITY_AREA+5, self.W_CITY_AREA-10, iCityAreaHeight-10, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
		...
		screen.addMultilineText("Child" + self.AREA2_ID, szRightCities, self.X_CITY2_AREA+5, self.Y_CITY_AREA+5, self.W_CITY_AREA-10, iCityAreaHeight-10, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
	...
	def HolyWarButton(self, inputClass):
		screen = self.getScreen()
		if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED) :
			targetList = utils.getHolyWarTargets(self.iActivePlayer)
			civList = []
			for target in targetList:
				iReligion = gc.getPlayer(target[0]).getStateReligion()
				if iReligion == -1:
				...
				szTemp = gc.getPlayer(target[0]).getCivilizationDescription(0) + " (" + localText.getText(target[1], (szReligion, )) + ")"
				civList.append(szTemp)
			civList.append(localText.getText("TXT_KEY_HOLY_WAR_CANCEL", ()))
			screen.hideScreen()
			utils.showPopup(7624, localText.getText("TXT_KEY_HOLY_WAR_POPUP_TITLE", ()), localText.getText("TXT_KEY_HOLY_WAR_SELECT_TARGET", ()), civList)
			
	# edead: start
	def drawHolyWarInfo(self):
		if (not self.bScreenUp):
			return
		screen = self.getScreen()
		...
		if iCaliphPlayer == self.iActivePlayer:
			targetList = utils.getHolyWarTargets(iCaliphPlayer)
			if targetList:
				szInfo = localText.getText("TXT_KEY_HOLY_WAR_TARGETS", ())
				for i in range(len(targetList)):
					if i > 0: szInfo += ","
					szInfo += " %s" %(gc.getPlayer(targetList[i][0]).getCivilizationShortDescription(0))
				iOffset = 32
			else:
				szInfo = localText.getText("TXT_KEY_HOLY_WAR_NO_TARGETS", ())
			iDelay = gc.getGame().getGameTurn() - sd.getVal('iLastHolyWarTurn') - 100
			if iDelay < 0:
				szInfo = localText.getText("TXT_KEY_HOLY_WAR_DELAY", (-iDelay, ))
				iOffset = 0
			if sd.getPiety(self.iActivePlayer) < 50:
				szInfo = localText.getText("TXT_KEY_HOLY_WAR_PIETY_TOO_LOW", ())
				iOffset = 0
		else:
			return
		
		szHolyWarArea = self.HOLY_WAR_AREA_ID
		screen.addPanel(self.HOLY_WAR_AREA_ID, "", "", True, True, self.X_HOLY_WAR_AREA, self.Y_HOLY_WAR_AREA, self.W_HOLY_WAR_AREA, self.H_HOLY_WAR_AREA, PanelStyles.PANEL_STYLE_MAIN)
		
		if iOffset:
			screen.setImageButton( "HolyWarButton", "", self.X_HOLY_WAR_AREA + 14, self.Y_HOLY_WAR_AREA + 14, 28, 28, WidgetTypes.WIDGET_ACTION, gc.getControlInfo(ControlTypes.CONTROL_HOLY_WAR).getActionInfoIndex(), -1 )
			screen.setStyle( "HolyWarButton", "Button_HUDAdvisorVictory_Style" )
	
		# Erase the flag?
		CyInterface().setDirty(InterfaceDirtyBits.MiscButtons_DIRTY_BIT, True)
		
		screen.setLabelAt( "", szHolyWarArea, szInfo, CvUtil.FONT_LEFT_JUSTIFY, 18 + iOffset, 16, 500, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)	
