...
def canTriggerHurricaneCity(argsList):
	...
	if not city.plot().getRegionID() in [con.rOman, con.rMahra, con.rMakran, con.rMaharashtra, con.rKarnataka, con.rGujarat, con.rSindh, con.rYemen, con.rHadhramaut]:
		return false
...
def canTriggerMonsoonCity(argsList):
	...
	if city.plot().getRegionID() in [con.rSindh, con.rGujarat, con.rMaharashtra, con.rKarnataka, con.rMalwa, con.rGird]:
		return true
...
def getHelpHarbormaster1(argsList):
	...
	iHarborsRequired = 4 # edead
	#iCaravelsRequired = iHarborsRequired / 2 + 1 # edead
	iCaravelsRequired = 4 # edead
...
def canTriggerHarbormasterDone(argsList):
	...
	#iShipsRequired = iHarborsRequired / 2 + 1 # edead
	iShipsRequired = 4 # edead
...
def canApplyBestDefenseDone3(argsList):
	...
	iKrak = CvUtil.findInfoTypeNum(gc.getBuildingInfo, gc.getNumBuildingInfos(), 'BUILDING_KRAK_DES_CHEVALIERS')
	# edead: end
	...
	while(loopCity):
		...
		if loopCity.isHasBuilding(iKrak):
		# edead: end
			return true
...
def getGreedUnit(player, plot):
	...
	for iUnitClass in range(gc.getNumUnitClassInfos()):
		...
		if iUnit in [con.iGhulamGuard, con.iGhulamLancer, con.iGhulamHorseArcher, con.iTemplarSergeant, con.iTemplarKnight, con.iHospitallerSergeant, con.iHospitallerCanon, con.iHospitallerKnight, con.iItalianMaceman, con.iItalianCrossbowman]: 
			continue
		# edead: end
...
def getHelpGreedDone1(argsList):
	...
	iNumPlayers = 0
	for iLoopPlayer in range(gc.getMAX_CIV_PLAYERS()):
		if gc.getPlayer(iLoopPlayer).isAlive() and not gc.getPlayer(iLoopPlayer).isMinorCiv():
			iNumPlayers += 1
	iNumUnits = min(5, iNumPlayers / 2 + 1)
	# edead: end
...
def getHelpNobleKnights1(argsList):
	...
	iBuilding = CvUtil.findInfoTypeNum(gc.getBuildingInfo, gc.getNumBuildingInfos(), 'BUILDING_KRAK_DES_CHEVALIERS') # edead
...
def canTriggerGreekFire(argsList):
	...
	if kTriggeredData.ePlayer != con.iByzantium:
		return false
...
def applyGreekFire1(argsList):
	...
	iGreekFlamethrower = CvUtil.findInfoTypeNum(gc.getUnitInfo, gc.getNumUnitInfos(), 'UNIT_GREEK_FLAMETHROWER')
    
	unitList = PyPlayer(kTriggeredData.ePlayer).getUnitList()
	for unit in unitList:
		if unit.getUnitType() == iWarGalley or unit.getUnitType() == iGreekFlamethrower:
...
def canTriggerAristocratsConcerned(argsList):
	...
	if utils.getStability(kTriggeredData.ePlayer) < 10:
		return false
...
def canTriggerReconquestOfHolyLand(argsList):
	...
	if player.getStateReligion() != con.iSunni and player.getStateReligion() != con.iShia:
		return false
	
	if gc.getGame().getGameTurn() < getTurnForYear(con.tBirth[con.iAyyubids]) + 2:
		return false
	
	if gc.getGame().getGameTurn() > getTurnForYear(1299):
		return false
	
	if gc.getPlayer(gc.getMap().plot(con.tJerusalem[0], con.tJerusalem[1]).getOwner()).getStateReligion() not in [con.iCatholicism, con.iOrthodoxy]:
		return false
...
def canTriggerSoilDegradation(argsList):
	...
	if utils.getYear() < 1200:
		...
		if gc.getMap().plot(con.tBaghdad[0],con.tBaghdad[1]).getOwner() == kTriggeredData.ePlayer:
			return true
		if gc.getMap().plot(con.tSamarra[0],con.tSamarra[1]).getOwner() == kTriggeredData.ePlayer:
			return true
		if gc.getMap().plot(con.tNajaf[0],con.tNajaf[1]).getOwner() == kTriggeredData.ePlayer:
			return true
...
def canTriggerPaulicians(argsList):
	kTriggeredData = argsList[0]
	
	if kTriggeredData.ePlayer != con.iByzantium:
		return false
	
	if utils.getYear() > 1950:
		return false
	
	plot = gc.getMap().plot(kTriggeredData.iPlotX, kTriggeredData.iPlotY)
	if plot.getRegionID() not in [con.rLesserArmenia, con.rGalatia, con.rCappadocia, con.rPontus, con.rCilicia, con.rEdessa, con.rNorthernSyria]:
		return false
	
	return true

def applyPaulicians1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	...
	if listPlots:
		pBarbarian = gc.getPlayer(gc.getMAX_CIV_PLAYERS())
		for i in range(random.randint(2, 3)):
			plot = random.choice(listPlots)
			pBarbarian.initUnit(con.iAxeman_Paulician, plot.getX(), plot.getY(), UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)

def getHelpPaulicians1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	...
	szHelp = localText.getText("TXT_KEY_EVENT_HELP_PAULICIANS_1", (city.getName(), ))

	return szHelp	

def applyPaulicians2(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	
	plot = gc.getMap().plot(kTriggeredData.iPlotX, kTriggeredData.iPlotY)
	gc.getPlayer(kTriggeredData.ePlayer).initUnit(con.iAxeman_Paulician, plot.getX(), plot.getY(), UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
	sd.changePiety(kTriggeredData.ePlayer, -5)
	sd.setStability(kTriggeredData.ePlayer, sd.getStability(kTriggeredData.ePlayer) - 2)
	CvMainInterface().updateGameDataStrings()

def getHelpPaulicians2(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	
	szHelp = localText.getText("TXT_KEY_EVENT_HELP_PAULICIANS_2", (2, 5))
...
def canTriggerPilgrimsCity(argsList):
	...
	if city.getOwner() == con.iCrusaders and city.isCoastal(20):
		return true
...
def canTriggerKnightsOfStJohnCity(argsList):
	...
	if ePlayer == con.iAntioch or ePlayer == con.iCrusaders:
		return true
...
def canTriggerAlmonds(argsList):
	...
	if not kTriggeredData.ePlayer in [con.iByzantium, con.iSeljuks, con.iRum, con.iOttomans, con.iSafavids, con.iAntioch, con.iCrusaders]:
		return false
...
def canTriggerApricots(argsList):
	...
	if not kTriggeredData.ePlayer in [con.iByzantium, con.iSeljuks, con.iRum, con.iOttomans, con.iSafavids, con.iArmenia, con.iGeorgia, con.iKhwarezm, con.iSamanids]:
		return false
...
def canTriggerSesame(argsList):
	...
	if not kTriggeredData.ePlayer in [con.iSeljuks, con.iRum, con.iOttomans, con.iSafavids, con.iYemen, con.iFatimids, con.iAyyubids, con.iMamluks, con.iSindh, con.iGhaznavids, con.iGhorids]:
		return false
...
def canTriggerPistachios(argsList):
	...
	if not kTriggeredData.ePlayer in [con.iSeljuks, con.iRum, con.iOttomans, con.iSafavids]:
		return false
...
def canTriggerPersianRugsCity(argsList):
	...
	if city.plot().getRegionID() in [con.rKerman, con.rYazd, con.rFars, con.rJibal, con.rLuristan, con.rKurdistan]:
		return true
...
def canTriggerConfectionersCity(argsList):
	...
	if city.plot().getRegionID() in [con.rKerman, con.rYazd, con.rFars, con.rJibal, con.rLuristan, con.rKurdistan, con.rYemen, con.rOman, con.rLowerEgypt, con.rMesopotamia, con.rAsuristan, con.rAzerbaijan, con.rWesternKhorasan, con.rEasternKhorasan]:
		return true
...
def canTriggerCamelRacesCity(argsList):
	...
	if city.getOwner() not in [con.iFatimids, con.iAyyubids, con.iMamluks, con.iYemen, con.iOman, con.iAbbasids, con.iSindh]:
		return false
		
	if city.plot().getRegionID() not in [con.rYemen, con.rOman, con.rUpperEgypt, con.rLowerEgypt, con.rHadhramaut, con.rBahrain, con.rMahra, con.rHejaz, con.rSindh]:
		return false
...
def applyPeasantRevolt1(argsList):
	...
	if kTriggeredData.ePlayer in [con.iByzantium, con.iAntioch, con.iCrusaders, con.iGeorgia, con.iArmenia]:
...
def canTriggerMassacreOfTheLatinsCity(argsList):
	...
	if city.getOwner() != con.iByzantium:
		return false
...
def canTriggerFourthCrusade(argsList):
	...
	if gc.getMap().plot(con.tConstantinople[0], con.tConstantinople[1]).getOwner() != con.iByzantium:
		return false
...
def applyFourthCrusade1(argsList):
	...
	if not gc.getTeam(gc.getPlayer(con.iIndependent).getTeam()).isAtWar(con.iByzantium):
		gc.getTeam(gc.getPlayer(con.iIndependent).getTeam()).declareWar(con.iByzantium, False, WarPlanTypes.WARPLAN_LIMITED)
	...
	utils.makeUnit(con.iManAtArms, con.iIndependent, tPlot, 3 + iHandicap, UnitAITypes.NO_UNITAI, [con.iCityRaider1])
	utils.makeUnit(con.iItalianCrossbowman, con.iIndependent, tPlot, 1, UnitAITypes.NO_UNITAI, [con.iDrill1, con.iDrill2])
	pLeader = utils.makeUnit(con.iManAtArms, con.iIndependent, tPlot, 1, UnitAITypes.NO_UNITAI, [con.iCityRaider1, con.iCityRaider2])
	pLeader.setHasPromotion(con.iLeader, True)
	pLeader.setExperience(20, -1)
	pLeader.setLeaderUnitType(con.iGreatGeneral2)
	pLeader.setName(localText.getText("TXT_KEY_BARB_BONIFACE",()))
...
def canTriggerMansaMusa(argsList):
	...
	if kTriggeredData.ePlayer not in [con.iFatimids, con.iMamluks, con.iAyyubids]:
		return false
	...
	if kTriggeredData.ePlayer != gc.getMap().plot(con.tFustat[0], con.tFustat[1]).getOwner():
		return false
...
def canTriggerReligiousLaw(argsList):
	...
	if player.getStateReligion() not in [con.iSunni, con.iShia]:
		return false
...
def canTriggerReligiousSupporters(argsList):
	...
	if player.getStateReligion() not in [con.iSunni, con.iShia, con.iHinduism, con.iCatholicism]:
		return false
...
def applyReligiousSupporters1(argsList):
	...
	if iStateReligion == con.iSunni or iStateReligion == con.iShia:
		unitList = [con.iMaceman_Harafisha, con.iSwordsman_Arab, con.iSpearman_Harafisha, con.iLancer_Syrian]
	elif iStateReligion == con.iCatholicism:
		unitList = [con.iSwordsman, con.iSpearman, con.iArcher_Syrian]
