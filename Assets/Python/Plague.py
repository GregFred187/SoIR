...
class Plague:
	...
	def setup(self):
		...
		#sd.setGenericPlagueDates(0, getTurnForYear(755 + gc.getGame().getSorenRandNum(16, 'Variation'))) # 755-770
		sd.setGenericPlagueDates(0, getTurnForYear(1350 + gc.getGame().getSorenRandNum(40, 'Variation') - 20)) # 1330-1370
		sd.setGenericPlagueDates(1, getTurnForYear(1420 + gc.getGame().getSorenRandNum(40, 'Variation') - 20)) # 1400-1440
		...
		for x in range(city.getX()-2, city.getX()+3):
			for y in range(city.getY()-2, city.getY()+3):
				...
				if (iImprovement == con.iVillage):
					pCurrent.setImprovementType(con.iCottage) # was hamlet
				if (iImprovement == con.iTown):
					pCurrent.setImprovementType(con.iHamlet) # was town
				if (pCurrent.isCity()):
					if (x == city.getX() and y == city.getY()):
