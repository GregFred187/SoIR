...
class CvGameUtils:
	...
	def cannotDoCivic(self,argsList):
		...
		if eCivic == con.iSlaveryCivic:
			...
			if eStateReligion == con.iCatholicism or eStateReligion == con.iOrthodoxy or eStateReligion == con.iHinduism:
				return True
	...
	def doInquisitorCore_AI(self, pUnit):
		...
		#utils.echo("doInquisitorCore_AI (%s)" %(gc.getPlayer(pUnit.getOwner()).getCivilizationShortDescription(0)))
		...
		for pCity in apCityList:
			for iReligion in con.tPersecutionOrder[iStateReligion]:
				if iReligion != iStateReligion and not pCity.GetCy().isHolyCityByType(iReligion):
					
					bFound = False
					if iTolerance <= 80:
						if iReligion == con.iZoroastrianism: bFound = True
					if iTolerance <= 60:
						if iReligion == con.iJudaism: bFound = True
					if iTolerance <= 50:
						if iReligion == con.iSunni or iReligion == con.iShia:
							if iStateReligion != con.iSunni and iStateReligion != con.iShia: 
								bFound = True
						elif iReligion == con.iCatholicism or iReligion == con.iOrthodoxy:
							if iStateReligion != con.iCatholicism and iStateReligion != con.iOrthodoxy: 
								bFound = True
						elif (iReligion == con.iHinduism or iReligion == con.iBuddhism) and iStateReligion != con.iHinduism:
							bFound = True
					if iTolerance <= 10: 
						bFound = True
	...
	def doInquisitorMove(self, pUnit, pCity):
		...
		#utils.echo("doInquisitorMove (%d,%d) to (%d,%d)" %(pUnit.getX(), pUnit.getY(), pCity.getX(), pCity.getY()))
	...
	# def doPilgrimCore_AI(self, pUnit):
		
		# tCoords = utils.getPilgrimageSite(pUnit.getOwner())
		# pHolyPlot = gc.getMap().plot(tCoords[0], tCoords[1])
		# if pUnit.generatePath(pHolyPlot, 0, False, None):
			# self.doPilgrimMove(pUnit, pHolyPlot)
			# return


	# def doPilgrimMove(self, pUnit, pPlot):
		
		# if pUnit.getX() != pPlot.getX() or pUnit.getY() != pPlot.getY():
			# pUnit.getGroup().pushMission(MissionTypes.MISSION_MOVE_TO, pPlot.getX(), pPlot.getY(), 0, False, True, MissionAITypes.NO_MISSIONAI, pUnit.plot(), pUnit)
		# else:
			# utils.doPilgrimageSuccess(pPlot.getX(), pPlot.getY(), pUnit.getOwner(), pUnit.getID())
	...
	def getWidgetHelp(self, argsList):
		...
		# edead: Religious Persecutor's button
		if iData1 == 666:
		...
		# edead: Missionary's Relic button
		if iData1 == iData2:
			for i in con.relics.keys():
				if iData1 == i + 700:
					szName = gc.getBuildingInfo(i).getText()
					return CyTranslator().getText("TXT_KEY_ACTION_LOAD_RELIC", (szName, ))
